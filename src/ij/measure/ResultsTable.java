...
/**
 * This is a table for storing measurement results and strings as columns of
 * values.
 * Call the static ResultsTable.getResultsTable() method to get a reference to
 * the
...
public class ResultsTable implements Cloneable {
	...
	/**
	 * Returns the ResultsTable with the specified title, or null if it does not
	 * exist,
	 */
	...
	public double getValue(String column, int row) {
		...
		if (col == COLUMN_NOT_FOUND)
			throw new IllegalArgumentException("\"" + column + "\" column not found");
		// IJLog.log("col: "+col+" "+(col==COLUMN_NOT_FOUND?"not
		// found":""+columns[col]));
	...
	/**
	 * Returns the label of the specified row. Returns null if the row does not have
	 * a label.
	 */
	...
	private String getValueAsString(int column, int row) {
		...
		// IJLog.log("getValueAsString1: col="+column+ ", row= "+row+", value=
		// "+value+", size="+stringColumns.size());
		if (Double.isNaN(value) && stringColumns != null) {
			String string = "NaN";
			ArrayList stringColumn = (ArrayList) stringColumns.get(Integer.valueOf(column));
			if (stringColumn == null)
				return string;
			// IJLog.log("getValueAsString2: "+column+ +row+" "+stringColumn.size());
	...
	/**
	 * @deprecated
	 *             Replaced by addValue(String,double) and
	 *             setValue(String,int,double)
	 */
	...
	public synchronized void deleteRow(int rowIndex) {
		...
		for (int col = 0; col <= lastColumn; col++) {
			if (columns[col] != null) {
				...
				ArrayList stringColumn = stringColumns != null ? (ArrayList) stringColumns.get(Integer.valueOf(col))
						: null;
	...
	public void update(int measurements, ImagePlus imp, Roi roi) {
		...
		// IJLog.log(rt2.getColumnHeadings());
		int last = rt2.getLastColumn();
		// IJLog.log("update1: "+last+" "+getMaxColumns());
		while (last + 1 >= getMaxColumns()) {
			addColumns();
			// IJLog.log("addColumns: "+getMaxColumns());
		}
		...
		for (int i = 0; i <= last; i++) {
			// IJLog.log(i+" "+rt2.getColumn(i)+" "+columns[i]+" "+rt2.getColumnHeading(i)+"
			// "+getColumnHeading(i));
	...
	/**
	 * Applies a macro to each row of the table; the columns are assigned variable
	 * names
	 * as given by getHeadingsAsVaribleNames(). New variables starting with an
	 * uppercase letter
	 * create a new column with this name.
	 * The variable 'row' (the row index) is pre-defined.
	 * Except for the row label (if existing), currently only supports numeric
	 * values, no Strings.
	 * 
	...
	public boolean applyMacro(String macro) {
		...
		sb.append("var ");
		for (int i = 0; i < columnNames.length; i++) { // create 'var' statement with 'real' data values, so errors are
														// less likely
			...
			if (columnIndices[i] < 0)
				...
				sb.append(Math.abs(getValueAsDouble(columnIndices[i], 0))); // avoid negative values since minus would
																			// be extra token
			sb.append(',');
		}
		sb.append("row;\n");
	...
	/**
	 * Returns the first index of a given non-null String in a String array, or -1
	 * if not found
	 */
	...
	/**
	 * Returns the column headings; headings not suitable as variable names are
	 * converted
	 * to valid variable names by replacing non-fitting characters with underscores
	 * and
	 * adding underscores. To make unique names, underscores+numbers are added as
	 * required.
	 */
