 ...
 * (i) The range of x values should not be too far from 0, especially for
 * higher-order polynomials.
 * For polynomials of 4th order, the average x value should fulfill |xMean| <
 * 2*(xMax-xMin).
 * For polynomials of 5th order or higher, the x range should encompass x=0; and
 * for
 * 7th and 8th order it is desirable to have x=0 near the center of the x range.
 * (ii) In contrast to some fitting algorithms using the normal equations and
 * matrix inversion, the
 * simplex algorithm used here can cope with parameters having very different
 * orders of magnitude,
 * as long as the coefficients of the polynomial are within a reasonable range
 * (say, 1e-80 to 1e80).
 * Thus, it is usually not needed to scale the x values, even for high-order
 * polynomials.
 *
 * Version history:
 *
 * 2008-01-21: Modified to do Gaussian fitting by Stefan Woerz (s.woerz at
 * dkfz.de).
 * 2012-01-30: Modified for external Minimizer class and UserFunction fit by
 * Michael Schmid.
 * - Also fixed incorrect equation string for 'Gamma Variate' & 'Rodbard (NIH
 * Image)',
 * - Added 'Inverse Rodbard', 'Exponential (linear regression)', 'Power (linear
 * regression)'
 * functions and polynomials of order 5-8. Added 'nicely' sorted list of types.
 * - Added absolute error for minimizer to avoid endless minimization if exact
 * fit is possible.
 ...
 * - Linear regression for up to 2 linear parameters, reduces the number of
 * parameters handled
 * by the simplex Minimizer and improves convergence. These parameters can be an
 * offset and
 * either a linear slope or a factor that the full function is multiplied with.
 * 2012-10-07: added GAUSSIAN_NOOFFSET fit type
 * 2012-11-20: Bugfix: exception on Gaussian&Rodbard with initial params, bad
 * initial params for Gaussian
 * 2013-09-24: Added "Exponential Recovery (no offset)" and "Chapman-Richards"
 * (3-parameter;
 * used e.g. to describe forest growth) fit types.
 * 2013-10-11: bugfixes, added setStatusAndEsc to show iterations and enable
 * abort by ESC
 * 2015-03-26: bugfix, did not use linear regression for RODBARD
 * 2016-11-28: added static getNumParams methods
 * 2018-03-23: fixes NullPointerException for custom fit without
 * initialParamVariations
...
public class CurveFitter implements UserFunction {
	...
	}; // fList, doFit(), getNumParams() and makeInitialParamsAndVariations() must also
		// be updated
	...
	private double sumY = Double.NaN, sumY2 = Double.NaN; // sum(y*w), sum(y^2*w) of the data used for fitting
															// (w=weight, default 1)
	 ...
	 * Use getStatus() and/or getStatusString() to see whether fitting was
	 * (probably) successful and
	...
	public void doFit(int fitType, boolean showSettings) {
		...
		if (isModifiedFitType(fitType)) // these fits don't use the original data and a different fit type
										// (this.fitType)
			if (!prepareModifiedFitType(fitType))
				return;
		numParams = getNumParams();
		if (fitType != CUSTOM)
			getOffsetAndFactorParams();
		// IJLog.log("special params: off="+offsetParam+(hasSlopeParam ? " slo=" : "
		// fac=")+factorParam+" numPar="+numParams+"
		// numRegressPar="+numRegressionParams);
		calculateSumYandY2(); // sumY, sumY2 needed for regression, abs Error; R, goodness of modified fit
								// functions
		...
		if (this.fitType == STRAIGHT_LINE) { // no minimizer needed
			...
			minimizer.setExtraArrayElements(numRegressionParams); // reserve space for extra parameters if minimizer has
																	// fewer paramters
			if (macro != null)
				minimizer.setMaximumThreads(1); // macro interpreter does not allow multithreading
			...
			// {String s="initVariations:";for(int ii=0;ii<numParams;ii++)s+="
			// ["+ii+"]:"+IJ.d2s(initialParamVariations[ii],5,9);IJLog.log(s);}
			// {String s="minInitVariations:";for(int ii=0;ii<numParams;ii++)s+="
			// ["+ii+"]:"+IJ.d2s(minimizerInitialParamVariations[ii],5,9);IJLog.log(s);}
			// {String s="minInitPars:";for(int ii=0;ii<numParams;ii++)s+="
			// ["+ii+"]:"+IJ.d2s(minimizerInitialParams[ii],5,9);IJLog.log(s);}
		...
		switch (fitType) { // postprocessing for nicer output:
			case GAUSSIAN: // Gaussians: width (std deviation) should be >0
				finalParams[3] = Math.abs(finalParams[3]);
				break;
			case GAUSSIAN_NOOFFSET:
				finalParams[2] = Math.abs(finalParams[2]);
				break;
		}
	 ...
	 * When showSettings is true, pops up a dialog allowing the user to set the
	 * initial
	 * fit parameters and various numbers controlling the Minimizer
	 ...
	 * getParams() to access the result.
	 *
	 * For complicated fits and good performance, it is advisable to use the
	 * doCustomFit method with
	 ...
	 * getParams() to access the result.
	 *
	 * For getter performance, if possible it is advisable to first call
	 * setOffsetMultiplySlopeParams,
	 * to avoid searching for one or two parameters that can be calculated directly
	 * by linear regression.
	 *
	 * @param userFunction           A class instance implementing the userFunction
	 *                               interface. There, the
	 *                               fit function hould be defined by the method
	 *                               userFunction(params, x).
	 *                               This function must allow simultaneous calls in
	 *                               multiple threads.
	 * @param numParams              Number of parameters of the fit function.
	 * @param formula                A String describing the fit formula, may be
	 *                               null.
	 * @param initialParams          Starting point for the parameters; the fit
	 *                               function with these parameters
	 *                               must not return NaN for any of the data points
	 *                               given in the constructor (xData).
	 *                               initialParams may be null, then random values
	 *                               are used,
	 *                               with repeated tries if the userFunction returns
	 *                               NaN.
	 * @param initialParamVariations Each parameter is initially varied by up to +/-
	 *                               this value.
	 *                               If not given (null), initial variations are
	 *                               taken as
	 *                               10% of initial parameter value or 0.01 for
	 *                               parameters that are zero.
	 *                               When this array is given, all elements must be
	 *                               positive (nonzero).
	 *                               See Minimizer.minimize for details. Providing
	 *                               this array is
	 *                               especially valuable if one or more initial
	 *                               parameters have a value of 0.
	 * @param showSettings           Displays a popup dialog for modifying the
	...
	/**
	 * Sets weights of the data points. The 'weights' array must have the same
	 * length as the data arrays
	 * passed with the constructor. If the error bars of the data points are known,
	 * the weights
	 * should be proportional to 1/error^2.
	 * When weights are specified, note that 'getSumResidualsSqr' will return the
	 * weighted sum.
	 */
	...
	/**
	 * Returns a reference to the Minimizer used, for accessing Minimizer methods
	 * directly.
	 * Note that no Minimizer is used if fitType is any of STRAIGHT_LINE,
	 * EXP_REGRESSION,
	 ...
	 *                      E.g. multiplyParams=1 if f(p0, p1, p2, p3...) can be
	 *                      expressed as
	 *                      p1*func(p0, p2, p3, ...) or p0 +p1*func(p0, p2, p3, ...)
	 *                      with '0' being
	 *                      the offsetparam.
	 * @param slopeParam    Index of a parameter that is multiplied with x and then
	 *                      summed to the function.
	 *                      E.g. '1' for f(p0, p1, p2, p3...) = p1*x + func(p0, p2,
	 *                      p3, ...)
	 *                      Only one, multiplyParam and slopeParam can be used
	 *                      (ie.e, the other
	 *                      should be set to -1)
	 */
	public void setOffsetMultiplySlopeParams(int offsetParam, int multiplyParam, int slopeParam) {
		if (multiplyParam >= 0 && slopeParam >= 0)
			throw new IllegalArgumentException(
					"CurveFitter: only one of multiplyParam and slopeParam may be given (i.e., >=0)");
	...
	/**
	 * Returns the number of parameters for a given fit type, except for the
	 * 'custom' fit,
	 * where the number of parameters is given by the equation: see
	 * getNumParams(String)
	 */
	...
	/**
	 * Returns the number of parameters for a custom equation given as a macro
	 * String,
	...
	public final double f(double[] p, double x) {
		if (fitType != CUSTOM)
			...
			if (macro == null) { // function defined in plugin
				...
				if (customParamCount > 1)
					macro.setVariable("b", p[1]);
				if (customParamCount > 2)
					macro.setVariable("c", p[2]);
				if (customParamCount > 3)
					macro.setVariable("d", p[3]);
				if (customParamCount > 4)
					macro.setVariable("e", p[4]);
				if (customParamCount > 5)
					macro.setVariable("f", p[5]);
	...
	public static double f(int fitType, double[] p, double x) {
		switch (fitType) {
			...
			case POLY8:
				return p[0] + x * (p[1]
						+ x * (p[2] + x * (p[3] + x * (p[4] + x * (p[5] + x * (p[6] + x * (p[7] + x * p[8])))))));
			case EXPONENTIAL:
			case EXP_REGRESSION:
	 ...
	 * Note that the array returned may have more elements than numParams; ignore
	 * the rest.
	 * May return an array with only NaN values if the minimizer could not start
	 * properly,
	 * i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE.
	 * See Minimizer.getParams() for details.
	 */
	public double[] getParams() {
		return finalParams == null ? minimizer.getParams() : finalParams; // if we have no result, take all_NaN result
																			// from the Minimizer
	}

	/**
	 * Returns residuals array, i.e., differences between data and curve.
	 * The residuals are with respect to the real data, also for fit types where the
	 * data are
	 * modified before fitting (power&exp fit by linear regression, 'Rodbard NIH
	 * Image' ).
	 * This is in contrast to sum of squared residuals, which is for the fit that
	 * was actually done.
	 */
	...
	/**
	 * Returns the sum of the residuals (may be NaN if the minimizer could not start
	 * properly
	 * i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE).
	 * If weights have been specified, each of the residuals is multiplied by the
	 * corresponding
	 ...
	 * Here, the standard deviation is defined here as the root-mean-square of the
	 * residuals
	 * times sqrt(n/(n-1)); where n is the number of points.
	 * If weights are provided, the standard deviation does not take the weights
	 * into account.
	 * With weights, the standard deviation and getSumResidualsSqr (which uses
	 * weights)
	 ...
	 * </pre>
	 * 
	 * For power, exp by linear regression and 'Rodbard NIH Image', this is
	 * calculated for the
	 ...
	 * Approaches R^2 if the number of points is much larger than the number of fit
	 * parameters.
	 * Assumes that the data points are independent (i.e., each point having a
	 * different x value).
	 * For power, exp by linear regression and 'Rodbard NIH Image', this is
	 * calculated for the
	 * fit actually done, not for the residuals of the original data.
	...
	/**
	 * Get a short text with a short description of the status. Should be preferred
	 * over
	...
	/**
	 * Get a string with detailed description of the curve fitting results (several
	 * lines,
	...
	/**
	 * Set maximum number of simplex restarts to do. See Minimizer.setMaxRestarts
	 * for details.
	 */
	...
	/**
	 * Set the maximum error. by which the sum of residuals may deviate from the
	 * true value
	...
	public void setMaxError(double maxRelError) {
		...
		if (maxRelError > 0.1)
			maxRelError = 0.1;
		if (maxRelError < 1e-16)
			maxRelError = 1e-16; // can't be less than numerical accuracy
	 ...
	 * 
	 * @param ijStatusString Displayed in the beginning of the status message. No
	 *                       display if null.
	 *                       E.g. "Curve Fit: Iteration "
	 * @param checkEscape    When true, the Minimizer stops if escape is pressed and
	 *                       the status
	 *                       becomes ABORTED. Note that checking for ESC does not
	 *                       work in the Event Queue thread.
	 */
	...
	/**
	 * Get number of iterations performed. Returns 1 in case the fit was done by
	...
	/**
	 * Get maximum number of iterations allowed (sum of iteration count for all
	 * restarts)
	 */
	...
	/**
	 * Set maximum number of iterations allowed (sum of iteration count for all
	 * restarts)
	 */
	...
	/**
	 * Get maximum number of simplex restarts to do. See Minimizer.setMaxRestarts
	 * for details.
	 */
	...
	/**
	 * Returns the status of the Minimizer after doFit. Minimizer.SUCCESS indicates
	 * a
	 * successful completion. In case of Minimizer.INITIALIZATION_FAILURE, fitting
	 * could
	 * not be performed because the data and/or initial parameters are not
	 * compatible
	 * with the function value. In that case, getStatusString may explain the
	 * problem.
	 * For further status codes indicating problems during fitting, see the status
	 * codes
	...
	/**
	 * Returns the code for a fit with given name as defined in fitList, or -1 if
	 * not found
	 */
	 ...
	 * This effectively reduces the number of free parameters by one or two and
	 * thereby
	...
	public final double userFunction(double[] params, double dummy) {
		...
		if (numRegressionParams == 0) { // simply calculate sum of residuals
			...
			// IJLog.log(IJ.d2s(params[0],3,5)+","+IJ.d2s(params[1],3,5)+":
			// r="+IJ.d2s(sumResidualsSqr,3,5)+Thread.currentThread().getName() );
		} else { // handle simple linear dependencies by linear regression:
			// if(getIterations()<1){String s="minimizerPar:";for(int
			// ii=0;ii<=numParams;ii++)s+=" ["+ii+"]:"+IJ.d2s(params[ii],5,9);IJLog.log(s);}
	...
	/**
	 * For fits where linear regression is used to reduce the number of parameters
	 * handled
	 * by the Minimizer, convert Minimizer parameters to the complete set of
	 * parameters.
	 ...
	 * The Minimizer stores the sum of squared residuals directly behind its last
	 * parameter.
	...
	private void minimizerParamsToFullParams(double[] params, boolean forRegression) {
		...
		double offset = 0;
		double factor = hasSlopeParam ? 0 : 1; // for regression, calculate function value without x*slope, but
												// multiplied with 1
		...
		if (!forRegression) { // recover regression-calculated parameters from extra array elements
			...
			if (offsetParam >= 0)
				offset = params[i];
			sumResidualsSqr = params[numParams - numRegressionParams]; // sum of squared residuals has been calculated
																		// already
	 ...
	 * The sum of squared residuals is written to the array element with index
	 * 'numParams',
	 * the offset and factor params (if any) are written to their proper positions
	 * in the
	 * params array
	 */
	private void doRegression(double[] params) {
		...
		for (int i = 0; i < numPoints; i++) {
			...
			sumWeights += w;
			// if(getIterations()==0)IJLog.log(xData[i]+"\t"+yData[i]+"\t"+fValue);
			// //x,y,function
		...
		if (offsetParam < 0) { // only 'factor' parameter, no offset:
			...
			// Don't report unrealistic or even negative values, otherwise minimization
			// could lead
			...
			// if(){IJLog.log("sumX="+sumX+" sumX2="+sumX2+" sumXY="+sumXY+"
	...
	/**
	 * Convert full set of parameters to minimizer parameters and returns the sum of
	 * residuals squared.
	 * The last array elements, not used by the minimizer, are the offset and factor
	 * parameters (if any)
	 */
	private double fullParamsToMinimizerParams(double[] params) {
		...
		for (int i = 0, iNew = 0; i < numParams; i++) // leave only the parameters for the minimizer in the beginning of
														// the array
	...
	/**
	 * In case one or two parameters are calculated by regression and not by the
	 * minimizer:
	...
	/**
	 * Estimate values for initial parameters and their typical range for the
	 * built-in
	 * function types. For fits with modifications for regression, 'fitType' is
	 * still
	...
	private boolean makeInitialParamsAndVariations(int fitType) {
		...
		if (fitType == CUSTOM) {
			for (int i = 0; i < numParams; i++) {
				...
				if (initialParamVariations[i] == 0)
					initialParamVariations[i] = 0.01; // should not be zero
			}
		...
		// We cannot fit the following cases because we would always get NaN function
		// values
		...
		if (!hasInitialParams) {
			switch (fitType) {
				// case POLY2: case POLY3: case POLY4: case POLY5: case POLY6: case POLY7: case
				// POLY8:
				// offset&slope calculated via regression; leave the others at 0

				// also for the other cases, some initial parameters are unused; only to show
				// them with 'showSettings'
				...
				case LOG2: // y = a+b*ln(x-c)
					initialParams[0] = yMean; // don't care, we will do this via regression
					initialParams[1] = (yMax - yMin + 1e-100) / (xMax - xMin + 1e-100); // don't care, we will do this
																						// via regression
				...
				case RODBARD2: // c*((x-a)/(d-x))^(1/b)
					...
					break;
				case GAMMA_VARIATE: // // b*(x-a)^c*exp(-(x-a)/d)
					// First guesses based on following observations (naming the 'x' coordinate
					// 't'):
					// t0 [a] = time of first rise in gamma curve - so use the user specified first
					// x value
					...
					initialParams[1] = yMax / (Math.pow(cd, initialParams[2]) * Math.exp(-cd / initialParams[3])); // don't
																													// care,
																													// we
																													// will
																													// do
				...
				case CHAPMAN: // a*(1-exp(-b*x))^c
					...
					if (Double.isNaN(initialParams[1]) || initialParams[1] > 1000. / xMax) // in case an outlier at the
																							// beginning has fooled us
				...
				case ERF: // a+b*erf((x-c)/d)
					...
					initialParams[1] = 0.5 * (yMax - yMin + 1e-100) * (lasty > firsty ? 1 : -1); // actually don't care,
																									// we will do this
																									// via regression
					initialParams[2] = xMin
							+ (xMax - xMin) * (lasty > firsty ? yMax - yMean : yMean - yMin) / (yMax - yMin + 1e-100);
		...
		if (!hasInitialParamVariations) { // estimate initial range for parameters
			...
			switch (fitType) {
				...
				case POLY8:
					...
					initialParamVariations[numParams - 1] = (yMax - yMin)
							/ (Math.pow(0.5 * (xMax - xMin), numParams - 1) + 1e-100);
				...
				case GAMMA_VARIATE: // // b*(x-a)^c*exp(-(x-a)/d); b calculated by regression
					...
					// t0 [b] = time of first rise in gamma curve - so use the user specified first
					// limit
	...
	/**
	 * Set multiplyParams and offsetParam for built-in functions. This allows us to
	 * use linear
	 * regression, reducing the number of parameters used by the Minimizer by up to
	 * 2, and
	...
	/**
	 * returns whether this a fit type that acutally fits modified data with a
	 * modified function
	 */
	...
	private boolean prepareModifiedFitType(int fitType) {
		if (fitType == GAUSSIAN) {
			...
			for (int i = 0; i < xData.length; i++) {
				...
				if (ySign == 0)
					ySign = Math.signum(y); // if unknown, determine whether y data are positive or negative
				if (y * ySign <= 0) {
		...
		} else if (fitType == RODBARD2) { // 'Rodbard (NIH Image)' fit is inverse Rodbard function, by fitting x(y); for
											// compatibility with NIH Image
			xDataSave = xData;
			yDataSave = yData;
	...
	/**
	 * Get correct params and revert xData, yData if fit has been done via another
	 * function
	 */
	...
	private void settingsDialog() {
		...
		for (int i = 0; i < numParams; i++) {
			...
			if (!Double.isNaN(p)) {
				initialParams[i] = p;
				initialParamVariations[i] = Math.max(0.01 * p, 0.001 * initialParamVariations[i]); // assume user-set
																									// params are
																									// accurate
			}
		}
	...
	public Plot getPlot(int points) {
		...
		legend.append(getName());
		legend.append('\n');
		legend.append(getFormula());
		legend.append('\n');
		double[] p = getParams();
		int n = getNumParams();
		char pChar = 'a';
		...
		legend.append("R^2 = " + IJ.d2s(getRSquared(), 4));
		legend.append('\n');
