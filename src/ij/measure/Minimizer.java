...
/**
 * Minimizer based on Nelder-Mead simplex method (also known as polytope
 * method),
 ...
 * - If outside contraction is rejected, instead of shrinking the whole simplex,
 * an inside
 * contraction is tried first. Own experiments show that this results in
 * slightly better
 * performance for some test functions (Perm, Mc Kinnon's function with tau=2,
 * theta=6,
 * Osborne1 curve-fitting problem). In most cases, there is no difference,
 * however.
 * - This implementation does not include any 'ordering rules' in case of equal
 * function values.
 * - When checking for convergence, a special iteration step may be performed,
 * improving
 ...
 * convergence
 * check might stop it prematurely. Therefore, the search is initialized again,
 * keeping the
 * best vertex of the simplex and setting the other vertices to random values,
 * but keeping
 * variations of the parameters w.r.t. the best vertex at a similar value. If
 * re-initializing
 * the simplex does not lead to a significant improvement, the value is accepted
 * as true
 * (local) minimum.
 *
 * Multiple minimization runs:
 * In spite of re-initializing (see above), there are rare cases where
 * minimization is stopped
 * too early. Also, minimization may result in a local minimum. Therefore,
 * unless determined
 * otherwise by setting 'setRestarts', two minimization runs with different
 * initialization
 * of the simplex are started in parallel threads. If the results don't agree
 * within the
 * error limit, two more minimization runs are started. This is repeated until
 * the two best
 * results agree within the error limits or the number of restarts (determined
 * by 'setRestarts';
 * default 2, i.e., up to 3 runs with two threads each) is exceeded.
 * This does not guarantee that the minimum is a global minimum, however: A
 * local minimum
 * will be accepted if the minimizer finds a local minimum twice (or two
 * different local
 * minima with the same function value within the error bounds), but no better
 * minimum has
 * been found at that time.
 *
 * The user-supplied target function should return NaN for out-of-bounds
 * parameters instead
 * of a high (penalty) value (minimization is faster and more reliable with
 * NaNs).
 * The region where the function is defined (e.g. not returning NaN) must be
 * convex.
 * Sharp corners of the region where the function value is defined (especially
 * in higher dimensions)
 * may cause a problem with finding suitable test points when (re-)initializing
 * the simplex.
 ...
 * 2013-09-24: 50% higher maximum iteration count, and never uses more than
 * 0.4*maxIter
 ...
 * 2014-09-16: normalization bug fixed. makeNewParamVariations checks for
 * paramResolutions
 *
 */
public class Minimizer {
    ...
    /**
     * Status returned: Could not initialize the simplex because either the
     * initialParams
     * resulted in the target function returning NaN or all attempts to find
     * starting
     * parameters for the other simplex points resulted in the target function
     * returning NaN.
    ...
    /**
     * Status returned: Could not reinitialize the simplex because all attempts to
     * find restarting
    ...
    private final static int ITER_FACTOR = 750; // maximum number of iterations per numParams^2; twice that value for 2
                                                // threads
    private final static int WORST = 0, NEXT_WORST = 1, BEST = 2;// indices in array to pass the numbers of the
                                                                 // respective vertices
    ...
    private int maxRestarts = 2; // number of times to try finding local minima; each uses 2 threads if
                                 // restarts>0
     ...
     * new Hashtable<Thread, double[][]>(); //for each thread, holds a reference to
     * its simplex
     */
     ...
     *                     This function must allow simultaneous calls in multiple
     *                     threads unless
     *                     setMaximumThreads(1); has been called.
     *                     Note that the function will be called with at least
     *                     numParams+1 array
     *                     elements; the last one is needed to store the function
     *                     value. Further
     *                     array elements for private use in the user function may
     *                     be added by
     ...
     * to reduce the probability of finding local minima or accepting one of the
     * rare
     ...
     * from the overhead of starting a new thread (typically < 1 ms), for
     * unproblematic
     * minimization problems on a dual-core machine this is almost as fast as
     * running
     * it once.
     *
     ...
     *
     * @param initialParams          Array with starting values of the parameters
     *                               (variables).
     *                               When null, the starting values are assumed to
     *                               be 0.
     *                               The target function must be defined (not
     *                               returning NaN) for
     *                               the values specified as initialParams.
     * @param initialParamVariations Parameters (variables) are initially varied by
     *                               up to +/-
     *                               this value. If not given (null), initial
     *                               variations are taken as
     *                               10% of initial parameter value or 0.01 for
     *                               parameters that are zero.
     *                               When this array is given, all elements must be
     *                               positive (nonzero).
     *                               If one or several initial parameters are zero,
     *                               is advisable to set the initialParamVariations
     *                               array to useful values indicating the typical
     *                               order of magnitude of the parameters.
     *                               For target functions with only one minimum,
     *                               convergence is fastest with large values of
     *                               initialParamVariations, so that the expected
     *                               value is within
     *                               initialParam+/-initialParamVariations.
     *                               If local minima can occur, it is best to use a
     *                               value close to the expected global minimum,
     *                               and rather small initialParamVariations, much
     *                               lower than the distance to the nearest local
     *                               minimum.
     *
    ...
    public int minimize(final double[] initialParams, final double[] initialParamVariations) {
        ...
        if (useSingleThread)
            maxLoopCount *= 2; // if we have only one thread, loop twice as many times
        for (int i = 0; i < maxLoopCount; i++) { // try several times, until we have twice the same result
            ...
            if (resultsVector.size() >= 2)
                return SUCCESS; // if we have two (almost) equal results, it's enough
        } // for i <= maxRestarts
        if (ijStatusString != null)
            IJ.showStatus(""); // reset status display
        return maxRestarts > 0 ? MAX_RESTARTS_EXCEEDED : // number of restarts exceeded without two equal results
                status; // if only one run was required, we can't have 2 equal results
    }

    /**
     * Perform minimization with the simplex method once, including
     * re-initialization until
     ...
     * @param initialParamVariations Parameters (variables) are initially varied by
     *                               up to +/-
     *                               this value. If not given (null), iniital
     *                               variations are taken as
     *                               10% of inial parameter value or 0.01 for
     *                               parameters that are zero.
     *                               When this array is given, all elements must be
     *                               positive (nonzero).
     *                               If one or several initial parameters are zero,
     ...
     * from the best corner of the simplex. Note that the array returned may have
     * more
     * elements than numParams; ignore the rest.
     * May return an array with only NaN values in case the minimize call has
     * returned
    ...
    /**
     * Get the value of the minimum, i.e. the value associated with the resulting
     * parameters
     * as obtained by getParams(). May return NaN in case the minimize call has
     * returned
     * an INITIALIZATION_FAILURE status or that abort() has been called at the very
     * beginning of the minimization.
     * Do not call this method before minimization.
    ...
    /**
     * Get number of iterations performed (includes all restarts). One iteration
     * needs
    ...
    /**
     * Set maximum number of iterations allowed (including all restarts and all
     * threads).
     ...
     * Default value is 1000 * numParams^2 (half that value if maxRestarts=0), which
     * is
     ...
     * Thus, on dual-core machines running no other jobs, values of n=1 or n=2
     * (default)
     * do not cause a notable increase of computing time for 'easy' optimization
     * problems,
     ...
     * (there, at least one restart is performed, and restart is repeated until the
     * result
     * does not change within the error bounds).
     * This value does not affect the 'minimizeOnce' function call.
     * When setting the maximum number of restarts to a value much higher than 3,
     * remember
    ...
    /**
     * Get number of minimizations completed (i.e. not aborted or stopped because
     * the
     * number of minimization was exceeded). After a minimize(..) call, typically 2
     * for unproblematic cases. Higher number indicate a functin that is difficult
     * to
    ...
    /**
     * Set a seed to initialize the random number generator, which is used for
     * initialization
     * of the simplex.
     */
     ...
     * relative error of the function value is more than this number (Default:
     * 1e-10).
     */
    ...
    /**
     * Sets the accuracy of convergence. Minimizing is done as long as the
     * relative error of the function value is more than maxRelError (Default:
     * 1e-10)
    ...
    /**
     * Set the resolution of the parameters, for problems where the target function
     * is not smooth
     * but suffers from numerical noise. If all parameters of all vertices are
     * closer to the
     * best value than the respective resolution value, minimization is finished,
     * irrespective
    ...
    /**
     * Call setMaximuThreads(1) to avoid multi-threaded execution (in case the
     * user-provided
     * target function does not allow moultithreading). Currently a maximum of 2
     * thread is used
    ...
    /**
     * Aborts minimization. Calls to getParams() will return the best solution found
     * so far.
     * This method may be called from the user-supplied target function.
     * If displayStatusAndCheckEsc has been called before, the Minimizer itself
    ...
    /**
     * Add a given number of extra elements to array of parameters (independent
     * vaiables)
     * for private use in the user function. Note that the first numParams+1
     * elements
    ...
    /**
     * Get the full simplex of the current thread. This may be useful if the target
     * function
    ...
    /**
     * One minimization run (including reinitializations of the simplex until the
     * result is stable)
     */
    private void minimizeOnce(double[] initialParams, double[] initialParamVariations, int seed) {
        ...
        // reinitialize until converged or error/aborted (don't care about
        // reinitialization failure in other thread)
        boolean reinitialisationFailure = false;
        while (status == SUCCESS || status == REINITIALIZATION_FAILURE) {
            double[] paramVariations = makeNewParamVariations(simp, bestVertexNumber, initialParams,
     ...
     * Note that one call to this function never does more than 0.4*maxIter
     * iterations.
     * 
    ...
    private int minimize(double[][] simp) {
        ...
        while (true) {
            ...
            if ((ijStatusString != null || checkEscape) && totalNumIter > nextIterationForStatus) {
                ...
                nextIterationForStatus = totalNumIter + (int) (totalNumIter * 500L / (time - startTime + 1)); // next
                                                                                                              // display
                                                                                                              // 0.5 sec
                                                                                                              // later
                if (time - startTime > 1000L) { // display status and check for ESC after the first second
                    ...
                    if (ijStatusString != null) {
                        ...
                        if (checkEscape)
                            statusString += " ESC to stop";
                        IJ.showStatus(statusString);
                    }
                }
            }
        }
        // showSimplex(simp, "after "+totalNumIter+" iterations:
        // value="+value(simp[best]));
        return best;
    }

    /**
     * Move along the line between center and worst, where +1 corresponds to
     * worstVertex
    ...
    private double[][] makeSimplex(double[] initialParams, double[] initialParamVariations, Random random) {
        ...
        if (initializeSimplex(simp, initialParamVariations, random))
            ...
            if (IJ.debugMode)
                showSimplex(simp, "Error: Could not make simplex vertices not yielding NaN");
            return null;
        }
    }
    ...
    /**
     * Whether the distance between two vertices is less than the resolution of the
     * parameters
     */
    ...
    /**
     * Find initial parameters not yielding a result of NaN in case those given
     * yield NaN
     * Called with params containing the initial parameters that have been tried
     * previously
     */
    private void findValidInitalParams(double[] params, double[] initialParamVariations, Random random) {
        final int maxAttempts = 50 * numParams * numParams; // max number of attempts to find params that do not lead to
                                                            // NaN
        double rangeFactor = 1; // will gradually become larger to handle different orders of magnitude
        final double rangeMultiplyLog = Math.log(1e20) / (maxAttempts - 1); // will try up to 1e-20 to
                                                                            // 1e20*initialParamVariations
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            if (!Double.isNaN(value(params)))
                return; // found a valid parameter set
        }
    }

    /**
     * Reinitialize an existing simplex: Create new vertices around the best one,
     * keeping the rough size of
     * the simplex. This helps to avoid premature termination or cases where the
     * simplex has become degenerate.
     * All points of the new simplex are evaluated already.
     * Returns false on error (if it could not create a simplex not resulting in
     * NaN).
     */
    private boolean reInitializeSimplex(double[][] simp, int bestVertexNumber, double[] paramVariations,
            Random random) {
    ...
    /**
     * Initialize simplex vertices except the number 0, which will be taken as
     * starting point.
     * All points of the new simplex are evaluated already.
     * Returns false on error (if it could not create a simplex not resulting in
     * NaN).
    ...
    private boolean initializeSimplex(double[][] simp, double[] paramVariations, Random random) {
        ...
        for (int i = 0; i < numParams; i++) {
            double range = paramVariations != null && i < paramVariations.length ? paramVariations[i]
                    : 0.1 * Math.abs(simp[0][i]); // if nothing else given, based on initial parameter
            ...
            variations[i] = range;
            // IJLog.log("param#"+i+" variation="+(float)variations[i]);
        }
        ...
        for (int v = 1; v < numVertices; v++) {
            ...
            do { // try finding a vertex that does not yield NaN
                if (numTries++ > maxAttempts)
                    return false;
                // Create a vector orthogonal to all others in a coordinate system where every
                // value is
                // normalized to its respective variations value. We first use this coordinate
                // system where
                // every parameter is in the [-1, +1] range.
                // Don't orthogonalize after getting only NaN function values in many attempts;
                // it might be
                ...
                if (numTries < maxAttempts / 3) // (don't orthogonalize if finding valid params was very difficult
                    // IJLog.log("orthogonalize vertex "+v);
                ...
                if (sumSqr < 1e-14)
                    continue; // bad luck with random numbers, excursion vector almost zero
     ...
     * happen that one parameter has the same value for all vertices of the
     * simplex].
     *
     * For each parameter, as a variation value we use the typical difference w.r.t.
     * the best
     * vertex of the simplex.
     * To avoid a degenerate simplex, we have to know what the the relative orders
     * of magnitude of
     * the parameter variations should be. If initialParamVariations are given, we
     * use them as
     * indication of the typoical relative values; otherwise we use the parameter
     * values themselves
     * and assume that the variation of each parameter should be roughly the same
     * fraction of the value.
     * If the variation of a parameter in the simplex w.r.t. this estimated
     * variation is much
     * smaller (<10^3) than typical for the parameters, we increase the variation
     * for this parameter.
     */
    private double[] makeNewParamVariations(double[][] simp, int bestVertexNumber,
        ...
        for (int i = 0; i < numParams; i++) { // for all parameters
            ...
            relatedTo[i] = initialParamVariations != null && initialParamVariations.length >= numParams
                    ? initialParamVariations[i]
                    : // relate to initialParamVariations (if given),
        ...
        for (int i = 0; i < numParams; i++) {
            if (paramVariations[i] < relatedTo[i]
            ...
            if (paramResolutions != null && paramVariations[i] < 10 * paramResolutions[i])
                paramVariations[i] = 10 * paramResolutions[i];// parameter variation must be well above numeric noise
                                                              // limit
        }
    ...
    private void order(double[][] simp, int[] worstNextBestArray) {
        ...
        for (int i = 0; i < numVertices; i++) {
            if (value(simp[i]) < value(simp[best]))
                best = i;
            if (value(simp[i]) > value(simp[worst]))
                worst = i;
        }
        nextWorst = best;
        for (int i = 0; i < numVertices; i++)
