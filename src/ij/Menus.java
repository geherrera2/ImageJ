...
public class Menus {
	...
	String addMenuBar() {
		...
		help.addSeparator();
		addPlugInItem(help, "Dev. Resources...", "ij.plugin.BrowserLauncher(\"" + IJ.URL + "/developer/index.html\")",
				0, false);
		addPlugInItem(help, "Macro Functions...",
				"ij.plugin.BrowserLauncher(\"" + IJ.URL + "/developer/macro/functions.html\")", 0, false);
		...
		addPlugInItem(help, "Release Notes...", "ij.plugin.BrowserLauncher(\"http://wsr.imagej.net/notes.html\")", 0,
				false);
	...
	static void addItemSorted(Menu menu, MenuItem item, int startingIndex) {
		...
		for (int i = startingIndex; i < count; i++) {
			...
			// IJLog.log(i+ " "+itemLabel+" "+label + " "+(itemLabel.compareTo(label)));
	...
	InputStream autoGenerateConfigFile(String jar) {
		...
		// IJLog.log(""+(sb!=null?sb.toString():"null"));
		if (sb == null)
			return null;
		else
	...
	void setupPluginsAndMacrosPaths() {
		...
		if (IJ.debugMode) {
			IJLog.log("Menus.setupPluginsAndMacrosPaths");
			IJLog.log("   user.dir: " + currentDir);
			IJLog.log("   plugins.dir: " + System.getProperty("plugins.dir"));
			IJLog.log("   ImageJPath: " + ImageJPath);
			IJLog.log("   pluginsPath: " + pluginsPath);
		}
	}
	...
	static synchronized void removeWindowMenuItem(int index) {
		// IJLog.log("removeWindowMenuItem: "+index+" "+windowMenuItems2+"
		// "+window.getItemCount());
		if (ij == null)
			return;
		try {
