...
public class Prefs {
	...
	private static final int USE_POINTER = 1 << 0, ANTIALIASING = 1 << 1, INTERPOLATE = 1 << 2,
			ONE_HUNDRED_PERCENT = 1 << 3,
			BLACK_BACKGROUND = 1 << 4, JFILE_CHOOSER = 1 << 5, UNUSED = 1 << 6, BLACK_CANVAS = 1 << 7,
			WEIGHTED = 1 << 8,
	...
	/**
	 * Assume DICOM volumes use identical RescaleSlope and RescaleIntercept across
	 * all slices
	 */
	...
	/**
	 * Use the file chooser to import and export image sequences on Windows and
	 * Linux
	 */
	...
	public static Point getLocation(String key) {
		...
		if (bounds != null && p.x + 100 <= bounds.x + bounds.width && p.y + 40 <= bounds.y + bounds.height) {
			if (locKeys.get(key) == null) { // first time for this key?
				...
				// System.out.println("getLoc: "+key+"
				// "+(ijLoc!=null&&primaryScreen.contains(ijLoc)) + "
				// "+!primaryScreen.contains(p));
	...
	/**
	 * Returns the number of threads used by PlugInFilters to process images and
	 * stacks.
	 */
