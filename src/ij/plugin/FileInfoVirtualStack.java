...
public class FileInfoVirtualStack extends VirtualStack implements PlugIn {
	...
	public ImageProcessor getProcessor(int n) {
		...
		if (n < 1 || n > nImages)
			throw new IllegalArgumentException("Argument out of range: " + n);
		// if (n>1) IJLog.log(" "+(info[n-1].getOffset()-info[n-2].getOffset()));
		...
		if (IJ.debugMode) {
			...
			ip = fo.openProcessor();
			IJLog.log("FileInfoVirtualStack: " + n + ", offset=" + info[n - 1].getOffset() + ", "
		...
		if (ip != null) {
			...
			IJLog.log("Read error or file not found (" + n + "): " + info[n - 1].directory + info[n - 1].fileName);
			switch (getBitDepth()) {
				case 8:
