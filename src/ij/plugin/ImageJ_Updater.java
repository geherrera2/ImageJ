...
public class ImageJ_Updater implements PlugIn {
	...
	public void run(String arg) {
		...
		if (IJ.debugMode)
			IJLog.log("Updater (jar loc): " + ij_jar);
	...
	int showDialog(String[] versions) {
		...
		String msg = "You are currently running v" + ImageJ.VERSION + ImageJ.BUILD + ".\n" +
				" \n" +
	...
	byte[] getJar(String address) {
		...
		try {
			...
			if (IJ.debugMode)
				IJLog.log("Updater (url): " + address + " " + len);
			if (len <= 0)
				return null;
	...
	void updateMenus() {
		if (IJ.debugMode) {
			...
			IJLog.log("Refresh Menus: " + (System.currentTimeMillis() - start) + " ms");
		} else
			Menus.updateImageJMenus();
	}
