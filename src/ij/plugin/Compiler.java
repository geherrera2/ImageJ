...
public class Compiler implements PlugIn, FilenameFilter {
	...
	void compileAndRun(String path) {
		...
		if (!isJavac()) {
			if (IJ.debugMode)
				IJLog.log("Compiler: javac not found");
	...
	private void checkForUpdate(String plugin, String currentVersion) {
		...
		if (!f.exists() || !f.canWrite()) {
			if (IJ.debugMode)
				IJLog.log("checkForUpdate: jar not found (" + plugin + ")");
			return;
		}
		String version = null;
		try {
		...
		if (version == null) {
			if (IJ.debugMode)
				IJLog.log("checkForUpdate: class not found (" + className + ")");
			return;
		}
		if (version.compareTo(currentVersion) >= 0) {
			if (IJ.debugMode)
				IJLog.log("checkForUpdate: up to date (" + className + "  " + version + ")");
			return;
		}
		boolean ok = Macro_Runner.downloadJar(plugin);
		if (IJ.debugMode)
			IJLog.log("checkForUpdate: " + className + " " + version + " " + ok);
	}
	...
	void addJars(String path, StringBuffer sb) {
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (f2.isDirectory())
				...
				// IJLog.log("javac classpath: "+path+list[i]);
			}
		}
	}
			...
			if (okay) {
				if (msg.startsWith("Compile")) {
					if (!(lcName.endsWith(".java") || lcName.endsWith(".class"))) {
						...
						|| lcName.endsWith(".js") || lcName.endsWith(".py") || lcName.endsWith(".bsh"))) {
...
class PlugInExecuter implements Runnable {
	...
	void runCompiledPlugin(String className) {
		if (IJ.debugMode)
			IJLog.log("Compiler: running \"" + className + "\"");
		IJ.resetClassLoader();
		...
		try {
			...
			if (IJ.debugMode)
				IJLog.log("NoClassDefFoundError: " + err);
			...
			// Logger.getLogger(getClass().getName()).log(Level.SEVERE, null, e); //IDE
			// output
		}
	}
...
abstract class CompilerTool {
	...
	public static class JavaxCompilerTool extends CompilerTool {
		...
		public boolean compile(List sources, List options, StringWriter log) {
			if (IJ.debugMode)
				IJLog.log("Compiler: using javax.tool.JavaCompiler");
			try {
				...
				JavaCompiler.CompilationTask task = javac.getTask(log, fileManager, null, options, null,
						compilationUnits);
				fileManager.close();
				return task.call();
			} catch (Exception e) {
	...
	public static class LegacyCompilerTool extends CompilerTool {
		...
		public boolean compile(List sources, List options, StringWriter log) {
			if (IJ.debugMode)
				IJLog.log("Compiler: using com.sun.tools.javac");
			try {
