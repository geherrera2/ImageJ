...
public class RGBStackConverter implements PlugIn, DialogListener {
	...
	void compositeToRGB(CompositeImage imp, String title) {
		...
		// IJLog.log("HyperStackReducer-2: "+keep+" "+channels2+" "+slices2+"
		// "+frames2);
	...
	public void convertHyperstack(ImagePlus imp, ImagePlus imp2) {
		...
		if (overlay != null) {
			...
			if (slices2 > 1 && frames2 > 1)
				overlay2.crop(firstC, lastC, firstZ, lastZ, firstT, lastT);// imp2 is hyperstack : ROI's
																			// hypercoordinates are conserved but only
																			// those with C = 1 are displayed
			else
