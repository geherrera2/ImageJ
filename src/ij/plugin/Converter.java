...
public class Converter implements PlugIn {
	...
	public void run(String arg) {
		...
		if (imp != null) {
			...
			if (imp.isComposite() && arg.equals("RGB Color") && !imp.getStack().isRGB() && !imp.getStack().isHSB()
					&& !imp.getStack().isLab()) {
			...
			} else if (imp.isComposite() && imp.getNChannels() > 1 && imp.getNChannels() == imp.getStackSize()
					&& arg.equals("RGB Stack")) {
				...
				IJLog.log("<<Converter: image is locked (" + imp + ")>>");
		} else
			IJ.noImage();
	}
	 ...
	 * ("8-bit", "16-bit", "32-bit", "8-bit Color", "RGB Color", "RGB Stack", "HSB
	 * Stack", "Lab Stack" or "HSB (32-bit)").
	 */
		...
		try {
			if (stack != null) {
				...
				if (stack.isRGB() && item.equals("RGB Color")) {
					...
					if (win != null)
						new ImageWindow(imp, imp.getCanvas()); // replace StackWindow with ImageWindow
