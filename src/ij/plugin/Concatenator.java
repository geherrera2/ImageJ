...
public class Concatenator implements PlugIn, ItemListener {
	...
	/**
	 * Optional string argument sets the name dialog boxes if called from another
	 * plugin.
	 */
	...
	private ImagePlus createHypervol() {
		...
		for (int i = 0; i < images.length; i++) {
			if (images[i] != null) { // Should only find null imp if user has closed an image after starting plugin
										// (unlikely...)
				currentImp = images[i];
				...
				if (currentImp.getType() != dataType) {
					IJLog.log("Omitting " + imageTitles[i] + " - image type not matched");
					continue;
				}
	...
	private boolean showDialog() {
		...
		if (!macro) { // Monitor user selections
			...
			if (all_option)
				itemStateChanged(new ItemEvent(allWindows, ItemEvent.ITEM_STATE_CHANGED, null, ItemEvent.SELECTED));
		}
		gd.showDialog();
		...
		for (int i = 0; i <= nImages; i++) { // compile a list of images to concatenate from user selection
			if (all_windows) { // Useful to not have to specify images in batch mode
				...
				if (i == ((nImages + 1) < maxEntries ? (nImages + 1) : maxEntries))
					break;
				gd.setSmartRecording(i == nImages);
				index = gd.getNextChoiceIndex();
			}
			if (index >= nImages)
				break; // reached the 'none' string or handled all images (in case of all_windows)
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		if (ie.getSource() == allWindows) { // User selected / unselected 'all windows' button
			...
			for (Enumeration e = choices.elements(); e.hasMoreElements();) {
				...
				if (!foundNone) {
					c.setEnabled(true);
					if (c.getSelectedItem().equals(none))
						foundNone = true;
