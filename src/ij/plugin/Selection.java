...
public class Selection implements PlugIn, Measurements {
	 ...
	 * Java version:
	 * https://github.com/mdoube/BoneJ/blob/master/src/org/doube/geometry/FitCircle.
	 * java<br>
	...
	void fitCircle(ImagePlus imp) {
		...
		for (int iter = 1; iter <= IterMax; iter++) {
			...
			if (Math.abs(ynew) > Math.abs(yold)) {
				if (IJ.debugMode)
					IJLog.log("Fit Circle: wrong direction: |ynew| > |yold|");
				xnew = 0;
				break;
			}
			...
			if (iter >= IterMax) {
				if (IJ.debugMode)
					IJLog.log("Fit Circle: will not converge");
				xnew = 0;
			}
			if (xnew < 0) {
				if (IJ.debugMode)
					IJLog.log("Fit Circle: negative root:  x = " + xnew);
				xnew = 0;
			}
		}
		if (IJ.debugMode)
			IJLog.log("Fit Circle: n=" + n + ", xnew=" + IJ.d2s(xnew, 2) + ", iterations=" + iterations);
	...
	PolygonRoi trimPolygon(PolygonRoi roi, double length) {
		...
		// IJLog.log("trim: "+length+" "+threshold);
	...
	// hull
	/*
	 * int findFirstPoint(int[] xCoordinates, int[] yCoordinates, int n, ImagePlus
	 * imp) {
	...
	void createMask(ImagePlus imp) {
		...
		boolean overlay = imp.getOverlay() != null
				&& imp.getProcessor().getMinThreshold() == ImageProcessor.NO_THRESHOLD;
	...
	void createSelectionFromMask(ImagePlus imp) {
		...
		if (!IJ.isMacro())
			IJLog.log("Create Selection: threshold not set; assumed to be " + threshold + "-" + threshold);
		IJ.runPlugIn("ij.plugin.filter.ThresholdToSelection", "");
	}
	 ...
	 * bounding rectangles
	 * among the ones having a side that is colinear with an edge of the convex
	 * hull.
	 * 
	 * Loosely based on:
	 * H. Freeman and R. Shapira. 1975. Determining the minimum-area encasing
	 * rectangle for an arbitrary
	 * closed curve. Commun. ACM 18, 7 (July 1975), 409–413.
