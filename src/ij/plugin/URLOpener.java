...
public class URLOpener implements PlugIn {
	...
	public void run(String urlOrName) {
		if (!urlOrName.equals("")) {
			if (urlOrName.equals("cache"))
				...
				else if (imp.getNChannels() > 1 && fi != null && fi.description != null
						&& fi.description.indexOf("mode=") != -1) {
					int mode = IJ.COLOR;
		...
		else if (url.endsWith(".html") || url.endsWith(".htm") || url.endsWith(".pdf") || url.indexOf(".html#") > 0
				|| noExtension(url))
	...
	public static String[] getSampleImageNames() {
		...
		for (int i = 0; i < samplesMenu.getItemCount(); i++) {
			...
			if (label.contains("Cache Sample Images"))
				continue;
