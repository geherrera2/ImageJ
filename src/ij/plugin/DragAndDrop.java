...
public class DragAndDrop implements PlugIn, DropTargetListener, Runnable {
	...
	public void drop(DropTargetDropEvent dtde) {
		...
		try {
			...
			if (IJ.debugMode)
				IJLog.log("DragAndDrop.drop: " + flavors.length + " flavors");
			for (int i = 0; i < flavors.length; i++) {
				if (IJ.debugMode)
					IJLog.log("  flavor[" + i + "]: " + flavors[i].getMimeType());
	...
	public void dragEnter(DropTargetDragEvent e) {
		...
		if (IJ.debugMode)
			IJLog.log("DragEnter: " + e.getLocation());
	...
	public void dragOver(DropTargetDragEvent e) {
		if (IJ.debugMode)
			IJLog.log("DragOver: " + e.getLocation());
	...
	private void openURL(String url) {
		if (IJ.debugMode)
			IJLog.log("DragAndDrop.openURL: " + url);
		if (url != null)
			IJ.open(url);
	}
	...
	public void openFile(File f) {
		if (IJ.debugMode)
			IJLog.log("DragAndDrop.openFile: " + f);
		try {
			if (null == f)
				return;
