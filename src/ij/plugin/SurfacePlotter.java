...
public class SurfacePlotter implements PlugIn {
	...
	public void run(String arg) {
		...
		if (stackFlags == PlugInFilter.DOES_STACKS && img.getStack().getSize() > 1) {
			...
			for (int i = 2; i <= stackSource.getSize(); i++) {
				IJ.showStatus(
						"Drawing slice " + i + "..." + " (" + (100 * (i - 1) / stackSource.getSize()) + "% done)");
	...
	public ImageProcessor makeSurfacePlot(ImageProcessor ip) {
		...
		for (int row = 0; row < height; row++) {
			...
			if (showWireframe) {
				...
				ip2.drawLine(ixstart + width - 1, (int) (ystart + 255.5 - ydelta), ixstart + width - 1,
						(int) (ystart + 255.5 - (profile[width - 1] + ydelta)));
			}

			xstart += xinc;
			ystart += yinc;
			if ((row % 10) == 0)
				IJ.showProgress((double) row / height);
		}
	...
	void drawAndLabelAxis(ImageProcessor ip, ImageProcessor ip2, Rectangle roi) {
		...
		// IJLog.log("");
		// IJLog.log(min+" "+max+" "+cal.getCValue((int)min)+"
		// "+cal.getCValue((int)max));
		// ip2.putPixelValue(0,0,0);
		// boolean zeroIsBlack
		// IJLog.log(ip2.getPixelValue(+" "+max+" "+cal.getCValue((int)min)+"
		// "+cal.getCValue((int)max));
		if (cal.calibrated()) {
	...
	void drawAxis(ImageProcessor ip, int x1, int y1, int x2, int y2, String label, int offset, int offsetXDirection,
			int offsetYDirection, int labelSide) {
		if (blackFill)
