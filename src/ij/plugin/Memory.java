...
public class Memory implements PlugIn {
	...
	public void run(String arg) {
		...
		// IJLog.log("setting="+getMemorySetting()/(1024*1024)+"MB");
		// IJLog.log("maxMemory="+maxMemory()/(1024*1024)+"MB");
	}
	...
	long getMemorySetting(String file) {
		...
		if (IJ.debugMode)
			IJLog.log("getMemorySetting: " + path);
		f = new File(path);
		if (!f.exists()) {
			fileMissing = true;
			return 0L;
		...
		try {
			...
			index1 = s.indexOf("-mx");
			if (index1 == -1)
				index1 = s.indexOf("-Xmx");
			if (index1 == -1)
				return 0L;
			if (s.charAt(index1 + 1) == 'X')
				index1 += 4;
			else
				index1 += 3;
			index2 = index1;
