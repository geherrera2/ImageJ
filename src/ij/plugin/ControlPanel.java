...
/**
 * ControlPanel.
 * This plugin displays a panel with ImageJ commands in a hierarchical tree
 * structure.
 * 
...
public class ControlPanel implements PlugIn {
	 ...
	 * Delegates to the {@link recursesubMenu(Menu, DefaultMutableTreeNode} method
	 * to gather the root children.
	 *
	 */
	 ...
	 * the TreeNode argument
	 * with children TreeNode objects constructed on the menu items.
	 * Descendants can be intermediate-level nodes (submenus) or leaf nodes (i.e.,
	 * no children).
	 * Leaf nodes will only be added if there are any commands associated with them,
	 * i.e.
	 * their labels correspond to keys in the hashtable returned by
	 * <code>ij.Menus.getCommands()</code>
	 * except for the "Reload Plugins" menu item, for which a local action command
	 * string is assigned
	 * to avoid clashes with the action fired from ImageJ Plugins->Utilties->Reload
	 * Plugins
	 * menu item.<br>
	...
	/**
	 * Populates the <code>node</code> argument with items retrieved from the
	 * collection.
	 * Actually, the method delegates to {@link populateNode{String[], String[],
	 * DefaultMutableTreeNode)}.
	 * 
	 * @param collection Hashtable with `keys' (java.lang.String) representing a
	 *                   tree path which
	 *                   is to be added to this node, but excludes it; the path
	 *                   extends to the last child (leaf node).
	 *                   The `values' are (java.lang.String) as labels and user
	 *                   objects for the last child of the path.
	 *                   Actually, <code><strong>null<strong></code> elements for
	 *                   labels are allowed, in which case
	 *                   the last child will be constructed on the last token in the
	 *                   `key'.
	...
	/**
	 * Populates the <code>node</code> argument with items retrieved from the two
	 * String[] arguments.
	 * Delegates indirectly to
	 * {@link buildTreePath(String.String,String,DefaultMutableTreeNode)} to do the
	 * job.
	 * If either arguments are empty (i.e., length=0) or have different sizes, the
	 * method does nothing.
	 * 
	 * @param items  String array where each element is the source of a tree path
	 *               (see {@see buildTreePath(String, String,
	 *               DefaultMutableTreeNode)}
	 *               and
	 *               {@see buildTreePath(String,String,String,DefaultMutableTreeNode)}.
	 * @param labels String array where each element is the label of the root of the
	 *               tree path
	 * @param node   The TreeNode to be populated
	 */
	 ...
	 * Populates the <code>node</code> argument with a tree path constructed as
	 * described below:
	 * 
	 * @param source  String to be parsed in a tree path; must be composed of tokens
	 *                delimited by "/"
	 * @param label   The label (String) of the leaf node for this path. If
	 *                <code><strong>null</strong></code>
	 *                then the leafe nod will be constructed from the last token.
	 * @param command The command string of the action event fired upon clicking the
	 *                leaf node.
	 *                If <code><strong>null</strong></code>, then the last token
	 *                will be taken as action command.
	...
	private void buildTreePath(String source, String label, String command, DefaultMutableTreeNode topNode) {
		...
		// 2. maybe `local' was passed in from some plugin class finder, and is prefixed
		// by
		...
		// plugin with different arguments will show up as children of the same tree
		// branch
		...
		// 5. parse the tree path: this code is entirely different from the logic in
		// TreePanel,
		...
		//
		// use the name of the token for intermediate nodes, and the `label' argument
		// for
	 ...
	 *
	 * @param s A string with the structure "[item1,item2,...,itemn]", as returned
	 *          by
	 *          a call to the <code>toString()</code> method in the
	 *          <code>javax.swing.tree.TreePath</code> class.
	 *
	 */
	TreePanel newPanel(String path) {
		if (path.equals("Control_Panel.@Main"))
			path = "Control_Panel";
		path = key2pStr(path);
		TreePanel pnl = null;
	...
	/**
	 * All properties related to the ControlPanel have keywords starting with
	 * "Control_Panel".
	 * This is to facilitate the integration of these properties with ImageJ's
	 * properties database.
	 * The keywords are dot-separated lists of tokens; in each token, spaces are
	 * relaced by
	 * underscores. Each token represents a node, and hence the keyword represents a
	 * tree path.
	 * The values can be either:
	 * <ol>
	 * <li>a space-separated list of integers,
	 * indicating panel frame geometry <strong>in the following fixed
	 * order:</strong>
	 * x coordinate, y coordinate , width, height</li>
	 * <li>or the word "expand" which indicates that the path represented by the
	 * keyword is
	 * an expanded branch
	 * </li>
	 * </ol>
	...
	void saveProperties() {
		if (IJ.debugMode)
			IJLog.log("CP.saveProperties: " + propertiesChanged);
	...
	void recordGeometry(TreePanel panel) {
		...
		if (frame != null) {
			...
			if (pTitle.equals("Control_Panel"))
				pTitle = "Control_Panel.@Main";
			String geometry = xCoord + " " + yCoord + " " + width + " " + height;
			if (IJ.debugMode)
				IJLog.log("CP.recordGeometry: " + pTitle + " " + geometry);
	...
	void restoreGeometry(TreePanel panel) {
		...
		if (pTitle.equals("Control_Panel"))
			pTitle = "Control_Panel.@Main";
		if (IJ.debugMode)
			IJLog.log("CP.restoreGeometry: " + pTitle);
 ...
 *
 * <br>
 * This class lays out the ImageJ user plugins in a vertical, hierarchical
 * tree.. Plugins are launched by double-clicking on their names, in the
 * vertical tree.
 *
 * Advantages: uses less screen estate, and provides a realistic graphical
 * presentation of the plugins installed in the file system.<br>
 *
...
class TreePanel implements
	...
	private static final int _uparrow1_ctable[] = {
			0x21, 0xff000000, 0xff303030, 0xffaaaaaa, 0xffffffff, 0xff3c3c3c, 0xff252525, 0xffb6b6b6, 0xff585858,
			0xffc3c3c3, 0xff222222, 0xff2b2b2b, 0xff2e2e2e, 0xffa0a0a0,
			0xff808080
	};

	private static IndexColorModel iconCM = new IndexColorModel(8, _uparrow1_ctable.length, _uparrow1_ctable, 0, true,
			255, DataBuffer.TYPE_BYTE);
	private static final ImageIcon upIcon = new ImageIcon(
			Toolkit.getDefaultToolkit().createImage(new MemoryImageSource(16, 16, iconCM, _uparrow1_data, 0, 16)));
	...
	public void buildTreePanel() {
		...
		if (defaultLocation != null) {
			if (IJ.debugMode)
				IJLog.log("CP.buildTreePanel: " + defaultLocation);
	...
	void restoreExpandedNodes() {
		...
		// for(Enumeration e = root.children(); e.hasMoreElements();)
		{
			...
			if (!node.isLeaf() && node != root) {
				...
				} else if ((pcp.hasExpandedStateProperty(npS) || pTree.isExpanded(newPath))
						&& pcp.hasPanelShowingProperty(npS)) {
	...
	// we implement window listener directly so that we can catch ImageJ's window
	// events
	/**
	 * Upon window closing, the panel removes itself from the vector of visible
	 * panels (member of pcp).
	 * Then, if this is the only visible panel left, the properties are saved; else,
	 * if this is the
	...
	public void windowClosing(WindowEvent e) {
		if (IJ.debugMode)
			IJLog.log("CP.windowClosing: " + isMainPanel);
		if (isMainPanel)
			pcp.saveProperties();
	...
	public void treeExpanded(TreeExpansionEvent ev) {
		...
		// (DefaultMutableTreeNode)evPath.getLastPathComponent();
		String evPathString = ev.getPath().toString();
	...
	/**
	 * Fires an ActionEvent upon double-click on the plugin item (leaf node) in the
	 * JTree
	 */
	void toAction() {
	...
	void setVisible() {
		if (pFrame != null && !pFrame.isVisible()) {
			...
			// close expanded path to this panel in its parent panel (if visible and if the
			// path is expanded)
			DefaultMutableTreeNode parent = (DefaultMutableTreeNode) root.getParent();
			if (parent != null) {
