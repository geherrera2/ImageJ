...
public class AVI_Reader extends VirtualStack implements PlugIn {
	...
	private final static int NV12_COMPRESSION = 0x3231564E; // 'NV12' - y plane followed by 2x2 subsampled interleaved
															// U, V
	private final static int NV21_COMPRESSION = 0x3132564E; // 'NV21' - y plane followed by 2x2 subsampled interleaved
															// V, U
	...
	private final static int MJPG_COMPRESSION = 0x47504a4d; // 'MJPG' Motion JPEG, also reads compression of individual
															// frames
	...
	// constants used to read 'AVI 2' index chunks (others than those defined here
	// are not supported)
	...
	private int type0xdb, type0xdc; // video stream chunks must have one of these two types (e.g. '00db' for straem
									// 0)
	...
	private int frameNumber = 1; // frame currently read; global because distributed over 1st AVi and further
									// RIFF AVIX chunks
	 ...
	 * 
	 * @param arg String 'arg' may be used to select the path. If it is an empty
	 *            string,
	 *            a file open dialog is shown, and the resulting ImagePlus is
	 *            displayed thereafter.
	 *            The ImagePlus is not displayed if 'arg' is a non-empty String; it
	 *            can be
	...
	/**
	 * Opens an AVI file as a stack in memory or a virtual stack. The ImagePlus is
	 * not displayed.
	 */
	 ...
	 * @param firstFrame    Number of first frame to read (first frame of the file
	 *                      is 1)
	 * @param lastFrame     Number of last frame to read or 0 for reading all, -1
	 *                      for all but last...
	 ...
	 *         The stack returned may be non-null, but have a length of zero if no
	 *         suitable frames were found.
	 *         Use <code>getErrorText</code> to determine whether there has been an
	 *         error reading the file.
	 *         For virtual stacks, not that I/O errors may also occur later, when
	 *         reading the frames.
	 */
	 ...
	 * <code>makeStack</code> or null if no error
	 */
	...
	/**
	 * Returns an ImageProcessor for the specified slice of this virtual stack (if
	 * it is one)
	 * where {@literal 1<=n<=nslices}. Returns null if no virtual stack or no slices
	 * or error reading the frame.
	 */
	...
	/**
	 * Returns the label of the specified slice in this virtual stack (if it is
	 * one).
	 */
	...
	public void deleteSlice(int n) {
		if (frameInfos == null || frameInfos.size() == 0)
			return;
		if (n < 1 || n > frameInfos.size())
			throw new IllegalArgumentException("Argument out of range: " + n);
	...
	private boolean showDialog() {
		...
		gd.setInsets(0, 40, 5);
		gd.addMessage(
				"* Leave empty or set to 0 for reading to the end.\n   Also accepts e.g. -5 to skip last 5 frames.",
				IJ.font10, Color.darkGray);
		gd.showDialog();
		if (gd.wasCanceled())
			return false;
	...
	private void readAVI(String path) throws Exception, IOException {
		...
		if (isVirtual || firstFrame > 1) { // avoid scanning frame-by-frame where we only need the positions
			...
			if (hasIndex && (frameInfos == null || frameInfos.size() == 0)) { // got nothing from indx, attempt to read
																				// AVI 1 index 'idx1'
				...
				while (positionBehindMovie < fileSize - 8) {
					if (verbose)
						IJLog.log("searching for 'idx1' at 0x" + Long.toHexString(positionBehindMovie));
					...
					if (nextPosition >= 0) // AVI-1 index 'idx1' found
						break;
					positionBehindMovie += FOUR_GB; // maybe position was wrong because it was a 32-bit number, but >
													// 4GB?
				}
			}
			if (verbose)
				IJLog.log("'frameInfos' has " + frameInfos.size() + " entries");
		}
		...
		if (firstFrame > 1 && frameInfos.size() > 0) {
			...
			if (verbose)
				IJLog.log("directly go to frame " + firstFrame + " @ 0x" + Long.toHexString(frameInfo[0] - 8));
		...
		// IJLog.log("at 0x"+Long.toHexString(pos)+"
	...
	private void openAndReadHeader(String path) throws Exception, IOException {
		...
		if (verbose)
			IJLog.log("OPEN AND READ AVI FILE HEADER " + timeString());
		...
		if (verbose)
			IJLog.log("File header: File type='" + fourccString(fileType) + "' (should be 'RIFF')" + timeString());
		...
		if (verbose)
			IJLog.log("File header: RIFF type='" + fourccString(riffType) + "' (should be 'AVI ')");
	...
	/**
	 * Read AVIX chunks following the first RIFF AVI for large files (sequential
	 * reading frame-by-frame beyond the first chunk)
	 **/
	private void readAVIX(long endPosition) throws Exception, IOException {
		if (verbose)
			IJLog.log("Trying to read AVIX" + timeString());
		int riffType = readInt();
		if (verbose)
			IJLog.log("File header: RIFF type='" + fourccString(riffType) + "' (should be 'AVIX')");
	...
	private long findFourccAndRead(int fourcc, boolean isList, long endPosition,
		...
		do {
			...
			if (nextPos > endPosition || nextPos > fileSize) {
				errorText = "AVI File Error: '" + fourccString(type) + "' @ 0x"
						+ Long.toHexString(raFile.getFilePointer() - 8)
						+ " has invalid length. File damaged/truncated?";
				IJLog.log(errorText); // this text is also remembered as error message for showing in message box
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJLog.log("Search for '" + fourccString(fourcc) + "', found " + fourccString(type) + "' data "
						+ posSizeString(nextPos - size, size));
			if (type == fourcc) {
				contentOk = readContents(fourcc, nextPos);
			} else if (verbose)
				IJLog.log("'" + fourccString(type) + "', ignored");
			raFile.seek(nextPos);
			if (contentOk)
	...
	private long findFourccAndSkip(int fourcc, boolean isList, long endPosition) throws IOException {
		while (true) {
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJLog.log("Searching for (to skip) '" + fourccString(fourcc) + "', found " + fourccString(type) +
		...
		switch (fourcc) {
			...
			case FOURCC_strh:
				...
				if (streamType != FOURCC_vids) {
					if (verbose)
						IJLog.log("Non-video Stream '" + fourccString(streamType) + " skipped");
					streamNumber++;
					return false;
				}
				readStreamHeader();
	...
	void readAviHeader() throws Exception, IOException { // 'avih'
		...
		if (verbose) {
			IJLog.log("AVI HEADER (avih):" + timeString());
			IJLog.log("   dwMicroSecPerFrame=" + dwMicroSecPerFrame);
			IJLog.log("   dwMaxBytesPerSec=" + dwMaxBytesPerSec);
			IJLog.log("   dwReserved1=" + dwReserved1);
			IJLog.log("   dwFlags=" + dwFlags);
			IJLog.log("   dwTotalFrames=" + dwTotalFrames);
			IJLog.log("   dwInitialFrames=" + dwInitialFrames);
			IJLog.log("   dwStreams=" + dwStreams);
			IJLog.log("   dwSuggestedBufferSize=" + dwSuggestedBufferSize);
			IJLog.log("   dwWidth=" + dwWidth);
			IJLog.log("   dwHeight=" + dwHeight);
		}
	}
	...
	void readStreamHeader() throws Exception, IOException { // 'strh'
		...
		if (verbose) {
			IJLog.log("VIDEO STREAM HEADER (strh):");
			IJLog.log("   fccStreamHandler='" + fourccString(fccStreamHandler) + "'");
			IJLog.log("   dwStreamFlags=" + dwStreamFlags);
			IJLog.log("   wPriority,wLanguage=" + dwPriorityLanguage);
			IJLog.log("   dwStreamInitialFrames=" + dwStreamInitialFrames);
			IJLog.log("   dwStreamScale=" + dwStreamScale);
			IJLog.log("   dwStreamRate=" + dwStreamRate);
			IJLog.log("   dwStreamStart=" + dwStreamStart);
			IJLog.log("   dwStreamLength=" + dwStreamLength);
			IJLog.log("   dwStreamSuggestedBufferSize=" + dwStreamSuggestedBufferSize);
			IJLog.log("   dwStreamQuality=" + dwStreamQuality);
			IJLog.log("   dwStreamSampleSize=" + dwStreamSampleSize);
		}
		if (dwStreamSampleSize > 1)
			throw new Exception(
					"Video stream with " + dwStreamSampleSize + " (more than 1) frames/chunk not supported");
		// what the chunks in that stream will be named (we have two possibilites:
	...
	private void readAvi2Index(long endPosition) throws Exception, IOException {
		...
		if (verbose) {
			...
			IJLog.log("AVI 2 INDEX:");
			IJLog.log("   wLongsPerEntry=" + wLongsPerEntry);
			IJLog.log("   bIndexSubType=" + bIndexSubType);
			IJLog.log("   bIndexType=" + bIndexType + bIndexString);
			IJLog.log("   nEntriesInUse=" + nEntriesInUse);
			IJLog.log("   dwChunkId='" + fourccString(dwChunkId) + "'");
			if (bIndexType == AVI_INDEX_OF_CHUNKS)
				IJLog.log("   qwBaseOffset=" + "0x" + Long.toHexString(qwBaseOffset));
		}
		if (bIndexType == AVI_INDEX_OF_INDEXES) { // 'indx' points to other indices
			if (wLongsPerEntry != 4)
				return; // badly formed index, ignore it
			for (int i = 0; i < nEntriesInUse; i++) { // read all entries (each pointing to an ix00 index)
				...
				if (verbose)
					IJLog.log("   indx entry: '" + fourccString(dwChunkId) + "' incl header "
							+ posSizeString(qwOffset, dwSize) + timeString());
			...
			if (verbose) {
				IJLog.log("readAvi2Index frameNumber=" + frameNumber + " firstFrame=" + firstFrame);
				if (indexForCountingOnly)
					IJLog.log("<just counting frames, not interpreting index now>");
			}
			if (wLongsPerEntry != 2)
				return; // badly formed index, ignore it
			if (dwChunkId != type0xdb && dwChunkId != type0xdc) { // not the stream we search for? (should not happen)
				if (verbose)
					IJLog.log("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be " +
			...
			for (int i = 0; i < nEntriesInUse; i++) {
				...
				if (frameNumber >= firstFrame && dwSize > 0) { // only valid frames (no blank frames)
					...
					if (verbose)
						IJLog.log("movie data " + frameNumber + " '" + fourccString(dwChunkId) + "' "
								+ posSizeString(pos, dwSize) + timeString());
				}
				frameNumber++;
				if (frameNumber > lastFrameToRead)
					break;
			...
			if (verbose)
				IJLog.log("Index read up to frame " + (frameNumber - 1));
		}
	}

	/** Read AVI 1 index 'idx1' */
	private void readOldFrameIndex(long endPosition) throws Exception, IOException {
		// IJLog.log("READ AVI 1 INDEX, isOversizedAvi1="+isOversizedAvi1);
		int offset = -1; // difference between absolute frame address and address given in idx1
		int[] offsetsToTry = new int[] { 0, (int) moviPosition }; // dwOffset may be w.r.t. file start or w.r.t. 'movi'
																	// list.
		long lastFramePos = 0;
		while (true) {
			if ((raFile.getFilePointer() + 16) > endPosition)
				break;
			...
			int dwSize = readInt();
			// IJLog.log("idx1: dwOffset=0x"+Long.toHexString(dwOffset));
			// IJLog.log("moviPosition=0x"+Long.toHexString(moviPosition));
			if ((dwChunkId == type0xdb || dwChunkId == type0xdc) && dwSize > 0) {
				if (offset < 0) { // find out what the offset refers to
					...
					for (int i = 0; i < offsetsToTry.length; i++) {
						...
						if (pos < moviPosition)
							continue; // frame must be in 'movi' list
						raFile.seek(pos);
						int chunkIdAtPos = readInt(); // see whether this offset points to the desired chunk
						// IJLog.log("read@=0x"+Long.toHexString(pos)+":
						// '"+fourccString(chunkIdAtPos)+"'");
					...
					if (verbose)
						IJLog.log("idx1: dwOffsets are w.r.t. 0x"
								+ (offset < 0 ? " UNKONWN??" : Long.toHexString(offset)));
					raFile.seek(temp);
					if (offset < 0)
						return; // neither offset works
				}
				long framePos = (dwOffset & SIZE_MASK) + offset;
				if (isOversizedAvi1)
					while (framePos < lastFramePos)
						framePos += FOUR_GB; // index entries are modulo 2^32, assume frames are ascending
				...
				if (frameNumber >= firstFrame) {
					...
					if (verbose)
						IJLog.log("idx1 movie data '" + fourccString(dwChunkId) + "' " + posSizeString(framePos, dwSize)
								+ timeString());
				}
				frameNumber++;
				if (frameNumber > lastFrameToRead)
	...
	void readBitMapInfo(long endPosition) throws Exception, IOException {
		...
		if (verbose) {
			IJLog.log("   biSize=" + biSize);
			IJLog.log("   biWidth=" + biWidth);
			IJLog.log("   biHeight=" + biHeight);
			IJLog.log("   biPlanes=" + biPlanes);
			IJLog.log("   biBitCount=" + biBitCount);
			IJLog.log("   biCompression=0x" + Integer.toHexString(biCompression) + " '" + fourccString(biCompression)
					+ "'");
			IJLog.log("   biSizeImage=" + biSizeImage);
			IJLog.log("   biXPelsPerMeter=" + biXPelsPerMeter);
			IJLog.log("   biYPelsPerMeter=" + biYPelsPerMeter);
			IJLog.log("   biClrUsed=" + biClrUsed);
			IJLog.log("   biClrImportant=" + biClrImportant);
		}
		...
		switch (biCompression) {
			...
			case NV21_COMPRESSION:
				dataCompression = (dataCompression == IYUV_COMPRESSION) ? I420_COMPRESSION : biCompression;
		...
		if (isPlanarFormat && ((biWidth & 1) != 0 || (biHeight & 1) != 0))
			throw new Exception("Odd size (" + biWidth + "x" + biHeight + ") unsupported with "
					+ fourccString(biCompression) + " compression");
		// raw & interleaved YUV: scan line is padded with zeroes to be a multiple of
		// four bytes
		scanLineSize = isPlanarFormat ? (biWidth * biBitCount) / 8 : ((biWidth * biBitCount + 31) / 32) * 4;
		...
		if (verbose) {
			IJLog.log("   > data compression=0x" + Integer.toHexString(dataCompression) + " '"
					+ fourccString(dataCompression) + "'");
			IJLog.log("   > palette colors=" + biClrUsed);
			IJLog.log("   > scan line size=" + scanLineSize);
			IJLog.log("   > data top down=" + dataTopDown);
		}

		// read color palette
		if (readPalette && biClrUsed > 0) {
			if (verbose)
				IJLog.log("   Reading " + biClrUsed + " Palette colors: " + posSizeString(spaceForPalette));
	...
	/**
	 * Read from the 'movi' chunk. Skips audio ('..wb', etc.), 'LIST' 'rec' etc,
	 * only reads '..db' or '..dc'
	 */
	void readMovieData(long endPosition) throws Exception, IOException {
		if (verbose)
			IJLog.log("MOVIE DATA " + posSizeString(endPosition - raFile.getFilePointer()) + timeString() +
		...
		if (isVirtual) {
			if (frameInfos == null) // we might have it already from reading the first chunk
				frameInfos = new Vector<long[]>(lastFrameToRead); // holds frame positions in file (for non-constant
																	// frame sizes, should hold long[] with pos and
																	// size)
		} else if (stack == null)
		...
		while (true) { // loop over all chunks
			int type = readType(endPosition);
			if (type == 0)
				break; // endPosition of 'movi' reached?
			long size = readInt() & SIZE_MASK;
			long pos = raFile.getFilePointer();
			...
			if ((type == type0xdb || type == type0xdc) && size > 0) {
				IJ.showProgress((double) frameNumber / lastFrameToRead);
				if (verbose)
					IJLog.log(frameNumber + " movie data '" + fourccString(type) + "' " + posSizeString(size)
							+ timeString());
				if (frameNumber >= firstFrame) {
					if (isVirtual)
				...
				IJLog.log("skipped '" + fourccString(type) + "' " + posSizeString(size));
	...
	private Object readFrame(RandomAccessFile rFile, long filePos, int size)
		...
		rFile.seek(filePos);
		// if (verbose)
		// IJLog.log("virtual AVI: readFrame @"+posSizeString(filePos, size)+"
		// varlength="+variableLength);
	...
	/**
	 * For one line: Unpack and convert YUV or RGB video data to grayscale (byte
	 * array for ByteProcessor)
	 */
	...
	/**
	 * For one line: Unpack 16bit grayscale data and convert to short array for
	 * ShortProcessor
	 */
	...
	/**
	 * For one line: Read YUV, RGB or RGB+alpha data and writes RGB int array for
	 * ColorProcessor
	 */
	...
	void unpackPlanarImage(byte[] rawData, int[] cPixels, boolean topDown) {
		...
		int uvInc = (dataCompression == NV12_COMPRESSION || dataCompression == NV21_COMPRESSION) ? 2 : 1; // NV12, NV21
																											// have
																											// interleaved
																											// u,v
	...
	private int readType(long endPosition) throws IOException {
		while (true) {
			...
			long size = readInt() & SIZE_MASK;
			if (verbose)
				IJLog.log("Skip JUNK: " + posSizeString(size));
	...
	/**
	 * An input stream reading from a RandomAccessFile (starting at the current
	 * position).
	 * This class also adds 'Define Huffman Table' (DHT) segments to convert MJPG to
	 * JPEG.
	 */
	...
	final private static byte[] HUFFMAN_TABLES = new byte[] { // the 'DHT' segment
			...
			0x33, 0x62, 0x72, (byte) 0x82, 0x09, 0x0A, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
			0x34,
			...
			(byte) 0x99,
			(byte) 0x9A, (byte) 0xA2, (byte) 0xA3, (byte) 0xA4, (byte) 0xA5, (byte) 0xA6, (byte) 0xA7, (byte) 0xA8,
			(byte) 0xA9,
			...
			(byte) 0xE8,
			(byte) 0xE9, (byte) 0xEA, (byte) 0xF1, (byte) 0xF2, (byte) 0xF3, (byte) 0xF4, (byte) 0xF5, (byte) 0xF6,
			(byte) 0xF7,
			(byte) 0xF8, (byte) 0xF9, (byte) 0xFA, 0x11, 0x00, 0x02, 0x01, 0x02, 0x04, 0x04, 0x03, 0x04, 0x07, 0x05,
			0x04, 0x04, 0x00, 0x01,
			0x02, 0x77, 0x00, 0x01, 0x02, 0x03, 0x11, 0x04, 0x05, 0x21, 0x31, 0x06, 0x12, 0x41, 0x51, 0x07, 0x61, 0x71,
			0x13, 0x22, 0x32, (byte) 0x81, 0x08, 0x14, 0x42, (byte) 0x91, (byte) 0xA1, (byte) 0xB1, (byte) 0xC1, 0x09,
			0x23, 0x33,
			0x52, (byte) 0xF0, 0x15, 0x62,
			0x72, (byte) 0xD1, 0x0A, 0x16, 0x24, 0x34, (byte) 0xE1, 0x25, (byte) 0xF1, 0x17, 0x18, 0x19, 0x1A, 0x26,
			0x27, 0x28, 0x29, 0x2A,
			0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x53, 0x54, 0x55, 0x56,
			0x57, 0x58, 0x59, 0x5A, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
			0x79, 0x7A, (byte) 0x82, (byte) 0x83, (byte) 0x84, (byte) 0x85, (byte) 0x86, (byte) 0x87, (byte) 0x88,
			(byte) 0x89,
			(byte) 0x8A, (byte) 0x92, (byte) 0x93, (byte) 0x94, (byte) 0x95, (byte) 0x96, (byte) 0x97, (byte) 0x98,
			(byte) 0x99, (byte) 0x9A, (byte) 0xA2, (byte) 0xA3, (byte) 0xA4, (byte) 0xA5, (byte) 0xA6, (byte) 0xA7,
			(byte) 0xA8,
			(byte) 0xA9, (byte) 0xAA, (byte) 0xB2, (byte) 0xB3, (byte) 0xB4, (byte) 0xB5, (byte) 0xB6, (byte) 0xB7,
			(byte) 0xB8,
			...
			(byte) 0xD8,
			(byte) 0xD9, (byte) 0xDA, (byte) 0xE2, (byte) 0xE3, (byte) 0xE4, (byte) 0xE5, (byte) 0xE6, (byte) 0xE7,
			(byte) 0xE8,
			(byte) 0xE9, (byte) 0xEA, (byte) 0xF2, (byte) 0xF3, (byte) 0xF4, (byte) 0xF5, (byte) 0xF6, (byte) 0xF7,
			(byte) 0xF8,
			(byte) 0xF9, (byte) 0xFA };
	...
	class raInputStream extends InputStream {
		...
		public int read() throws IOException {
			...
			if (fixMJPG) {
				...
				if (bufferPointer >= bufferLength)
					fixMJPG = false; // buffer exhausted, no more attempt to fix it
				return result;
			} else
				return rFile.read();
		}
		...
		public int read(byte[] b, int off, int len) throws IOException {
			// IJLog.log("read "+len+" bytes, fix="+fixMJPG);
			int nBytes;
			if (fixMJPG) {
		...
		private void addHuffmanTables() {
			if (readShort(0) != 0xffd8 || bufferLength < 6)
				return; // not a start of JPEG-like data
			int offset = 2;
			int segmentLength = 0;
			do {
				int code = readShort(offset); // read segment type
				// IJLog.log("code=0x"+Long.toHexString(code));
		...
		private void insertHuffmanTables(int position) {
			// IJLog.log("inserting Huffman tables");
