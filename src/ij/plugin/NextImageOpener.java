...
public class NextImageOpener implements PlugIn {
	...
	public void run(String arg) {
		...
		if (IJ.debugMode)
			IJLog.log("OpenNext.currentPath:" + currentPath);
		if (currentPath == null) {
		...
		if (IJ.debugMode)
			IJLog.log("OpenNext.nextPath:" + nextPath);
		// open
		if (nextPath != null) {
	...
	String getNext(String path, String imageName, boolean forward) {
		...
		if (IJ.debugMode)
			IJLog.log("OpenNext.thisfile:" + thisfile);
		if (thisfile == -1)
		...
		while (candidate != thisfile) {
			...
			if (IJ.debugMode)
				IJLog.log("OpenNext: " + candidate + "  " + names[candidate]);
		...
		if (IJ.debugMode)
			IJLog.log("OpenNext: Search failed");
		return null;
	}
