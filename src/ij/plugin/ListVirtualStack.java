...
public class ListVirtualStack extends VirtualStack implements PlugIn {
	...
	public void run(String arg) {
		...
		// IJLog.log("ListVirtualStack: "+dir+" "+name);
		list = open(dir + name);
		if (list == null)
			return;
		nImages = list.length;
	...
	public ImageProcessor getProcessor(int n) {
		...
		if (imp != null) {
			...
			switch (getBitDepth()) {
				case 8:
					ip = new ByteProcessor(imageWidth, imageHeight);
					break;
				case 16:
					ip = new ShortProcessor(imageWidth, imageHeight);
					break;
				case 24:
					ip = new ColorProcessor(imageWidth, imageHeight);
					break;
				case 32:
					ip = new FloatProcessor(imageWidth, imageHeight);
					break;
			}
			IJ.redirectErrorMessages(false);
			return ip;
