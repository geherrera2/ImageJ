...
public class RGBStackMerge implements PlugIn {
	...
	public void mergeStacks() {
		...
		int frames = 0;
		for (int i = 0; i < maxChannels; i++) {
			// IJLog.log(i+" "+index[i]+" "+titles[index[i]]+" "+wList.length);
	...
	private String getName(int channel, String[] titles) {
		...
		for (int i = titles.length - 1; i >= 0; i--) {
			if (titles != null && titles[i].startsWith(str)
					&& (firstChannelName == null || titles[i].contains(firstChannelName))) {
				name = titles[i];
				if (channel == 1) {
	...
	public ImagePlus mergeHyperstacks(ImagePlus[] images, boolean keep) {
		...
		Color[] defaultColors = { Color.red, Color.green, Color.blue, Color.white, Color.cyan, Color.magenta,
				Color.yellow };
		...
		// IJLog.log("mergeHyperstacks: "+w+" "+h+" "+channels+" "+slices+" "+frames);
