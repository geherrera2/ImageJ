...
public class FolderOpener implements PlugIn, TextListener {
	...
	public void run(String arg) {
		...
		if (IJ.debugMode)
			IJLog.log("FolderOpener: " + directory + " (" + list.length + " files)");
			...
			for (int i = this.start - 1; i < list.length; i++) {
				...
				if (imp != null && stack == null) {
					...
					if (openAsVirtualStack) {
						...
						stack = new ImageStack((int) (width * this.scale / 100.0), (int) (height * this.scale / 100.0),
								cm);
					else
				...
				if (imp.getWidth() != width || imp.getHeight() != height) {
					if (stackWidth > 0 && stackHeight > 0) {
						...
						IJLog.log(list[i] + ": wrong size; " + width + "x" + height + " expected, " + imp.getWidth()
								+ "x" + imp.getHeight() + " found");
						continue;
					}
				}
		...
		if (stack != null && stack.size() > 0) {
			...
			if (info1 != null && info1.lastIndexOf("7FE0,0010") > 0) { // DICOM
				...
				if (voxelDepth > 0.0) {
					if (IJ.debugMode)
						IJLog.log("DICOM voxel depth set to " + voxelDepth + " (" + cal.pixelDepth + ")");
	...
	/**
	 * Removes names that start with "." or end with ".db", ".txt", ".lut", "roi",
	 * ".pty", ".hdr", ".py", etc.
	 */
	...
	/*
	 * Returns true if 'name' ends with ".txt", ".lut", ".roi", ".pty", ".hdr",
