...
public class Macro_Runner implements PlugIn {
	...
	/**
	 * Opens and runs the specified macro file (.txt or .ijm) or script file (.js,
	 * .bsh or .py)
	...
	public void run(String name) {
		if (IJ.debugMode)
			IJLog.log("Macro_Runner.run(): " + name);
	 ...
	 * in the plugins folder, in or out of a JAR file, so name conflicts are
	 * possible.
	...
	private static String runScript(Object plugin, String script, String arg) {
		if (plugin instanceof PlugInInterpreter) {
			...
			if (IJ.debugMode)
				IJLog.log("Running " + interp.getName() + " script; arg=\"" + arg + "\"");
