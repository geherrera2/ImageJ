...
public class ThresholdAdjuster extends PlugInDialog implements PlugIn, Measurements, Runnable,
		ActionListener, AdjustmentListener, ItemListener, FocusListener, KeyListener, MouseWheelListener,
		ImageListener {
	...
	int maxValue = -1;
	int sliderRange = 256;
	boolean doAutoAdjust, doReset, doApplyLut, doStateChange, doSet, doBackground; // actions required from user
																					// interface
	...
	Checkbox darkBackground, stackHistogram, rawValues, noResetCheckbox;
	 ...
	 * Auto-thresholding is performed if there is currently no threshold and
	 * 'enableAutoThreshold' is true.
	 * Returns the ImageProcessor of the image that should be used, or null if no
	 * appropriate image.
	 */
	ImageProcessor setup(ImagePlus imp, boolean enableAutoThreshold) {
		if (IJ.debugMode)
			IJLog.log("ThresholdAdjuster.setup: enableAuto=" + enableAutoThreshold);
		if (imp == null)
			return null;
		ImageProcessor ip;
	...
	void updatePercentiles(ImagePlus imp, ImageProcessor ip) {
		...
		if (stats != null && stats.histogram != null && stats.histogram.length == 256
			...
			// IJLog.log("<"+minThresholdInt+":"+below+" in:"+inside+";
	 ...
	 */
	String d2s(double x) {
		return Math.abs(x) >= 1e6 ? IJ.d2s(x, -2) : ResultsTable.d2s(x, 2); // the latter uses exp notation also for
																			// small x
	}
	...
	void reset(ImagePlus imp, ImageProcessor ip) {
		// IJLog.log("reset1: "+noReset+" "+noResetChanged+" "+mode);
		if (noResetChanged) {
	...
	public void run() {
		while (!done) {
			synchronized (this) {
				if (!doAutoAdjust && !doReset && !doApplyLut && !doStateChange && !doSet && !doBackground
...
class ThresholdPlot extends Canvas implements Measurements, MouseListener {
	...
	ImageStatistics setHistogram(ImagePlus imp, boolean entireStack, boolean rawValues) {
		if (IJ.debugMode)
			IJLog.log("ThresholdAdjuster:setHistogram: " + entireStack + " " + entireStack2);
