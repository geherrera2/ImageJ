...
public class ColorThresholder extends PlugInFrame implements PlugIn, Measurements,
	...
	public synchronized void adjustmentValueChanged(AdjustmentEvent e) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.adjustmentValueChanged ");
		if (!checkImage())
			return;
	...
	public synchronized void itemStateChanged(ItemEvent e) {
		if (IJ.debugMode)
			IJLog.log("ColorThresolder.itemStateChanged");
		ImagePlus imp = WindowManager.getCurrentImage();
		if (imp == null)
			return;
	...
	public void focusGained(FocusEvent e) {
		if (IJ.debugMode)
			IJLog.log("ColorThresolder.focusGained");
		checkImage();
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.actionPerformed");
	...
	private boolean checkImage() {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.checkImage");
		ImagePlus imp = WindowManager.getCurrentImage();
		if (imp == null || imp.getBitDepth() != 24) {
			IJ.beep();
	...
	boolean setup(ImagePlus imp) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.setup");
		ImageProcessor ip;
	...
	void autoSetThreshold() {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.autoSetThreshold");
	...
	void apply(ImagePlus imp) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.apply");
	...
	void reset(ImagePlus imp) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.reset");
		ImageProcessor ip = imp.getProcessor();
	...
	public void windowActivated(WindowEvent e) {
		if (IJ.debugMode)
			IJLog.log("ColorThresholder.windowActivated ");
		ImagePlus imp = WindowManager.getCurrentImage();
		if (imp == null || imp.getBitDepth() != 24) {
			IJ.beep();
			IJ.showStatus("No RGB image");
