...
public class ContrastAdjuster extends PlugInDialog implements Runnable,
	...
	public void run(String arg) {
		...
		if (!windowLevel) {
			...
			if (IJ.debugMode)
				minLabel.setBackground(Color.yellow);
		...
		if (windowLevel) {
			...
			if (realValue) {
				...
				if (amin > 9999999.0 * s || amax > 9999999.0 * s)
					digits = -2;
				if ((amin > 0 && amin < 0.001) || (amax > 0 && amax < 0.001))
					digits = -2;
			}
	...
	void adjustMax(ImagePlus imp, ImageProcessor ip, double maxvalue) {
		...
		// IJLog.log("adjustMax: "+maxvalue+" "+max);
		if (min < defaultMin)
			min = defaultMin;
		if (max < min)
			min = max;
	...
	void apply(ImagePlus imp, ImageProcessor ip) {
		...
		if (IJ.debugMode)
			IJLog.log("Apply: mapping " + min + "-" + max + " to 0-" + (range - 1));
	...
	private void applyRGBStack(ImagePlus imp) {
		...
		double max = imp.getDisplayRangeMax();
		if (IJ.debugMode)
			IJLog.log("applyRGBStack: " + min + "-" + max);
		int current = imp.getCurrentSlice();
	...
	void setMinAndMax(ImagePlus imp, ImageProcessor ip) {
		...
		if (imp.isComposite() && channels > 1) {
			...
			label = channels == 2 ? label + "channel of this image"
					: label + (channels - 1) + " channels of this image";
		...
		if (maxValue >= minValue) {
			...
			if (allChannels) {
				...
				for (int c = 1; c <= channels; c++) {
					...
					imp.setDisplayRange(min, max);
					// IJLog.log("setDisplayRange: "+c+" "+min+" "+max);
				}
	...
	private void propagate(ImagePlus img) {
		if (img.getBitDepth() == 24) {
			...
			gd.addMessage(
					"Propagation of RGB images not supported. As a work-around,\nconvert images to multi-channel composite color.");
			gd.hideCancelButton();
			gd.showDialog();
			return;
		}
	...
	void setWindowLevel(ImagePlus imp, ImageProcessor ip) {
		...
		double minValue = cal.getCValue(min);
		double maxValue = cal.getCValue(max);
		// IJLog.log("setWindowLevel: "+min+" "+max);
	...
	public static void recordSetMinAndMax(double min, double max) {
		if ((int) min == min && (int) max == max) {
			...
			if (Recorder.scriptMode()) {
				Recorder.recordCall(
						"imp.setDisplayRange(" + ResultsTable.d2s(min, 2) + ", " + ResultsTable.d2s(max, 2) + ");");
				Recorder.recordCall("imp.updateAndDraw();");
			} else
				Recorder.recordString(
						"setMinAndMax(" + ResultsTable.d2s(min, 2) + ", " + ResultsTable.d2s(max, 2) + ");");
		}
	}
	...
	public void windowActivated(WindowEvent e) {
		...
		if (owin == null || !(owin instanceof ImageWindow))
			return;
		if (IJ.debugMode)
			IJLog.log("windowActivated: " + owin);
		if (IJ.isMacro()) {
