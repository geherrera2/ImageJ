...
class DicomDecoder {
	...
	public DicomDecoder(String directory, String fileName) {
		...
		if (dictionary == null && IJ.getApplet() == null) {
			...
			if (f.exists())
				try {
					...
					is.close();
					if (IJ.debugMode)
						IJLog.log("DicomDecoder: using " + dictionary.size() + " tag dictionary at " + path);
				} catch (Exception e) {
					dictionary = null;
				}
		}
		if (dictionary == null) {
			...
			if (IJ.debugMode)
				IJLog.log("DicomDecoder: " + path + " not found; using " + dictionary.size()
						+ " tag built in dictionary");
		}
	}
	...
	int getNextTag() throws IOException {
		...
		// IJLog.log("getNextTag: "+tag+" "+elementLength);
		return tag;
	}
	...
	FileInfo getFileInfo() throws IOException {
		...
		if (IJ.debugMode) {
			IJLog.log("");
			IJLog.log("DicomDecoder: decoding " + fileName);
		}
		...
		if (!getString(4).equals(DICM)) {
			...
			location = 0;
			if (IJ.debugMode)
				IJLog.log(DICM + " not found at offset " + ID_OFFSET + "; reseting to offset 0");
		} else {
			dicmFound = true;
			if (IJ.debugMode)
				IJLog.log(DICM + " found at offset " + ID_OFFSET);
		}
		...
		if (IJ.debugMode) {
			IJLog.log("width: " + fi.width);
			IJLog.log("height: " + fi.height);
			IJLog.log("images: " + fi.nImages);
			IJLog.log("bits allocated: " + bitsAllocated);
			IJLog.log("offset: " + fi.offset);
		}
	...
	void addInfo(int tag, String value) throws IOException {
		...
		if (info != null && tag != ITEM) {
			...
			// (previousInfo!=null&&previousInfo.indexOf("Sequence:")==-1))
			// dicomInfo.append("\n");
			previousGroup = group;
			previousInfo = info;
