...
public class Duplicator implements PlugIn, TextListener, ItemListener {
	...
	private void straightenRotatedRect(ImagePlus impA, Roi roiA, ImagePlus impB) {
		...
		ImageStack stackNew = (new CanvasResizer()).expandStack(stackOld, (int) rrDia, (int) rrDia, (int) xOff,
				(int) yOff);
	...
	/**
	 * Returns a copy of the image, stack or hyperstack contained in the specified
	 * ImagePlus.
	 * 
	...
	/**
	 * Returns a copy the current image or stack slice, cropped if there is a
	 * selection.
	 * 
	...
	/**
	 * Returns a new hyperstack containing a possibly reduced version of the input
	 * image.
	 */
	public ImagePlus run(ImagePlus imp, int firstC, int lastC, int firstZ, int lastZ, int firstT, int lastT) {
		...
		if (overlay != null && !imp.getHideOverlay()) {
			...
			if (!(firstC == 1 && lastC == nChannels && firstZ == 1 && lastZ == nSlices && firstT == 1
					&& lastT == nFrames))
				overlay2.crop(firstC, lastC, firstZ, lastZ, firstT, lastT);
			imp2.setOverlay(overlay2);
		}
		if (Recorder.record)
			Recorder.recordCall("imp = new Duplicator().run(imp, " + firstC + ", " + lastC + ", " + firstZ + ", "
					+ lastZ + ", " + firstT + ", " + lastT + ");");
		return imp2;
	}
	...
	String showDialog(ImagePlus imp, String dialogTitle, String prompt) {
		...
		if (!isMacro) {
			...
			if (!ignoreNextSelection)
				staticIgnoreSelection = ignoreSelection;
		}
	...
	private String getNewTitle() {
		...
		if (imp.getStackSize() > 1 && !duplicateStack && !legacyMacro
				&& (stackCheckbox == null || !stackCheckbox.getState())) {
			ImageStack stack = imp.getStack();
	...
	void duplicateHyperstack(ImagePlus imp, String newTitle) {
		...
		if (!duplicateStack) {
			...
			if (!singleComposite && nChannels > 1 && imp.isComposite()
					&& ((CompositeImage) imp).getMode() == IJ.COMPOSITE) {
				firstC = 1;
				lastC = nChannels;
			} else
	...
	public void textValueChanged(TextEvent e) {
		if (IJ.debugMode)
			IJLog.log("Duplicator.textValueChanged: " + e);
