 ...
 * When finding maxima (not minima), pixels with a level below the lower
 * threshold
 * can be left unprocessed.
 *
 * Except for segmentation, this plugin works with area ROIs, including
 * non-rectangular ROIs,
 * which define the area where maxima are reported.
 * Since this plug-in creates a separate output image it processes only single
 * images or slices, no stacks.
 *
 * Notes:
 * - When using one instance of MaximumFinder for more than one image in
 * parallel threads,
 * all must images have the same width and height.
 *
 * version 09-Nov-2006 Michael Schmid
 * version 21-Nov-2006 Wayne Rasband. Adds "Display Point Selection" option and
 * "Count" output type.
 * version 28-May-2007 Michael Schmid. Preview added, bugfix: minima of
 * calibrated images, uses Arrays.sort
 * version 07-Aug-2007 Fixed a bug that could delete particles when doing
 * watershed segmentation of an EDM.
 * version 21-Apr-2007 Adapted for float instead of 16-bit EDM; correct progress
 * bar on multiple calls
 * version 05-May-2009 Works for images>32768 pixels in width or height
 * version 01-Nov-2009 Bugfix: extra lines in segmented output eliminated;
 * watershed is also faster now
 * Maximum points encoded in long array for sorting instead of separete objects
 * that need gc
 * New output type 'List'
 * version 22-May-2011 Bugfix: Maximum search in EDM and float images with large
 * dynamic range could omit maxima
 * version 13-Sep-2013 added the findMaxima() and findMinima() functions for
 * arrays (Norbert Vischer)
 * version 20-Mar-2014 Watershed segmentation of EDM with tolerance>=1.0 does
 * not kill fine particles
 * version 11-Mar-2019 adds "strict" option, "noise tolerance" renamed to
 * "prominence"
 */
...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
    // filter params
    /**
     * prominence: maximum height difference between points that are not counted as
     * separate maxima
     */
    private static double tolerance = 10;
    /**
     * strict=off allows one maximum even if it is not higher than the prominence
     * above all other pixels
     */
    ...
    /** output type names */
    final static String[] outputTypeNames = new String[] { "Single Points", "Maxima Within Tolerance",
            "Segmented Particles", "Point Selection", "List", "Count" };
    ...
    private boolean thresholdWarningShown = false; // whether the warning "can't find minima with thresholding" has been
                                                   // shown
    ...
    private int nPasses = 0; // for progress bar, how many images to process (sequentially or parallel
                             // threads)
    ...
    /**
     * directions to 8 neighboring pixels, clockwise: 0=North (-y), 1=NE, 2=East
     * (+x), ... 7=NW
     */
    ...
    /**
     * Set his to the number of images to process (for the watershed progress bar
     * only).
    ...
    public void run(ImageProcessor ip) {
        ...
        outIp = findMaxima(ip, tolerance, strictMode, threshold, outputType, excludeOnEdges, false); // process the
                                                                                                     // image
        if (outIp == null)
     ...
     * @param tolerance      Height tolerance: maxima are accepted only if
     *                       protruding more than this value
     *                       from the ridge to a higher maximum
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines whether
     *                       strict mode is on, i.e.,
     *                       whether the global maximum is accepted even if all
     *                       other pixel are less than 'tolerance'
     *                       below this level (In 1.52m and before, 'strict' and
     *                       'excludeOnEdges' were the same).
     * @return A Polygon containing the coordinates of the maxima, where
     *         poly.npoints
     ...
     *                       protruding more than this value
     *                       from the ridge to a higher maximum
     * @param strict         When off, the global maximum is accepted even if all
     *                       other pixel are less than
     *                       'tolerance' below this level. With
     *                       <code>excludeOnEdges=true</code>, 'strict' also
     *                       means that the surounding of a maximum within
     *                       'tolerance' must not include an edge pixel
     *                       (otherwise, it is enough that there is no edge pixel
     ...
     *                  edgeMode = 0 (include edges) peak may be separated by one
     *                  qualified valley and by a border.
     *                  edgeMode = 1 (exclude edges) peak must be separated by two
     *                  qualified valleys
     ...
     *                       protruding more than this value
     *                       from the ridge to a higher maximum
     * @param outputType     What to mark in output image: SINGLE_POINTS,
     *                       IN_TOLERANCE or SEGMENTED.
     *                       No output image is created for output types
     *                       POINT_SELECTION, LIST and COUNT.
     * @param excludeOnEdges Whether to exclude edge maxima. Also determines whether
     *                       strict mode is on, i.e.,
     *                       whether the global maximum is accepted even if all
     *                       other pixel are less than 'tolerance'
     *                       below this level (In 1.52m and before, 'strict' and
     *                       'excludeOnEdges' were the same).
     * @return A new byteProcessor with a normal (uninverted) LUT where the marked
     *         points
     *         are set to 255 (Background 0). Pixels outside of the roi of the input
     *         ip are not set.
     ...
     *
     * LIMITATIONS: With outputType=SEGMENTED (watershed segmentation), some
     * segmentation lines
     ...
     *                       protruding more than this value
     *                       from the ridge to a higher maximum
     * @param threshold      minimum height of a maximum (uncalibrated); for no
     *                       minimum height set it to
    ...
    /**
     * Here the processing is done: Find the maxima of an image (does not find
     * minima).
     *
     * LIMITATIONS: With outputType=SEGMENTED (watershed segmentation), some
     * segmentation lines
    ...
    public ByteProcessor findMaxima(ImageProcessor ip, double tolerance, boolean strict, double threshold,
        ...
        // for segmentation, exclusion of edge maxima cannot be done now but has to be
        ...
        analyzeAndMarkMaxima(ip, typeP, maxPoints, excludeEdgesNow, isEDM, globalMin, tolerance, strict, outputType,
                maxSortingError);
        ...
        if (outputType == SEGMENTED) {
            // Segmentation required, convert to 8bit (also for 8-bit images, since the
            // calibration
    ...
    /**
     * Find all local maxima (irrespective whether they finally qualify as maxima or
     * not)
     * 
     * @param ip              The image to be analyzed
     * @param typeP           A byte image, same size as ip, where the maximum
     *                        points are marked as MAXIMUM
     *                        (do not use it as output: for rois, the points are
     *                        shifted w.r.t. the input image)
     ...
     * @param threshold       The threshold (calibrated) below which no pixels are
     *                        processed. Ignored if ImageProcessor.NO_THRESHOLD
     * @return Maxima sorted by value. In each array element (long, i.e., 64-bit
     *         integer), the value
     *         is encoded in the upper 32 bits and the pixel offset in the lower 32
     *         bit
     *         Note: Do not use the positions of the points marked as MAXIMUM in
     *         typeP, they are invalid for images with a roi.
     */
    long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow,
        ...
        for (int y = roi.y; y < roi.y + roi.height; y++) { // find local maxima now
            if (y % 50 == 0 && thread.isInterrupted())
                return null;
            for (int x = roi.x, i = x + y * width; x < roi.x + roi.width; x++, i++) { // for better performance with
                                                                                      // rois, restrict search to roi
                float v = ip.getPixelValue(x, y);
                ...
                for (int d = 0; d < 8; d++) { // compare with the 8 neighbor pixels
                    if (isInner || isWithin(x, y, d)) {
                        ...
                        float vNeighborTrue = isEDM ? trueEdmHeight(x + DIR_X_OFFSET[d], y + DIR_Y_OFFSET[d], ip)
                                : vNeighbor;
        ...
        if (thread.isInterrupted())
            return null;
        // long t1 = System.currentTimeMillis();IJLog.log("markMax:"+(t1-t0));
        ...
        // long t2 = System.currentTimeMillis();IJLog.log("makeArray:"+(t2-t1));
        if (thread.isInterrupted())
            return null;
        Arrays.sort(maxPoints); // sort the maxima by value
        // long t3 = System.currentTimeMillis();IJLog.log("sort:"+(t3-t2));
        return maxPoints;
    } // getSortedMaxPoints
     ...
     * 
     * @param ip              the image to be analyzed
     * @param typeP           8-bit image, here the point types are marked by type:
     *                        MAX_POINT, etc.
     * @param maxPoints       input: a list of all local maxima, sorted by height.
     *                        Lower 32 bits are pixel offset
     ...
     * @param maxSortingError sorting may be inaccurate, sequence may be reversed
     *                        for maxima having values
     *                        not deviating from each other by more than this (this
     *                        could be a result of
     *                        precision loss when sorting ints instead of floats, or
     *                        because sorting does not
     *                        take the height correction in 'trueEdmHeight' into
     *                        account
     * @param outputType
     */
    void analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints, boolean excludeEdgesNow,
        ...
        for (int iMax = nMax - 1; iMax >= 0; iMax--) { // process all maxima now, starting from the highest
            ...
            if ((types[offset0] & PROCESSED) != 0) // this maximum has been reached from another one, skip it
                continue;
            // we create a list of connected points and start the list at the current
            // maximum
                    ...
                    for (int d = 0; d < 8; d++) { // analyze all neighbors (in 8 directions) at the same level
                        ...
                        if ((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0) {
                            ...
                            if ((types[offset2] & PROCESSED) != 0) {
                                maxPossible = false; // we have reached a point processed previously, thus it is no
                                                     // maximum now
                                break;
                            }
    ...
    /**
     * Create an 8-bit image by scaling the pixel values of ip to 1-254 (<lower
     * threshold 0) and mark maximum areas as 255.
     ...
     * @param threshold Pixels of ip below this value (calibrated) are considered
     *                  background. Ignored if ImageProcessor.NO_THRESHOLD
     * @return The 8-bit output image.
     */
    ByteProcessor make8bit(ImageProcessor ip, ByteProcessor typeP, boolean isEDM, float globalMin, float globalMax,
            double threshold) {
        byte[] types = (byte[]) typeP.getPixels();
        double minValue;
        if (isEDM) {
        ...
        double offset = minValue - (globalMax - minValue) * (1. / 253 / 2 - 1e-6); // everything above minValue should
                                                                                   // become >(byte)0
     ...
     * Distance Map.
     * This is needed since the point sampled is not necessarily at the highest
     * position.
     * For simplicity, we don't care about the Sqrt(5) distance here although this
    ...
    /**
     * eliminate unmarked maxima for use by watershed. Starting from each previous
     * maximum,
     * explore the surrounding down to successively lower levels until a marked
     * maximum is
     * touched (or the plateau of a previously eliminated maximum leads to a marked
     * maximum).
     ...
     * @param maxPoints array containing the coordinates of all maxima that might be
     *                  relevant
     */
                    ...
                    for (int d = 0; d < 8; d++) { // analyze all neighbors (in 8 directions) at the same level
                        ...
                        if ((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0) {
                            if ((types[offset2] & MAX_AREA) != 0
                                    || (((types[offset2] & ELIMINATED) != 0) && (pixels[offset2] & 255) >= loLevel)) {
     ...
     * Needed for post-processing watershed-segmented images that can have local
     * minima
     * 
     * @param ip 8-bit image with background = 0, lines between 1 and 254 and
     *           segmented particles = 255
     */
    ...
    void removeLineFrom(byte[] pixels, int x, int y) {
        // IJLog.log("del line from "+x+","+y);
        pixels[x + width * y] = (byte) 255; // delete the first point
        boolean continues;
        do {
            continues = false;
            boolean isInner = (y != 0 && y != height - 1) && (x != 0 && x != width - 1); // not necessary, but faster
                                                                                         // than isWithin
        ...
        // IJLog.log("deleted to "+x+","+y);
    } // void removeLineFrom

    /**
     * Analyze the neighbors of a pixel (x, y) in a byte image; pixels <255
     * ("non-white") are
     ...
     * @return Number of 4-connected lines emanating from this point. Zero if the
     *         point is
    ...
    int nRadii(byte[] pixels, int x, int y) {
        ...
        boolean isInner = (y != 0 && y != height - 1) && (x != 0 && x != width - 1); // not necessary, but faster than
                                                                                     // isWithin
     ...
     *              pixels with bit MAX_AREA at the edge are considered indicators
     *              of an edge maximum.
    ...
    void deleteParticle(int x, int y, ByteProcessor ip, Wand wand) {
        ...
        if (wand.npoints == 0) {
            IJLog.log("wand error selecting edge particle at x, y = " + x + ", " + y);
            return;
        }
     ...
     * set to 255 and the background set to 0. The image should not have any local
     * maxima
     * other than the marked ones. Local minima will lead to artifacts that can be
     * removed
     * later. On output, all particles will be set to 255, segmentation lines remain
     * at their
     * old value.
     * 
     * @param ip The byteProcessor containing the image, with size given by the
     *           class variables width and height
     * @return false if canceled by the user (note: can be cancelled only if called
     *         by "run" with a known ImagePlus)
     */
        ...
        for (int level = highestValue; level >= 1; level--) {
            ...
            while (remaining > 0 && idle < 8) {
                ...
                do { // expand each level in 8 directions
                    ...
                    // IJLog.log("level="+level+" direction="+directionSequence[dIndex%8]+"
                    // remain="+remaining+"-"+n);
            ...
            if (remaining > 0 && level > 1) { // any pixels that we have not reached?
                ...
                if (nextLevel > 0) {
                    ...
                    // IJLog.log("level="+level+": add "+(newNextLevelEnd-levelStart[nextLevel+1])+"
                    // points to "+nextLevel);
    ...
    /**
     * dilate the UEP on one level by one pixel in the direction specified by step,
     * i.e., set pixels to 255
     * 
     * @param pass            gives direction of dilation, see makeFateTable
     * @param ip              the EDM with the segmeted blobs successively getting
     *                        set to 255
     ...
     * @param pixelPointers[] list of pixel coordinates (x+y*width) sorted by level
     *                        (in sequence of y, x within each level)
     * @param xCoordinates    list of x Coorinates for the current level only (no
     *                        offset levelStart)
    ...
    private int processLevel(int pass, ImageProcessor ip, int[] fateTable,
        ...
        for (int i = 0, p = levelStart; i < levelNPoints; i++, p++) {
          ...
          // IJLog.log("pass="+pass+", changed="+nChanged+" unchanged="+nUnchanged);
     ...
     * pass gives the direction of rotation, with 0 = to top left (x--,y--), 1 to
     * top,
    ...
    private int[] makeFateTable() {
        ...
        for (int item = 0; item < 256; item++) { // dissect into pixels
            ...
            for (int i = 0, mask = 1; i < 8; i++) { // we dilate in the direction opposite to the direction of the
                                                    // existing neighbors
                if (isSet[(i + 4) % 8])
                    table[item] |= mask;
                mask *= 2;
            }
            for (int i = 0; i < 8; i += 2) // if side pixels are set, for counting transitions it is as good as if the
                                           // adjacent edges were also set
                if (isSet[i]) {
                    isSet[(i + 1) % 8] = true;
                    isSet[(i + 7) % 8] = true;
                }
            ...
            if (transitions >= 4) { // if neighbors contain more than one region, dilation ito this pixel is
                                    // forbidden
                table[item] = 0;
            } else {
            }
    ...
    /**
     * create an array of offsets within a pixel array for directions in clockwise
     * order:
     * 0=(x,y-1), 1=(x+1,y-1), ... 7=(x-1,y)
     * Also creates further class variables:
     * width, height, and the following three values needed for storing coordinates
     * in single ints for watershed:
     * intEncodeXMask, intEncodeYMask and intEncodeShift.
     * E.g., for width between 129 and 256, xMask=0xff and yMask = 0xffffff00 are
     * bitwise masks
     * for x and y, respectively, and shift=8 is the bit shift to get y from the
     * y-masked value
     * Returns as class variables: the arrays of the offsets to the 8 neighboring
     * pixels
    ...
    void makeDirectionOffsets(ImageProcessor ip) {
        ...
        // IJLog.log("masks
        // (hex):"+Integer.toHexString(xMask)+","+Integer.toHexString(xMask)+";
        // shift="+shift);
