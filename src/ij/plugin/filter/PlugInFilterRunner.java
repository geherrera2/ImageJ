...
public class PlugInFilterRunner implements Runnable, DialogListener {
	...
	private Object snapshotPixels; // the snapshot to show we have one and for undo in case of parallel actions
									// intervening
	 ...
	 * @param arg       The argument specified for this PlugInFilter in IJ_Props.txt
	 *                  or in the
	 *                  plugins.config file of a .jar archive conatining a
	 *                  collection of plugins. <code>arg</code>
	...
	public PlugInFilterRunner(Object theFilter, String command, String arg) {
		...
		if (theFilter instanceof ExtendedPlugInFilter) { // calling showDialog required?
			try {
				flags = ((ExtendedPlugInFilter) theFilter).showDialog(imp, command, this); // D I A L O G (may include
																							// preview)
			} catch (Exception e) {
				killPreview();
		...
		// IJLog.log("processedAsPreview="+processedAsPreview+"; slices="+slices+";
		// doesStacks="+((flags&PlugInFilter.DOES_STACKS)!=0));
		...
		if (doStack || processedAsPreview == 0) { // if processing during preview was not enough
			...
			if (!doStack) { // single image
				...
				if ((flags & PlugInFilter.NO_CHANGES) == 0) { // (filters doing no modifications don't change undo
																// status)
					if (snapshotPixels != null) {
				...
				if ((flags & PlugInFilter.PARALLELIZE_STACKS) != 0) {
					...
					if (threads > slicesToDo)
						threads = slicesToDo;
					if (threads > 1)
						slicesForThread = new Hashtable<Thread, int[]>(threads - 1);
				}
				int startSlice = 1;
				for (int i = 1; i < threads; i++) { // setup the background threads
					...
					bgThread.start();
					// IJLog.log("Stack: Thread for slices "+startSlice+"-"+endSlice+" started");
					startSlice = endSlice + 1;
				}
				// IJLog.log("Stack: Slices "+startSlice+"-"+slices+" by main thread");
	 ...
	 * @param fp             A Floatprocessor as a target for conversion to Float.
	 *                       May be null.
	 * @param snapshotPixels Valid snapshotPixels for the current ImageProcessor
	 *                       (null when processing a stack)
	 *                       Class variables used: flags (input), snapshotPixels
	 *                       (set if a snapshot of ip
	...
	private void processOneImage(ImageProcessor ip, FloatProcessor fp, Object snapshotPixels) {
		...
		if (convertToFloat) {
			for (int i = 0; i < ip.getNChannels(); i++) {
				...
				if (thread.isInterrupted())
					return;
				// IJLog.log("slice="+getSliceNumber()+" pass="+pass+"/"+nPasses);
				pass++;
	...
	private void processImageUsingThreads(ImageProcessor ip, FloatProcessor fp, Object snapshotPixels) {
		if (IJ.debugMode)
			IJLog.log("using threads: " + ip.getNChannels());
		Thread thread = Thread.currentThread();
		boolean convertToFloat = (flags & PlugInFilter.CONVERT_TO_FLOAT) != 0 && !(ip instanceof FloatProcessor);
		boolean doMasking = (flags & PlugInFilter.SUPPORTS_MASKING) != 0 && ip.getMask() != null;
		if (snapshotPixels == null && (doMasking || ((flags & PlugInFilter.SNAPSHOT) != 0) && !convertToFloat)) {
	...
	private void processChannelUsingThreads(ImageProcessor ip) {
		...
		if (IJ.debugMode)
			IJLog.log("processing channel: " + threads);
		if (threads > roi.height)
			threads = roi.height;
		if (threads > 1)
			roisForThread = new Hashtable<Thread, ImageProcessor>(threads - 1);
		int y1 = roi.y;
		for (int i = 1; i < threads; i++) {
			...
			bgThread.start();
			if (IJ.debugMode)
				IJLog.log("  starting thread: " + y1 + "-" + y2);
			y1 = y2 + 1;
		}
		if (IJ.debugMode)
			IJLog.log("  main thread " + y1 + "-" + (roi.y + roi.height));
	...
	/**
	 * interrupt threads processing the rois of an image and wait till they have
	 * finished
	 */
	void interruptRoiThreads(Hashtable<Thread, ImageProcessor> roisForThread) {
		if (roisForThread == null)
			return; // class variable may become null in other thread
		for (Enumeration<Thread> en = roisForThread.keys(); en.hasMoreElements();)
	...
	private void runPreview() {
		if (IJ.debugMode)
			IJLog.log("preview thread started; imp=" + imp.getTitle());
		Thread thread = Thread.currentThread();
		ImageProcessor ip = imp.getProcessor();
		...
		while (bgPreviewOn) {
			...
			interruptable: {
				...
				if (thread.isInterrupted())
					break interruptable;
				// IJLog.log("process preview start now");
				...
				imp.updateAndDraw();
				if (IJ.debugMode)
					IJLog.log("preview processing done");
			}
	...
	/**
	 * stop the background process responsible for preview and wait until the
	 * preview thread has finished
	 */
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (previewCheckboxOn && previewThread == null) {
			...
			previewThread.start();
			if (IJ.debugMode)
				IJLog.log(command + " Preview thread was started");
			return true;
		}
