 ...
 * | | | strh STREAM HEADER (Required after above; fourcc type is 'vids' for
 * video stream)
...
public class AVI_Writer implements PlugInFilter {
    ...
    // Note: byte sequence in four-cc is reversed - ints in Intel (little endian)
    // byte order.
    // Note that compression codes BI_JPEG=4 and BI_PNG=5 are not understood by avi
    // players
    // (even not by MediaPlayer, even though these codes are specified by
    // Microsoft).
    ...
    private final static int JUNK_SIZE_THRESHOLD = 950 * 1024 * 1024; // if size exceeds this, makes a new RIFF AVIX
                                                                      // chunk
    ...
    public void run(ImageProcessor ip) {
        if (!showDialog(imp))
            return; // compression type dialog
    ...
    public void writeImage(ImagePlus imp, String path, int compression, int jpegQuality)
        ...
        writeInt(0); // dwPaddingGranularity (for header length?), previously dwReserved1, usually
                     // set to zero.
        ...
        writeInt(biSizeImage); // biSizeImage (buffer size for decompressed mage) may be 0 for uncompressed
                               // data
        ...
        writeInt(0);
        writeInt(0);
        writeInt(0); // dwReserved[3]
        pointer2indxNextEntry = raFile.getFilePointer();
        chunkEndWriteSize(); // 'indx' chunk finished (nesting level 3), will be modified by
                             // writeMainIndxEntry
        writeString("JUNK"); // write a JUNK chunk for padding (will be moved and shortened by
                             // writeMainIndxEntry)
        ...
        int currentFilePart = 0;// 0 is inside RIFF AVI (AVI 1.0 compatible), >0 is RIFF AVIX (data chunk of AVI
                                // 2.0)
        ...
        while (iFrame < zDim) {
            if (currentFilePart > 0) { // open new RIFF AVIX chunk
                ...
                // IJLog.log("AVIX starts at iFrame="+iFrame);
            }
            ...
            while (iFrame < zDim) {
                ...
                dataChunkLength[iFrame] = (int) (raFile.getFilePointer() - chunkPointer - 8); // size excludes '00db'
                                                                                              // and size fields
            ...
            if (writeAVI2index) {
                ...
                // IJLog.log("write ix00: frames "+firstFrameInChunk+"-"+(iFrame-1)+" offset
                // "+Long.toHexString(dataChunkOffset[firstFrameInChunk])+"-"+Long.toHexString(dataChunkOffset[iFrame-1]));
    ...
    private void chunkEndWriteSize() throws IOException {
        ...
        // IJLog.log("chunk at
        // 0x"+Long.toHexString(sizePointers[stackPointer]-4)+"-0x"+Long.toHexString(position));
    }
    ...
    private void writeCompressedFrame(ImageProcessor ip) throws IOException {
        // IJLog.log("BufferdImage Type="+bufferedImage.getType()); // 1=RGB, 13=indexed
    ...
    private void writeLong(long v) throws IOException {
        ...
        // IJLog.log("long: 0x"+Long.toHexString(v)+"="+v);
    }
    ...
    private void writeInt(int v) throws IOException {
        ...
        // IJLog.log("int: 0x"+Integer.toHexString(v)+"="+v);
    }
    ...
    /**
     * An output stream directed to a RandomAccessFile (starting at the current
     * position)
     */
    class RaOutputStream extends OutputStream {
        ...
        public void write(int b) throws IOException {
            // IJLog.log("stream: byte");
            raFile.writeByte(b); // just for completeness, usually not used by image encoders
        }

        public void write(byte[] b) throws IOException {
            // IJLog.log("stream: array len="+b.length);
            raFile.write(b);
        }

        public void write(byte[] b, int off, int len) throws IOException {
            // IJLog.log("stream: array="+b.length+" off="+off+" len="+len);
