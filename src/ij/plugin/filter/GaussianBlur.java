...
public class GaussianBlur implements ExtendedPlugInFilter, DialogListener {
     ...
     * PlugInFilterRunner of ImageJ, an ImagePlus is known and conversion of RGB
     * images
    ...
    /**
     * Gaussian Filtering of an ImageProcessor. This method is for compatibility
     * with the
     * previous code (before 1.38r) and uses a low-accuracy kernel, only slightly
     * better
     * than the previous ImageJ code.
     * The 'radius' in this call is different from the one used in ImageJ 1.38r and
     * later.
     * Therefore, use blurGaussian(ip, sigma, sigma, accuracy), where 'sigma' is
     * equivalent
     * to the 'sigma (radius)' of the Menu, and accuracy should be 0.02 unless
     * better
     * accuracy is desired.
     */
    @Deprecated
    ...
    /**
     * Blur an image in one direction (x or y) by a Gaussian, using multiple threads
     * on multiprocessor machines
     * 
     * @param ip         The Image with the original data where also the result will
     *                   be stored
    ...
    public void blur1Direction(final FloatProcessor ip, final double sigma, final double accuracy,
        ...
        if (lineFromA < 0)
            ...
            lineFrom = lineFromA;
        final int lineToA = (xDirection ? roi.y + roi.height : roi.x + roi.width) + extraLines; // the last line+1 to
                                                                                                // process
        final int lineTo;
         ...
         * standard
         * deviation for this:
         * Downscaling gives std devation sigma = 1/sqrt(3); upscale gives sigma = 1/2
         * (in downscaled pixels).
        ...
        final double sigmaGauss = doDownscaling ? Math.sqrt(sigma * sigma / (reduceBy * reduceBy) - 1. / 3. - 1. / 4.)
                : sigma;
        final int maxLength = doDownscaling ? (length + reduceBy - 1) / reduceBy + 2 * (UPSCALE_K_RADIUS + 1) // downscaled
                                                                                                              // line
                                                                                                              // can't
                                                                                                              // be
                                                                                                              // longer
        ...
        final int readFrom = (writeFrom - kRadius < 0) ? 0 : writeFrom - kRadius; // not including broadening by
                                                                                  // downscale&upscale
        ...
        final int newLength = doDownscaling ? // line length for convolution
                ...
                : length;
        final int unscaled0 = readFrom - (UPSCALE_K_RADIUS + 1) * reduceBy; // input point corresponding to cache index
                                                                            // 0
        // the following is relevant for upscaling only
        // IJLog.log("reduce="+reduceBy+", newLength="+newLength+",
        // unscaled0="+unscaled0+", sigmaG="+(float)sigmaGauss+",
        // kRadius="+gaussKernel[0].length);
     ...
     * @param kernel    downscale kernel, runs form -1.5 to +1.5 in downscaled
     *                  coordinates
     * @param reduceBy  downscaling factor
     * @param pixel0    index in pixels array corresponding to start of line or
     *                  column
     * @param unscaled0 index in input line corresponding to output line index 0,
     *                  May be negative.
     * @param length    length of full input line or column
     * @param pointInc  spacing of values in input array (1 for lines, image width
     *                  for columns)
    ...
    /**
     * the above code is equivalent to the following one; but the above code is
     * faster
     ...
     */
    /*
     * final static private void downscaleLine(final float[] pixels, final float[]
     * cache, final float[] kernel,
     * final int reduceBy, final int pixel0, final int unscaled0, final int length,
     * final int pointInc, final int newLength) {
     ...
     * v += kernel[x+2*reduceBy] * pixels[pp<pixel0 ? pixel0 : (pp>pLast ? pLast :
     * pp)];
     * }
     * cache[xout] = v;
     * }
    ...
    final static private void upscaleLine(final float[] cache, final float[] pixels, final float[] kernel,
        ...
        for (int xout = writeFrom; xout < writeTo; xout++, p += pointInc) {
            final int xin = (xout - unscaled0 + reduceBy - 1) / reduceBy; // the corresponding point in the cache (if
                                                                          // exact) or the one above
     ...
     * possibly the pixels array of a FloatProcessor (as a row or column or part
     * thereof)
     *
     * @param input     Input array containing the line
     * @param pixels    Float array for output, can be the pixels of a
     *                  FloatProcessor
     ...
     *                  where n runs from the kernel periphery (last element) to 0
     *                  and
     *                  back. Normalization should include all kernel points, also
     *                  these
     ...
     *                  <code>writeTo+kernel.length</code> or
     ...
     * @param point0    Array index of first element of the 'line' in pixels (i.e.,
     *                  lineNumber * lineInc)
     * @param pointInc  Increment of the pixels array index to the next point (for
     *                  an ImageProcessor,
     *                  it should be <code>1</code> for a row, <code>width</code>
     *                  for a column)
     */
    final static private void convolveLine(final float[] input, final float[] pixels, final float[][] kernel,
        ...
        for (; i < iEndInside; i++, p += pointInc) { // while only pixels within the line are be addressed (the easy
                                                     // case)
            float result = input[i] * kern0;
            for (int k = 1; k < kRadius; k++)
    ...
    /**
     * Create a 1-dimensional normalized Gaussian kernel with standard deviation
     * sigma
     ...
     * replaced by a 2nd-order polynomial with its minimum=0 at the first
     * out-of-kernel
     * pixel. Thus, the kernel function has a smooth 1st derivative in spite of
     * finite
     * length.
     *
     * @param sigma     Standard deviation, i.e. radius of decay to 1/sqrt(e), in
     *                  pixels.
     * @param accuracy  Relative accuracy; for best results below 0.01 when
     *                  processing
     *                  8-bit images. For short or float images, values of 1e-3 to
     *                  1e-4
     ...
     *                  large sigma, should be set to image width or height. For
     *                  small
    ...
    public float[][] makeGaussianKernel(final double sigma, final double accuracy, int maxRadius) {
        ...
        for (int i = 0; i < kRadius; i++) {
            ...
            // IJLog.log("k["+i+"]="+(float)v+" sum="+(float)rsum);
        }
        return kernel;
    }
