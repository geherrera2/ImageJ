 ...
 */
// Version 2012-07-15 M. Schmid: Fixes a bug that could cause preview not to
// work correctly
// Version 2012-12-23 M. Schmid: Test for inverted LUT only once (not in each
// slice)
// Version 2014-10-10 M. Schmid: Fixes a bug that caused Threshold=0 when
// calling from API
// Version 2019-07-26 M. Schmid: Nonblocking dialog enabled
// Version 2020-07-17 M. Schmid: bugfix: changing preview parameters could lead
// to partial filtering with different parameters.
...
public class RankFilters implements ExtendedPlugInFilter, DialogListener {
	...
	// Atomic objects are used to avoid caching (i.e., to ensure that always the
	// current state of the variable is read).
	private AtomicInteger highestYinCache = new AtomicInteger(Integer.MIN_VALUE); // the highest line read into the
																					// cache so far
	private AtomicInteger nThreadsWaiting = new AtomicInteger(0); // number of threads waiting until they may read data
	private AtomicBoolean copyingToCache = new AtomicBoolean(false); // whether a thread is currently copying data to
																		// the cache
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
		...
		if ((flags & DOES_STACKS) != 0) {
			...
			if (filterType == MEAN || filterType == VARIANCE)
				workToDo *= 0.5;
	...
	/**
	 * Filters an image by any method except 'despecle', 'remove outliers', or
	 * top-hat
	 * 
	 * @param ip         The ImageProcessor that should be filtered (all 4 types
	 *                   supported)
	 * @param radius     Determines the kernel size, see Process>Filters>Show
	 *                   Circular Masks.
	 *                   Must not be negative. No checking is done for large values
	 *                   that would
	...
	/**
	 * Filters an image by any method except 'despecle' and top-hat (for
	 * 'despeckle', use 'median' and radius=1)
	 * 
	...
	/**
	 * Filters an image by any method except 'despecle' (for 'despeckle', use
	 * 'median' and radius=1)
	 * 
	 * @param ip              The image subject to filtering
	 * @param radius          The kernel radius
	 ...
	 * @param whichOutliers   BRIGHT_OUTLIERS or DARK_OUTLIERS for 'outliers' filter
	 * @param threshold       Threshold for 'outliers' filter
	 * @param lightBackground for top-hat background subtraction, background is
	 *                        light, not dark
	 * @param dontSubtract    fpr top-hat filter, performs a grayscale open or close
	 *                        instead of top-hat,
	 *                        where the result of grayscale open/close is subtracted
	 *                        from the original.
	 */
	public void rank(ImageProcessor ip, double radius, int filterType, int whichOutliers, float threshold,
			boolean lightBackground, boolean dontSubtract) {
	...
	// Implementation: each thread uses the same input buffer (cache), always works
	// on the next unfiltered line
	// Usually, one thread reads reads several lines into the cache, while the
	// others are processing the data.
	// 'nextY.get()' is set to a large negative number if the main thread has been
	// interrupted (during preview) or ESC pressed.
	// 'nextY' must not be a class variable because it is also used with one thread
	// (with stack parallelization)
	private void doFiltering(final ImageProcessor ip, final int[] lineRadii, final int filterType,
		...
		// 'cache' is the input buffer. Each line y in the image is mapped onto cache
		// line y%cacheHeight
		...
		final AtomicIntegerArray yForThread = new AtomicIntegerArray(numThreads); // threads announce here which line
	...
	//
	// Synchronization: unless a thread is waiting, we avoid the overhead of
	// 'synchronized'
	// statements. That's because a thread waiting for another one should be rare.
	//
	// Data handling: The area needed for processing a line is written into the
	// array 'cache'.
	// This is a stripe of sufficient width for all threads to have each thread
	// processing one
	// line, and some extra space if one thread is finished to start the next line.
	// This array is padded at the edges of the image so that a surrounding with
	// radius kRadius
	// for each pixel processed is within 'cache'. Out-of-image
	// pixels are set to the value of the nearest edge pixel. When adding a new
	// line, the lines in
	// 'cache' are not shifted but rather the smaller array with the start and end
	// pointers of the
	// kernel area is modified to point at the addresses for the next line.
	//
	// Algorithm: For mean and variance, except for very small radius, usually do
	// not calculate the
	// sum over all pixels. This sum is calculated for the first pixel of every line
	// only. For the
	// following pixels, add the new values and subtract those that are not in the
	// sum any more.
	// For min/max, also first look at the new values, use their maximum if larger
	// than the old
	// one. The look at the values not in the area any more; if it does not contain
	// the old
	// maximum, leave the maximum unchanged. Otherwise, determine the maximum inside
	// the area.
	// For outliers, calculate the median only if the pixel deviates by more than
	// the threshold
	// from any pixel in the area. Therfore min or max is calculated; this is a much
	// faster
	...
	private void doFiltering(ImageProcessor ip, int[] lineRadii, float[] cache, int cacheWidth, int cacheHeight,
		...
		if (nextY.get() < 0 || Thread.currentThread().isInterrupted())
			return;
		int width = ip.getWidth();
		...
		while (true) {
			...
			// IJLog.log("thread "+threadNumber+" @y="+y+"
			// needs"+(y-kHeight/2)+"-"+(y+kHeight/2)+"
			// highestYinC="+highestYinCache.get());
			boolean threadFinished = y >= roi.y + roi.height || y < 0; // y<0 if aborted
			if (numThreads > 1 && (nThreadsWaiting.get() > 0 || threadFinished)) // 'if' is not synchronized to avoid
																					// overhead
				// If nThreadsWaiting gets incremented to 1 after the 'if' by another thread,
				// there will be no notifyAll.
				// This is not an issue since the the other thread rechecking for a slow thread
				// (synchronized, ~30 lines below)
				// will not see the current thread being slow any more, so that other thread
				// will not wait.
				synchronized (this) {
					notifyAll(); // we may have blocked another thread
					// IJLog.log("thread "+threadNumber+" @y="+y+" notifying");
				}
			if (threadFinished)
					...
					synchronized (this) {
						...
						if (y - slowestThreadY + kHeight > cacheHeight) {
							do {
								...
								// IJLog.log("Thread "+threadNumber+" waiting @y="+y+"
								// slowest@y="+slowestThreadY);
								try {
									wait();
									if (nextY.get() < 0)
										return;
									...
									nextY.set(Integer.MIN_VALUE);
									notifyAll();
									Thread.currentThread().interrupt(); // keep interrupted status (PlugInFilterRunner
																		// needs it)
									return;
								}
			...
			if (numThreads == 1) { // R E A D
				...
				if (!copyingToCache.get() || highestYinCache.get() < y + kHeight / 2)
					synchronized (cache) {
						...
						while (highestYinCache.get() < arrayMinNonNegative(yForThread) - kHeight / 2 + cacheHeight
								- 1) {
			...
			// IJLog.log("thread "+threadNumber+" @y="+y+" line done");
		} // while (true); loop over y (lines)
	}

	// returns the minimum of the array, which may be modified concurrently, but not
	// less than 0
	...
	private void filterLine(float[] values, int width, float[] cache, int[] cachePointers, int kNPoints, int cacheLineP,
			Rectangle roi, int y,
			double[] sums, float[] medianBuf1, float[] medianBuf2, float minMaxOutliersSign, float maxValue,
			boolean isFloat, int filterType,
		...
		for (int x = 0; x < roi.width; x++, valuesP++) { // x is with respect to roi.x
			...
			if (sumFilter) {
				...
				if (v * minMaxOutliersSign + threshold < max) { // for low outliers: median can't be higher than max
																// (minMaxOutliersSign is +1)
					median = getMedian(cache, x, cachePointers, medianBuf1, medianBuf2, kNPoints, median);
				...
				if (Float.isNaN(values[valuesP]))
					values[valuesP] = getNaNAwareMedian(cache, x, cachePointers, medianBuf1, medianBuf2, kNPoints,
							median);
				else
	 ...
	 * If y==0, it also creates the data for y<0, as far as necessary, thus filling
	 * the cache with
	...
	/**
	 * Read a line into the cache (includes conversion to flaot). Pad with edge
	 * pixels in x if necessary
	 */
	 ...
	 * @param ignoreRight should be 0 for analyzing all data or 1 for leaving out
	 *                    the row at the right
	 * @param max         should be -Float.MAX_VALUE or the smallest value the
	 *                    maximum can be
	 */
	private static float getAreaMax(float[] cache, int xCache0, int[] kernel, int ignoreRight, float max, float sign) {
		for (int kk = 0; kk < kernel.length; kk++) { // y within the cache stripe (we have 2 kernel pointers per cache
														// line)
	...
	/**
	 * Get max (or -min if sign=-1) at the right border inside or left border
	 * outside the kernel area.
	...
	private static float getSideMax(float[] cache, int xCache0, int[] kernel, boolean isRight, float sign) {
		...
		if (!isRight)
			xCache0--;
		for (int kk = isRight ? 1 : 0; kk < kernel.length; kk += 2) { // y within the cache stripe (we have 2 kernel
	...
	/**
	 * Add all values and values squared at the right border inside minus at the
	 * left border outside the kernal area.
	 * Output is added or subtracted to/from array sums[0] += sum; sums[1] += sum of
	 * squares when at
	...
	/**
	 * Get median of values within kernel-sized neighborhood. Kernel size kNPoints
	 * should be odd.
	 */
	 ...
	 * NaN data values are ignored; the output is NaN only if there are only NaN
	 * values in the
	 ...
	 * 
	 * @param buf       The input array. Only values 0 ... bufLength are read.
	 *                  <code>buf</code> will be modified.
	 * @param bufLength Number of values in <code>buf</code> that should be read
	 * @param n         which value should be found; n=0 for the lowest,
	 *                  n=bufLength-1 for the highest
	 * @return the value
	 */
	...
	/**
	 * Reset region between inner rectangle 'roi' and outer rectangle 'roi1' to the
	 * snapshot
	 */
	private void resetRoiBoundary(ImageProcessor ip, Rectangle roi, Rectangle roi1) {
		...
		for (int y = roi.y, pL = roi1.x + y * width,
				pR = roi.x + roi.width + y * width; y < roi.y + roi.height; y++, pL += width, pR += width) {
			if (leftWidth > 0)
	...
	/**
	 * @deprecated
	 *             Not needed any more, use the rank(ip, ...) method, which creates
	 *             the kernel
	 */
	...
	/**
	 * Sets the display range after variance and top-hat (16-bit & floating-point
	 * images only)
	 */
	 ...
	 * 
	 * @param radius
	 *               Radius = 0.5 includes the 4 neighbors of the pixel in the
	 *               center,
	 *               radius = 1 corresponds to a 3x3 kernel size.
	 * @return the circular kernel
	 *         The output is an array that gives the length of each line of the
	 *         structuring element
	 ...
	 *         length-1: kernelRadius in x direction (kernel width is
	 *         2*kernelRadius+1)
	 *         Kernel height can be calculated as (array length - 1)/2 (odd number);
	 *         Kernel radius in y direction is kernel height/2 (truncating integer
	 *         division).
	 *         Note that kernel width and height are the same for the circular
	 *         kernels used here,
	 *         but treated separately for the case of future extensions with
	...
	protected int[] makeLineRadii(double radius) {
		...
		// for (int i=0; i<kHeight;i++)IJLog.log(i+": "+kernel[2*i]+"-"+kernel[2*i+1]);
		return kernel;
	}
