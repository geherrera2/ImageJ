 ...
 * "http://imagej.nih.gov/ij/plugins/fft-filter.html".
 * 2001/10/29: First Version (JW)
 * 2003/02/06: 1st bugfix (works in macros/plugins, works on stacks, overwrites
 * image(=>filter)) (JW)
 * 2003/07/03: integrated into ImageJ, added "Display Filter" option (WSR)
 * 2007/03/26: 2nd bugfix (Fixed incorrect calculation of filter from structure
 * sizes, which caused
 * the real structure sizes to be wrong by a factor of 0.75 to 1.5 depending on
 * the image size.)
 */
public class FFTFilter implements PlugInFilter, Measurements {
	...
	public ImageProcessor tileMirror(ImageProcessor ip, int width, int height, int x, int y) {
		if (IJ.debugMode)
			IJLog.log("FFT.tileMirror: " + width + "x" + height + " " + ip);
	 ...
	 */
	void filterLargeSmall(ImageProcessor ip, double filterLarge, double filterSmall, int stripesHorVert,
			double scaleStripes) {
