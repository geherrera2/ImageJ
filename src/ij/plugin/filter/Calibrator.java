...
public class Calibrator implements PlugInFilter, Measurements, ActionListener {
	...
	private int nFits = Calibration.EXP_RECOVERY + 1; // don't set to CurveFitter.fitList.length; Calibration can't cope
														// with it
	...
	public boolean showDialog(ImagePlus imp) {
		...
		if (function == Calibration.NONE)
			defaultChoice = NONE;
		else if (function < nFits && function == Calibration.STRAIGHT_LINE && p != null && p[0] == 255.0
				&& p[1] == -1.0)
		...
		// column contains known values (e.g., OD).");
	...
	public void calibrate(ImagePlus imp) {
		...
		if (function != Calibration.NONE && bitDepth != 8 && imp.getNChannels() == 1
				&& !(bitDepth == 16 && imp.getDefault16bitRange() > 0)) {
