...
public class Thresholder implements PlugIn, Measurements, ItemListener {
	...
	private void applyThreshold(ImagePlus imp, boolean oneSlice) {
		...
		if (IJ.debugMode)
			IJLog.log("Thresholder (apply): " + minThreshold + "-" + maxThreshold + " " + fcolor + " " + bcolor + " "
					+ fill1 + " " + fill2);
		int[] lut = new int[256];
		for (int i = 0; i < 256; i++) {
		...
		if (convertToMask && !oneSlice) {
			...
			if ((invertedLut && Prefs.blackBackground) || (!invertedLut && !Prefs.blackBackground)) {
				...
				if (!IJ.isMacro() && ThresholdAdjuster.isDarkBackground() && !invertedLut && !Prefs.blackBackground)
					IJLog.log("\"Black background\" not set in Process>Binary>Options; inverting LUT");
				if (IJ.debugMode)
					IJLog.log("Thresholder (inverting lut)");
			}
		}
		if (fill1 && fill2 && ((fcolor == 0 && bcolor == 255) || (fcolor == 255 && bcolor == 0))) {
			...
			if (IJ.debugMode)
				IJLog.log("Thresholder: " + fcolor + "-" + fcolor + " (" + (Prefs.blackBackground ? "black" : "white")
						+ " background)");
		}
	...
	private void applyShortOrFloatThreshold(ImagePlus imp) {
		...
		if (IJ.debugMode)
			IJLog.log("Thresholder16: 255-255 (" + (Prefs.blackBackground ? "black" : "white") + " background)");
		IJ.showStatus("");
		imp.unlock();
	}
	...
	void convertStackToBinary(ImagePlus imp) {
		...
		if (listThresholds)
			IJLog.log("Thresholding method: " + method);
		for (int i = 1; i <= nSlices; i++) {
			IJ.showProgress(i, nSlices);
			...
			if (listThresholds) {
				...
				IJLog.log("  " + i + ": " + IJ.d2s(t1, digits) + "-" + IJ.d2s(t2, digits));
			}
			int[] lut = new int[256];
			for (int j = 0; j < 256; j++) {
	...
	void autoThreshold(ImageProcessor ip) {
		...
		maxThreshold = ip.getMaxThreshold();
		if (IJ.debugMode)
			IJLog.log("Thresholder (auto): " + minThreshold + "-" + maxThreshold);
	}
