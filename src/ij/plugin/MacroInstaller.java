 ...
 * used by the Editor
 * class to install macros in menus and by the ImageJ class to install macros at
 * startup.
 */
public class MacroInstaller implements PlugIn, MacroConstants, ActionListener {
	...
	void install() {
			...
			if (token == MACRO) {
				...
				if (nextToken == STRING_CONSTANT) {
					...
					if (!isTool && !name.contains("Tool Options"))
						count++;
					// IJLog.log("install: "+isTool+" "+count+" "+toolCount+" "+name);
					macroNames.add(name);
					macroStarts.add(i + 2);
					if (isTool) {
						tools.add(name);
		...
		if (toolCount > 0 && installTools) {
			...
			if (toolCount == 1)
				...
				for (int i = 0; i < tools.size(); i++) {
					...
					// IJLog.log("addMacroTool: "+i+" "+toolName+" "+this);
		...
		if (macrosMenu != null && nMacros > 0) {
			instance = this;
			// IJLog.log("Updating instance: "+nMacros+" "+this);
		}
		if (shortcutsInUse != null && text != null)
			IJ.showMessage("Install Macros",
	...
	private void resetMenu(boolean isPluginsMacrosMenu) {
		...
		// IJLog.log("resetMenu: "+isPluginsMacrosMenu+" "+itemCount+" "+baseCount);
	...
	public boolean runMacroTool(String name) {
		for (int i = 0; i < macroNames.size(); i++) {
			if (macroNames.get(i).startsWith(name)) {
				if (macroToolThread != null && macroToolThread.getName().indexOf(name) != -1
	...
	public boolean runMenuTool(String name, String command) {
		// IJLog.log("runMenuTool: "+name+" "+command+" "+macroNames.size()+"
		// "+instance);
		for (int i = 0; i < macroNames.size(); i++) {
			// IJLog.log(" "+i+" "+macroNames.get(i));
			if (macroNames.get(i).startsWith(name)) {
