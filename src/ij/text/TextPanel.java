...
public class TextPanel extends Panel implements AdjustmentListener,
	...
	void doCommand(String cmd) {
		...
		if (cmd.equals("Save As..."))
			...
			new PlotContentsDialog(title, getOrCreateResultsTable())
					.showDialog(getParent() instanceof Frame ? (Frame) getParent() : null);
	...
	public void setSelection(int startLine, int endLine) {
		...
		if (endLine < 0)
			endLine = 0;
		if (startLine >= iRowCount)
			startLine = iRowCount - 1;
		if (endLine >= iRowCount)
			endLine = iRowCount - 1;
		selOrigin = startLine;
		selStart = startLine;
		selEnd = endLine;
	...
	public ResultsTable getOrCreateResultsTable() {
		if ((rt == null || rt.size() == 0) && iRowCount > 0 && labels != null && !labels.equals("")) {
			...
			if (tmpDir == null) {
				if (IJ.debugMode)
					IJLog.log("getOrCreateResultsTable: tmpDir null");
				return null;
			}
			...
			try {
				...
				rt = null;
				if (IJ.debugMode)
					IJLog.log("getOrCreateResultsTable: " + e);
			}
		}
		if (IJ.debugMode)
			IJLog.log("getOrCreateResultsTable: " + rt);
		return rt;
	}
