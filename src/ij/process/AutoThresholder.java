...
/**
 * Autothresholding methods (limited to 256 bin histograms) from the
 * Auto_Threshold plugin
...
public class AutoThresholder {
	...
	public int getThreshold(Method method, int[] histogram) {
		...
		switch (method) {
			case Default:
				threshold = defaultIsoData(histogram);
				break;
			case IJ_IsoData:
				threshold = IJIsoData(histogram);
				break;
			case Huang:
			...
			case IsoData:
				...
				break;
			case Li:
				threshold = Li(histogram);
				break;
			case MaxEntropy:
				threshold = MaxEntropy(histogram);
				break;
			case Mean:
				threshold = Mean(histogram);
				break;
			case MinError:
				threshold = MinErrorI(histogram);
				break;
			case Minimum:
				threshold = Minimum(histogram);
				break;
			case Moments:
				threshold = Moments(histogram);
				break;
			case Otsu:
			...
			case Percentile:
				...
				break;
			case RenyiEntropy:
				threshold = RenyiEntropy(histogram);
				break;
			case Shanbhag:
				threshold = Shanbhag(histogram);
				break;
			case Triangle:
				threshold = Triangle(histogram);
				break;
			case Yen:
				threshold = Yen(histogram);
				break;
		}
		if (threshold == -1)
			threshold = 0;
	...
	int Intermodes(int[] data) {
		...
		while (!bimodalTest(hist)) {
			...
			if (iter > 10000) {
				threshold = -1;
				IJLog.log("Intermodes Threshold not found after 10000 iterations.");
				return threshold;
			}
		}
	...
	int IsoData(int[] data) {
		...
		// initial threshold,
		// then the averages of the pixels at or below the threshold and pixels above
		// are computed.
		...
		// The code in ImageJ that implements this function is the getAutoThreshold()
		// method in the ImageProcessor class.
		//
	...
	int defaultIsoData(int[] data) {
		// This is the modified IsoData method used by the "Threshold" widget in
		// "Default" mode
		int n = data.length;
		int[] data2 = new int[n];
	...
	int MinErrorI(int[] data) {
		...
		while (threshold != Tprev) {
			...
			sqterm = (w1 * w1) - w0 * w2;
			if (sqterm < 0) {
				IJLog.log("MinError(I): not converging.");
				return threshold;
			}

			// The updated threshold is the integer part of the solution of the quadratic
			// equation.
			Tprev = threshold;
	...
	int Minimum(int[] data) {
		...
		while (!bimodalTest(iHisto)) {
			...
			if (iter > 10000) {
				threshold = -1;
				IJLog.log("Minimum: threshold not found after 10000 iterations.");
				return threshold;
			}
		}
	...
	int Moments(int[] data) {
		...
		// Ported to ImageJ plugin by G.Landini from the the open source project FOURIER
		// 0.8
		// by M. Emre Celebi , Department of Computer Science, Louisiana State
	...
	int Otsu(int[] data) {
		...
		for (k = 1; k < L - 1; k++) { // No need to check endpoints k = 0 or k = L-1
			...
			// The float casting here is to avoid compiler warning about loss of precision
			// and
	...
	int RenyiEntropy(int[] data) {
		...
		// IJLog.log(""+t_star1+" "+t_star2+" "+t_star3);
	...
	int Triangle(int[] data) {
		...
		// The Triangle algorithm cannot tell whether the data is skewed to one side or
		// another.
		// This causes a problem as there are 2 possible thresholds between the max and
		// the 2 extremes
		// of the histogram.
		...
		// IJLog.log(""+min+" "+max+" "+min2);
		boolean inverted = false;
		if ((max - min) < (min2 - max)) {
			// reverse the histogram
			// IJLog.log("Reversing histogram.");
			inverted = true;
