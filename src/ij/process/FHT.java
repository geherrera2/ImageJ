...
public class FHT extends FloatProcessor {
	...
	/**
	 * Returns true of this FHT contains a square image with a width that is a power
	 * of two.
	 */
	 ...
	 * domain.
	 * The image contained in this FHT must be square and its width must be a power
	 * of 2.
	 */
	...
	/**
	 * Calculates the Fourier amplitudes of an array, based on a 1D Fast Hartley
	 * Transform.
	 * With no Window function, if the array size is a power of 2, the input
	 * function
	 * should be either periodic or the data at the beginning and end of the array
	 * should
	 ...
	 * For data that do not fulfill these conditions, a window function can be used
	 * to
	 * avoid artifacts from the edges. See
	 * http://en.wikipedia.org/wiki/Window_function.
	 *
	 * Supported window functions: Hamming, Hann ("raised cosine"), flat-top.
	 * Flat-top
	 * refers to the HFT70 function in the report cited below, it is named for its
	 * response in the frequency domain: a single-frequency sinewave becomes a peak
	 * with
	 ...
	 * (roughly) corresponds to the RMS amplitude of a sinewave (i.e.,
	 * amplitude/sqrt(2)),
	 * and the first Fourier amplitude corresponds to DC component (average value of
	 * the data). If the sine frequency falls between two discrete frequencies of
	 * the
	 * Fourier transform, peak heights can deviate from the true RMS amplitude by up
	 * to
	 * approx. 36, 18, 15, and 0.1% for no window function, Hamming, Hann and
	 * flat-top
	 * window functions, respectively.
	 * When calculating the power spectrum from the square of the output, note that
	 * the
	 * result is quantitative only if the input array size is a power of 2; then the
	 * spectral density of the power spectrum must be divided by 1.3628 for the
	 * Hamming,
	 ...
	 * (DFT),
	 * including a comprehensive list of window functions and some new flat-top
	 * windows.
	 * Technical Report, MPI f. Gravitationsphysik, Hannover, 2002;
	 * http://edoc.mpg.de/395068
	 *
	 * @param data       Input array; its size need not be a power of 2. The input
	 *                   is not modified..
	 * @param windowType may be NO_WINDOW, then the input array is used as it is.
	 *                   Otherwise, it is multiplied by a window function, which can
	 *                   be HAMMING, HANN or
	 *                   FLATTOP.
	 * @return Array with the result, i.e., the RMS amplitudes for each frequency.
	 *         The output array size is half the size of the 2^n-sized array used
	 *         for the FHT;
	 *         array element [0]corresponds to frequency zero (the "DC component").
	 *         The first
	 *         nonexisting array element, result[result.length] would correspond to
	 *         a frequency
	 *         of 1 cycle per 2 input points, i.e., the Nyquist frequency. In other
	 *         words, if
	 *         the spacing of the input data points is dx, results[i] corresponds to
	 *         a frequency
	...
	public float[] fourier1D(float[] data, int windowType) {
		...
		int size = 2;
		while (size < n)
			size *= 2; // find power of 2 where the data fit
	 ...
	 * Note that all amplitudes in the output 'x' are multiplied by the array
	 * length.
	 * Therefore, to get the power spectrum, for 1 <=i < N/2, use
	 * ps[i] = (x[i]*x[i]+x[maxN-i]*x[maxN-i])/(maxN*maxN), where maxN is the array
	 * length.
	 * To get the real part of the complex FFT, for i=0 use x[0]/maxN,
	 * and for i>0, use (x[i]+x[maxN-i])/(2*maxN).
	 * The imaginary part of the complex FFT, with i>0, is given by
	 * (x[i]-x[maxN-i])/(2*maxN)
	 * The coefficients of cosine and sine are like the real and imaginary values
	 * above,
	 ...
	 * 
	 * @param x       Input array; will be overwritten by the output in the range
	 *                given by base and maxN.
	 ...
	 *                Note that all amplitudes in the output 'x' are multiplied by
	 *                maxN.
	 */
	...
	/**
	 * Converts this FHT to a complex Fourier transform and returns it as a two
	 * slice stack.
