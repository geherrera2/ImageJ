...
public abstract class ImageProcessor implements Cloneable {
	...
	/**
	 * Returns the bit depth, 8, 16, 24 (RGB) or 32. RGB images actually use 32 bits
	 * per pixel.
	 */
	...
	public int getBestIndex(Color c) {
		...
		for (int i = 0; i < mapSize; i++) {
			...
			// ij.IJLog.log(" "+i+" "+minIndex+" "+distance+" "+(rLUT[i]&255)+"
	...
	public void setThreshold(double minThreshold, double maxThreshold, int lutUpdate) {
		// ij.IJLog.log("setThreshold: "+" "+minThreshold+" "+maxThreshold+"
		// "+lutUpdate);
	 ...
	 * 'red' (red LUT, the default), 'b&w' (black and white LUT), 'over/under'
	 * (over/under LUT) or
	 * 'no-lut' (no LUT changes), for example "Huang dark b&w". The display range
	 * of 16-bit and 32-bit images is not reset if the 'method' string contains
	 * 'no-reset'.
	 * 
	...
	public void setBinaryThreshold() {
		// ij.IJLog.log("setMaskThreshold1");
		...
		// ij.IJLog.log("setMaskThreshold2 "+t1+" "+t2);
		setThreshold(t1, t2, ImageProcessor.NO_LUT_UPDATE);
	}

	/**
	 * Resets the threshold if minThreshold=maxThreshold and
	 * lutUpdateMode=NO_LUT_UPDATE.
	 * This removes the invisible threshold set by the MakeBinary and Convert to
	 * Mask commands.
	 * 
	 ...
	 * </pre>
	 * 
	 * The example assumes <code>snapshot()</code> has been called, which is the
	 * case
	 * for code executed in the <code>run()</code> method of plugins that implement
	 * the
	...
	/*
	 * Draws a line using the Bresenham's algorithm that is 4-connected instead of
	 * 8-connected.<br>
	 * Based on code from
	 * http://stackoverflow.com/questions/5186939/algorithm-for-drawing-a-4-
	 * connected-line<br>
	...
	/**
	 * Draws a string at the current drawing location using the current fill/draw
	 * value.
	 ...
	 * including the descent of the font (i.e., characters reaching below the
	 * baseline)
	 ...
	 * Draws multiple lines if the string contains newline characters.
	 * The x coordinate is the left/center/right end of the string for
	 * left/center/right
	 * justification.
	 ...
	 * is CENTER_JUSTIFY, RIGHT_JUSTIFY or LEFT_JUSTIFY. The default is
	 * LEFT_JUSTIFY.
	 */
	 ...
	 * space (whitespace) between the x drawing coordinate and the string, not
	 * necessarily
	 * including all whitespace at the right.
	 */
	public int getStringWidth(String s) {
		// Note that fontMetrics.getStringBounds often underestimates the width (worst
		// for italic fonts on Macs)
		// On the other hand, GlyphVector.getPixelBounds (returned by
		// this.getStringBounds)
		// does not include the full character width of e.g. the '1' character, which
		// would make
	 ...
	 * assuming it is drawn at (x=0, y=0). As drawString draws above the drawing
	 * location,
	...
	/**
	 * Uses the current interpolation method to find the pixel value at real
	 * coordinates (x,y).
	...
	/**
	 * Sets a new pixel array for the image. The length of the array must be equal
	 * to width*height.
	 ...
	 *                    aliasing artifacts; the kernel shape for averaging is
	 *                    determined by
	...
	protected ImageProcessor resizeLinearly(int width2, int height2) {
		...
		for (int x = 0; x < width2; x++) {
			...
			if (x2 == width1)
				x2 = width1 - 1;
			fraction = x * scale - x1;
			// ij.IJLog.log(x+" "+x1+" "+x2+" "+fraction+" "+width1+" "+width2);
	 ...
	 * possible
	 * value that a pixel can have, from 0 to 255 (8-bit image) or 0-65535 (16-bit
	 * image).
	 * Thus, the array size is 256 or 65536, and the bin width in uncalibrated units
	 * is 1.
	 * <p>
	 * For RGB images, the brightness is evaluated using the color weights (which
	 * would result in a
	 * float value) and rounded to an int. This gives 256 bins.
	 * FloatProcessor.getHistogram is not
	 * implemented (returns null).
	 */
	public abstract int[] getHistogram();

	/**
	 * Returns the histogram of the image or ROI, using the specified number of
	 * bins.
	 */
	...
	/**
	 * Erodes the image or ROI using a 3x3 maximum filter. Requires 8-bit or RGB
	 * image.
	 */
	public abstract void erode();

	/**
	 * Dilates the image or ROI using a 3x3 minimum filter. Requires 8-bit or RGB
	 * image.
	 */
	 ...
	 * For byte and short images, converts to binary using an automatically
	 * determined
	...
	/**
	 * Returns a pixel value (threshold) that can be used to divide the image into
	 * objects
	 * and background. It does this by taking a test threshold and computing the
	 * average
	 * of the pixels at or below the threshold and pixels above. It then computes
	 * the average
	 * of those two, increments the threshold, and repeats the process. Incrementing
	 * stops
	 * when the threshold is larger than the composite average. That is, threshold =
	 * (average
	 * background + average objects)/2. This description was posted to the ImageJ
	 * mailing
	...
	/**
	 * This is a version of getAutoThreshold() that uses a histogram passed as an
	 * argument.
	 */
	...
	/**
	 * Updates the clipping rectangle used by lineTo(), drawLine(), drawDot() and
	 * drawPixel().
	 * The clipping rectangle is reset by passing a null argument or by calling
	 * resetRoi().
	 */
	 ...
	 * accessed by toFloat(channelNumber, fp) and written by
	 ...
	 *                      FloatProcessor
	 *                      is overwritten when converting data (re-using its pixels
	 *                      array
	...
	/**
	 * Returns a binary mask, or null if a threshold is not set or this is an RGB
	 * image.
	 * 
