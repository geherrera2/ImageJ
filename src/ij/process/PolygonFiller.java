 ...
 * 
 * Note that by ImageJ convention, outline and pixel coordinates are shifted by
 * 0.5:
 * Pixel (0,0) is enclosed by the rectangle between (0,0) and (1,1); thus all
 * 0.5
...
public class PolygonFiller {
	...
	/**
	 * Constructs a PolygonFiller using the specified polygon with integer
	 * coordinates.
	 */
	...
	/**
	 * Constructs a PolygonFiller using the specified polygon with floating-point
	 * coordinates.
	 */
	...
	public void fillByteProcessorMask(ByteProcessor mask) {
		...
		// IJLog.log("yMin="+yMin+" yStart="+yStart+" nActive="+activeEdges);
	...
	void printEdges() {
		for (int i = 0; i < edges; i++) {
			int index = i;
			IJLog.log(i + ": x=" + IJ.d2s(ex[index]) + " y=" + ey1[index] + " to " + ey2[index] + " sl="
