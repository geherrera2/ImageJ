...
public class FloatProcessor extends ImageProcessor {
	...
	public Image createImage() {
		...
		// ij.IJLog.log("createImage: "+firstTime+" "+lutAnimation+" "+thresholding);
	...
	private FloatProcessor downsize1D(int dstWidth, int dstHeight, boolean xDirection) {
		...
		DownsizeTable dt = xDirection ? new DownsizeTable(getWidth(), roiX, roiWidth, dstWidth, interpolationMethod)
		...
		for (int srcLine = srcLine0, dstLine = 0; dstLine < dstLines; srcLine++, dstLine++) {
			...
			int tablePointer = 0;
			for (int srcPoint = dt.srcStart, p = srcPoint * srcPointInc
					+ srcLine * srcLineInc; srcPoint <= dt.srcEnd; srcPoint++, p += srcPointInc) {
				float v = pixels[p];
	...
	/**
	 * Returns a 256 bin histogram of the current ROI or of the entire image if
	 * there is no ROI.
	 */
