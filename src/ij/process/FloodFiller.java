...
/**
 * This class, which does flood filling, is used by the floodFill() macro
 * function and
...
public class FloodFiller {
	...
	public boolean fill(int x, int y) {
		...
		while (true) {
			...
			if (x == -1)
				return true;
			y = popy();
			if (ip.getPixel(x, y) != color)
				continue;
			int x1 = x;
			int x2 = x;
			while (ip.getPixel(x1, y) == color && x1 >= 0)
				x1--; // find start of scan-line
			x1++;
			while (ip.getPixel(x2, y) == color && x2 < width)
	...
	/**
	 * This method is used by the particle analyzer to remove interior holes from
	 * particle masks.
	 */
	public void particleAnalyzerFill(int x, int y, double level1, double level2, ImageProcessor mask,
			Rectangle bounds) {
		int width = ip.getWidth();
		int height = ip.getHeight();
		// IJLog.log("ff: "+x+" "+y+" "+level1+" "+level2+" "+ip+" "+mask+" "+bounds);
		...
		while (true) {
			...
			if (x == -1)
				return;
			y = popy();
			if (!inParticle(x, y, level1, level2))
				continue;
			int x1 = x;
			int x2 = x;
			while (inParticle(x1, y, level1, level2) && x1 >= 0)
				x1--; // find start of scan-line
			x1++;
			while (inParticle(x2, y, level1, level2) && x2 < width)
				x2++; // find end of scan-line
			x2--;
