 ...
 * Convention used: The left edges of the first pixel are the same for source
 * and destination.
 * E.g. when downsizing by a factor of 2, pixel 0 of the destination
 * takes the space of pixels 0 and 1 of the source.
 *
 * Example for use: Downsizing row 0 of 'pixels' from 'roi.width' to
 * 'destinationWidth'.
...
public class DownsizeTable {
	...
	/**
	 * Kernel sizes corresponding to the interpolation methods NONE, BILINEAR,
	 * BICUBIC
	 */
	 ...
	 * 
	 * @param srcSize             Size of source data, i.e., width or height of
	 *                            input image
	 * @param srcOrigin           Index of first pixel of source data that
	 *                            corresponds to an ouput pixel,
	 *                            0 or origin of source rectangle if only a roi is
	 *                            scaled
	 * @param srcLength           Number of pixels of source data that should
	 *                            correspond to output, i.e.,
	 *                            width or height of source roi
	 * @param dstSize             Number of destination pixels.
	 * @param interpolationMethod One of the methods defined in ImageProcessor:
	...
	DownsizeTable(int srcSize, int srcOrigin, int srcLength, int dstSize, int interpolationMethod) {
		...
		// IJLog.log("src size="+srcSize+" range="+srcStart+"-"+srcEnd+"
		// array:"+arraySize+" scale="+(float)scale);
		...
		for (int dst = 0; dst < dstSize; dst++) {
			...
			for (int src = lowestS; src <= highestS; src++) {
				...
				// if(p-(s-srcStart)*kernelSize>=kernelSize)IJLog.log(srcSize+">"+dstSize+": too
				// long: src="+src+" dst="+dst);
				indices[p] = dst;
				float weight = kernel(dst - dstIndex(src));
				sum += weight;
				weights[p] += weight;
				// IJLog.log("src="+src+"("+s+") to "+dst+" w="+weight+" p="+p);
			}
			...
			if (iStop >= indices.length)
				iStop = indices.length - 1;
			// IJLog.log("normalize "+iStart+"-"+iStop+" sum="+sum);
