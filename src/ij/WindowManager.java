...
public class WindowManager {
	...
	public static void setCurrentWindow(ImageWindow win) {
		if (win == null || win.isClosed() || win.getImagePlus() == null) // deadlock-"wait to lock"
			return;
		// IJLog.log("setCurrentWindow: "+win.getImagePlus().getTitle()+"
	...
	public static ImagePlus getCurrentImage() {
		...
		if (img == null)
			img = getActiveImage();
		// if (img!=null) IJLog.log("getCurrentImage: "+img.getTitle()+"
	...
	public static void setTempCurrentImage(ImagePlus img) {
		// IJLog.log("setTempImage: "+(img!=null?""+img:"null")+"
	...
	public static void setWindow(Frame win) {
		...
		if (win != null && win instanceof TextWindow && !(win instanceof Editor)
				&& !"Log".equals(((TextWindow) win).getTitle()))
			frontTable = win;
	...
	public static void showList() {
		for (int i = 0; i < imageList.size(); i++) {
			ImageWindow win = (ImageWindow) imageList.get(i);
			ImagePlus imp = win.getImagePlus();
			IJLog.log(i + " " + imp.getTitle() + (win == currentWindow ? "*" : "") + " " + imp.getID());
		}
		for (int i = 0; i < activations.size(); i++) {
			ImageWindow win = (ImageWindow) activations.get(i);
			ImagePlus imp = win.getImagePlus();
			IJLog.log(i + " " + imp.getTitle() + " " + imp.getID());
		}
		if (imageList.size() == 0)
			IJLog.log("imageList is empty");
		if (activations.size() == 0)
			IJLog.log("activations list is empty");
		IJLog.log(" ");
	}
