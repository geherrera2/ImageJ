...
/**
 * An ImagePlus contain an ImageProcessor (2D image) or an ImageStack (3D, 4D or
 * 5D image).
 * It also includes metadata (spatial calibration and possibly the
 * directory/file where
 * it was read from). The ImageProcessor contains the pixel data (8-bit, 16-bit,
 * float or RGB)
 * of the 2D image and some basic methods to manipulate it. An ImageStack is
 * essentually
...
public class ImagePlus implements ImageObserver, Measurements, Cloneable {
		...
		if (locked) {
			if (Thread.currentThread() == lockingThread) {
				...
				if (loud) {
					...
					if (IJ.debugMode)
						IJLog.log(title + " is locked by " + lockingThread + "; refused locking by "
			...
			if (IJ.debugMode)
				IJLog.log(title + ": locked" + (loud ? "" : "silently") + " by " + Thread.currentThread().getName());
			return true;
		}
	}
	...
	public void waitTillActivated() {
		if (win == null)
			return;
		if (EventQueue.isDispatchThread()) { // 'activated' is set in the EventQueue, we can't wait for it in the
												// EventQueue
			WindowManager.setTempCurrentImage(this);
			return;
		}
	...
	public void setImage(Image image) {
		if (image instanceof BufferedImage) {
			...
			if (IJ.debugMode)
				IJLog.log("setImage: type=" + type + ", bands=" + nBands + ", rgb=" + rgb);
			if (nBands > 1 && !rgb) {
	...
	public static ImageProcessor convertToImageProcessor(BufferedImage img, int band) {
		...
		// Read data as float (no matter what it is - it's the most accuracy ImageJ can
		// provide)
	 ...
	 * With
	 * stacks, the ImageProcessor must be the same type as other images in the stack
	 * and
	 * it must be the same width and height. Set 'title' to null to leave the title
	 * unchanged.
	 */
	...
	public void setStack(String title, ImageStack newStack) {
		...
		boolean invalidDimensions = (isDisplayedHyperStack() || (this instanceof CompositeImage))
				&& (win instanceof StackWindow) && !((StackWindow) win).validDimensions();
		if (newStackSize > 1 && !(win instanceof StackWindow)) {
			...
			if (IJ.isMacro())
				waitTillActivated(); // wait for stack window to be activated
			setPosition(1, 1, 1);
	...
	public void setStack(ImageStack newStack, int channels, int slices, int frames) {
		...
		if (IJ.debugMode)
			IJLog.log("setStack: " + newStack.getSize() + " " + channels + " " + slices + " " + frames + " "
					+ isComposite());
	 ...
	 * IJLog.log("Area: " + stats.area);
	 * IJLog.log("Mean: " + stats.mean);
	 * IJLog.log("Max: " + stats.max);
	 * </pre>
	 * 
	...
	public void setDimensions(int nChannels, int nSlices, int nFrames) {
		// IJLog.log("setDimensions: "+nChannels+" "+nSlices+" "+nFrames+"
		// "+getImageStackSize());
		...
		boolean updateWin = isDisplayedHyperStack()
				&& (this.nChannels != nChannels || this.nSlices != nSlices || this.nFrames != nFrames);
		boolean newSingleImage = win != null && (win instanceof StackWindow) && nChannels == 1 && nSlices == 1
				&& nFrames == 1;
		if (newSingleImage)
			updateWin = true;
	...
	/**
	 * Returns 'true' if this is a hyperstack currently being displayed in a
	 * StackWindow.
	 */
	...
	public void setTypeToColor256() {
		if (imageType == ImagePlus.GRAY8) {
			ImageProcessor ip2 = getProcessor();
			if (ip2 != null && ip2.getMinThreshold() == ImageProcessor.NO_THRESHOLD && ip2.isColorLut()
					&& !ip2.isPseudoColorLut()) {
				imageType = COLOR_256;
				typeSet = true;
			}
		}
	...
	public void setProperties(String[] props) {
		...
		if (props == null)
			return;
		// IJLog.log("setProperties: "+props.length+" "+getTitle());
		for (int i = 0; i < props.length; i += 2) {
			String key = props[i];
			String value = props[i + 1];
			// IJLog.log(" "+key+" "+value.length());
			setProp(key, value);
		}
	}
	...
	final void setCurrentSlice(int slice) {
		currentSlice = slice;
		int stackSize = getStackSize();
		if (nChannels == stackSize)
			updatePosition(currentSlice, 1, 1);
		if (nSlices == stackSize)
			updatePosition(1, currentSlice, 1);
		if (nFrames == stackSize)
			updatePosition(1, 1, currentSlice);
	}
	...
	public void setPosition(int channel, int slice, int frame) {
		// IJLog.log("setPosition: "+channel+" "+slice+" "+frame+" "+noUpdateMode);
		verifyDimensions();
		if (channel < 0)
			channel = 0;
		if (slice < 0)
	...
	/**
	 * Converts the stack index 'n' (one-based) into a hyperstack position (channel,
	 * slice, frame).
	 */
	...
	public void setRoi(Roi newRoi, boolean updateDisplay) {
		...
		if (newRoi.isVisible()) {
			if ((newRoi instanceof Arrow) && newRoi.getState() == Roi.CONSTRUCTING && bounds.width == 0
					&& bounds.height == 0) {
				deleteRoi();
				roi = newRoi;
				return;
		...
		if (bounds.width == 0 && bounds.height == 0
				&& !(newRoi.getType() == Roi.POINT || newRoi.getType() == Roi.LINE)) {
			deleteRoi();
			return;
		}
		roi = newRoi;
	 ...
	 * in
	 * the tool bar is active. The user interactively sets the selection size and
	 * shape.
	 */
	...
	public boolean okToDeleteRoi() {
		if (roi != null && (roi instanceof PointRoi) && getWindow() != null
				&& ((PointRoi) roi).promptBeforeDeleting()) {
			int npoints = ((PolygonRoi) roi).getNCoordinates();
			int counters = ((PointRoi) roi).getNCounters();
			String msg = "Delete this multi-point selection (" + npoints + " points, " + counters + " counter"
					+ (counters > 1 ? "s" : "") + ")?";
	...
	public void restoreRoi() {
		...
		if (previousRoi != null) {
			...
			if (r.width <= width || r.height <= height || (r.x < width && r.y < height) || isSmaller(pRoi)) { // will it
																												// (mostly)
																												// fit
																												// in
																												// this
				...
				roi = (Roi) pRoi.clone();
				roi.setImage(this);
				if (r.x >= width || r.y >= height || (r.x + r.width) < 0 || (r.y + r.height) < 0) // does it need to be
																									// moved?
	 ...
	 * where 'directory' is the directory path and 'format' is "tif", "png" or
	 * "jpg".
	...
	public void copyAttributes(ImagePlus imp) {
		if (IJ.debugMode)
			IJLog.log("copyAttributes: " + imp.getID() + "  " + this.getID() + " " + imp + "   " + this);
	...
	public Calibration getCalibration() {
		// IJLog.log("getCalibration: "+globalCalibration+" "+calibration);
	...
	public void setGlobalCalibration(Calibration global) {
		// IJLog.log("setGlobalCalibration: "+calibration);
		if (global == null)
	...
	/**
	 * This is a version of getGlobalCalibration() that can be called from a static
	 * context.
	 */
	 ...
	 * is a selection the same size as the image on the clipboard, the image is
	 * inserted
	 * into that selection, otherwise the selection is inserted into the center of
	 * the image.
	 */
	public void paste() {
		if (clipboard == null)
		...
		if (r == null || (cr.width != r.width || cr.height != r.height)) {
			// Create a new roi centered on visible part of image, or centered on image if
			// clipboard is >= image
	...
	public static void logImageListeners() {
		if (listeners.size() == 0)
			IJLog.log("No ImageListeners");
		else {
			for (Object li : listeners) {
				IJLog.log("imageListener: " + li);
				if (li instanceof Window)
					IJLog.log("   (" + (((Window) li).isShowing() ? "showing" : "invisible") + ")");
			}
		}
	}
	...
	/**
	 * Set the default 16-bit display range, where 'bitDepth' must be 0
	 * (auto-scaling),
	 * 8 (0-255), 10 (0-1023), 12 (0-4095, 14 (0-16383), 15 (0-32767) or 16
	 * (0-65535).
	 */
	...
	/**
	 * Returns the default 16-bit display range, 0 (auto-scaling), 8, 10, 12, 14, 15
	 * or 16.
	 */
	 ...
	 *                                       does not have an overlay and the
	 *                                       RoiManager overlay is null.<br>
	 *                                       Non-RGB stacks are converted to
	 *                                       RGB.<br>
	 *                                       Copied from OverlayCommands and
	 *                                       modified by Marcel Boeglin
	 *                                       on 2014.01.08 to work with HyperStacks.
	 */
	public void flattenStack() {
		if (IJ.debugMode)
			IJLog.log("flattenStack");
	...
	private void flattenImage(ImageStack stack, int slice, Overlay overlay, boolean showAll) {
		...
		for (int i = 0; i < overlay.size(); i++) {
			...
			// IJLog.log(slice+" "+i+" "+roiPosition+" "+showAll+" "+overlay.size());
	...
	/**
	 * Installs a list of ROIs that will be drawn on this image as a non-destructive
	 * overlay.
	 * 
	...
	public String toString() {
		return "img[\"" + getTitle() + "\" (" + getID() + "), " + getBitDepth() + "-bit, " + width + "x" + height + "x"
				+ getNChannels() + "x" + getNSlices() + "x" + getNFrames() + "]";
	}
