...
public class ImageWindow extends Frame
	...
	private void setLocationAndSize(boolean updating) {
		...
		if (xbase == -1) {
			...
			// if (IJ.debugMode) IJLog.log("ImageWindow.xbase: "+xbase);
			xloc = xbase;
			yloc = ybase;
		}
		int x = xloc;
	...
	public Insets getInsets() {
		...
		if (extraHeight < 0)
			extraHeight = 0;
		insets = new Insets(insets.top + textGap + extraHeight, insets.left + extraWidth, insets.bottom + extraHeight,
	...
	Dimension getExtraSize() {
		...
		for (int i = 1; i < members; i++) {
			...
			if (IJ.debugMode)
				IJLog.log(i + "  " + d.height + " " + extraHeight);
		}
	...
	public void maximize() {
		...
		if (IJ.debugMode)
			IJLog.log("maximize: " + rect);
		setLocationAndSize(rect.x, rect.y, rect.width, rect.height);
	}

	public void minimize() {
		if (IJ.debugMode)
			IJLog.log("minimize: " + unzoomWhenMinimizing);
	...
	public void focusGained(FocusEvent e) {
		if (!Interpreter.isBatchMode() && ij != null && !ij.quitting() && imp != null) {
			// if (IJ.debugMode) IJLog.log("focusGained: "+imp);
			WindowManager.setCurrentWindow(this);
		}
	}
	...
	public void windowActivated(WindowEvent e) {
		if (IJ.debugMode)
			IJLog.log("windowActivated: " + imp.getTitle());
		if (IJ.isMacOSX())
	...
	public void windowStateChanged(WindowEvent e) {
		...
		if (IJ.debugMode)
			IJLog.log("windowStateChanged: " + oldState + " " + newState);
	...
	public synchronized void mouseWheelMoved(MouseWheelEvent e) {
		...
		if (IJ.debugMode) {
			IJLog.log("mouseWheelMoved: " + e);
			IJLog.log("  type: " + e.getScrollType());
			IJLog.log("  ctrl: " + ctrl);
			IJLog.log("  rotation: " + rotation);
			IJLog.log("  amount: " + amount);
		}
		if (amount < 1)
			amount = 1;
		if (rotation == 0)
	...
	public static void setImageJMenuBar(ImageWindow win) {
		...
		if (setMenuBar && ij != null && !ij.quitting() && !Interpreter.nonBatchMacroRunning()) {
			...
			// if (IJ.debugMode) IJLog.log("setMenuBar: "+time+"ms
			// ("+Menus.setMenuBarCount+")");
			if (time > 2000L)
