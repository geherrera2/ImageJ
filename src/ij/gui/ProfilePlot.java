...
public class ProfilePlot {
	 ...
	 * the line coordinates are interpreted as the roi line shown at high zoom
	 * level,
	...
	double[] getIrregularProfile(Roi roi, ImageProcessor ip, Calibration cal) {
		...
		for (int i = 1; i < n; i++) {
			...
			do {
				...
				// IJLog.log(i+" "+IJ.d2s(xinc,5)+" "+IJ.d2s(yinc,5)+" "+IJ.d2s(distance,2)+"
	 ...
	 * boolean interpolate = PlotWindow.interpolate;
	 * FloatPolygon p = roi.getFloatPolygon();
	 * float[][] xyPoints = ((PolygonRoi)roi).getEquidistantPoints(p.xpoints,
	 * p.ypoints, p.npoints, 1.0, imp);
