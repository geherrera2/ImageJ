...
/**
 * This class creates an image that line graphs, scatter plots and plots of
 * vector fields
 * (arrows) can be drawn on and displayed.
 *
 * Note that the clone() operation is a shallow clone: objects like arrays, the
 * PlotProperties,
 * PlotObjects, the ImagePlus etc. of the clone remain the same as those of the
 * original.
 *
 * @author Wayne Rasband
 * @author Philippe CARL, CNRS, philippe.carl (AT) unistra.fr (log axes, arrows,
 *         ArrayList data)
 * @author Norbert Vischer (overlay range arrows, 'R'eset range, filled plots,
 *         dynamic plots, boxes and whiskers, superscript)
 * @author Michael Schmid (axis grid/ticks, resizing/panning/changing range,
 *         high-resolution, serialization)
 */
public class Plot implements Cloneable {

	/** Text justification. */
	public static final int LEFT = ImageProcessor.LEFT_JUSTIFY, CENTER = ImageProcessor.CENTER_JUSTIFY,
			RIGHT = ImageProcessor.RIGHT_JUSTIFY;
	/** Legend positions */
	// NOTE: These have only bits of LEGEND_POSITION_MASK set. The flags of the
	// legend are stored as flags of the legend PlotObject.
	// These are saved in the flags of the PlotObject class; thus bits up to 0x0f
	// are reserved
	public static final int TOP_LEFT = 0x90, TOP_RIGHT = 0xA0, BOTTOM_LEFT = 0xB0, BOTTOM_RIGHT = 0xC0,
			AUTO_POSITION = 0x80;
	/**
	 * Masks out bits for legend positions; if all these bits are off, the legend is
	 * turned off
	 */
	...
	/**
	 * Display points using a circle (5 pixels in diameter if line thickness<=1,
	 * otherwise 7).
	 */
	...
	/**
	 * Draw black lines between the dots and a circle with the given color at each
	 * dot
	 */
	...
	/**
	 * Draw a histogram bar for each point (bars touch each other unless the x axis
	 * has categories set via the axis label.
	 * x values should be sorted (ascending or descending)
	 */
	public static final int BAR = 11;
	/**
	 * Draw a free-standing bar for each point. x values should be equidistant and
	...
	/**
	 * The default axisFlags, will be modified by PlotWindow.noGridLines and
	 * PlotWindow.noTicks (see getDefaultFlags)
	 */
	public static final int DEFAULT_FLAGS = X_NUMBERS + Y_NUMBERS + /* X_TICKS + Y_TICKS + */
			X_GRID + Y_GRID + X_LOG_TICKS + Y_LOG_TICKS;

	/**
	 * Flag for addressing the x axis, for copying from a template: copy/write x
	 * axis range.
	 */
	// This must be 0x1 because bit shift operations are used for the other axes
	public static final int X_RANGE = 0x1;
	/**
	 * Flag for addressing the y axis, for copying from a template: copy/write y
	 * axis range
	 */
	...
	/**
	 * Flag for copying PlotObjects (curves...) from a template if the template has
	 * more PlotObjects than the Plot to copy to.
	 */
	public static final int COPY_EXTRA_OBJECTS = 0x200;

	/**
	 * The default margin width left of the plot frame (enough for 5-digit numbers
	 * such as unscaled 16-bit
	 * 
	 * @deprecated Not a fixed value any more, use getDrawingFrame() to get the
	 *             drawing area
	 */
	...
	private static final int MIN_X_GRIDSPACING = 45; // minimum distance between grid lines or ticks along x at plot
														// width 0
	private static final int MIN_Y_GRIDSPACING = 30; // minimum distance between grid lines or ticks along y at plot
														// height 0
	private final double MIN_LOG_RATIO = 3; // If max/min ratio is less than this, force linear axis even if log
	...
	private static final String MULTIPLY_SYMBOL = "\u00B7"; // middot, default multiplication symbol for scientific
															// notation. Use setOptions("msymbol=\\u00d7") for '×'
	...
	Vector<PlotObject> allPlotObjects = new Vector<PlotObject>(); // all curves, labels etc., also serialized for
																	// saving/reading
	Vector<PlotObject> allPlotObjectsSnapshot; // copy for reverting
	private PlotVirtualStack stack;
	/**
	 * For high-resolution plots, everything will be scaled with this number.
	...
	// The following are the margin sizes actually used. They are modified for font
	// size and also scaled for high-resolution plots
	...
	int preferredPlotWidth = PlotWindow.plotWidth; // default size of plot frame (not taking 'High-Resolution' scale
													// factor into account)
	...
	double[] currentMinMax = new double[] { Double.NaN, 0, Double.NaN, 0 }; // current plot range, xMin, xMax, yMin,
																			// yMax (values, not logarithm if log axis)
	...
	int[] enlargeRange; // whether to enlarge the range slightly to avoid values at the border (0=off,
						// USUALLY_ENLARGE, ALWAYS_ENLARGE)
	boolean logXAxis, logYAxis; // whether to really use log axis (never for small relative range)
	// for passing on what should be kept when 'live' plotting (PlotMaker), but note
	...
	private static double SEPARATED_BAR_WIDTH = 0.5; // for plots with separate bars (e.g. categories), fraction of
														// space, 0.1-1.0
	...
	private Point2D.Double textLoc; // remembers position of previous addLabel call (replaces text if at the same
									// position)
	private int textIndex; // remembers index of previous addLabel call (for replacing if at the same
							// position)
	 ...
	 * @param xLabel the x-axis label; see setXYLabels for seting categories on an
	 *               axis via the label
	 * @param yLabel the y-axis label; see setXYLabels for seting categories on an
	 *               axis via the label
	...
	public Plot(String title, String xLabel, String yLabel, double[] x, double[] y) {
		this(title, xLabel, yLabel, x != null ? Tools.toFloat(x) : null, y != null ? Tools.toFloat(y) : null,
				getDefaultFlags());
	}
	...
	/**
	 * Constructs a new plot from an InputStream and closes the stream. If the
	 * ImagePlus is
	 * non-null, its title and ImageProcessor are used, but the image displayed is
	 * not modified.
	 */
	...
	/**
	 * Writes this plot into an OutputStream containing (1) the serialized
	 * PlotProperties and
	 * (2) the serialized Vector of all 'added' PlotObjects. The stream is NOT
	 * closed.
	 * The plot should have been drawn already.
	 */
	// Conversion to Streams can be also used to clone plots (not a shallow clone),
	...
	/**
	 * Writes this plot into a byte array containing (1) the serialized
	 * PlotProperties and
	 * (2) the serialized Vector of all 'added' PlotObjects.
	 * The plot should have been drawn already. Returns null on error (which should
	 * never happen).
	 */
	...
	/**
	 * Sets the x-axis and y-axis range. Saves the new limits as default (so the 'R'
	 * field sets the limits to these).
	 ...
	 * Does not update the image and leaves the default limits (for reset via the
	 * 'R' field) untouched.
	 */
	void setLimitsNoUpdate(double xMin, double xMax, double yMin, double yMax) {
		...
		if (containsNaN && getNumPlotObjects(PlotObject.XY_DATA | PlotObject.ARROWS, false) == 0)// can't apply
																									// auto-range
																									// without data
			return;
		...
		if (containsNaN) { // auto range for at least one limit
			...
			for (int a = 0; a < range.length; a += 2) { // for all axes (0 for x, 2 for y): would semi-auto reverse the
														// axis?
				if (auto[a] == auto[a + 1])
					continue; // ignore if not semi-auto
				boolean currentAxisReverse = defaultMinMax[a + 1] < defaultMinMax[a];
				if ((!currentAxisReverse && range[a + 1] <= range[a])
						|| (currentAxisReverse && range[a] <= range[a + 1])) {
	...
	/**
	 * Takes over the current limits as the default ones (i.e., the limits set by
	 * clicking at the 'R' field in the bottom-left corner)
	 */
	 ...
	 * (note that ImageJ versions before to 1.52i have returned incorrect values in
	 * case of log axes)
	 * Note that future versions might return a longer array (e.g. for y2 axis
	 * limits)
	 */
	 ...
	 * Does not save the previous limits, i.e., leaves the default limits (for reset
	 * via the 'R' field) untouched.
	 */
	...
	/**
	 * Sets options for the plot. Multiple options may be separated by whitespace or
	 * commas.
	 * Note that whitespace surrounding the '=' characters is not allowed.
	 * Currently recognized options are:
	 * "addhspace=10 addvspace=5" Increases the left&right or top&bottom margins by
	 * the given number of pixels.
	 * "xinterval=30 yinterval=90" Sets interval between numbers, major ticks & grid
	 * lines
	 * (default intervals are used if the custom intervals would be too dense or too
	 * sparse)
	 * "xdecimals=2 ydecimals=-1" Sets the minimum number of decimals; use negative
	 * numbers for scientific notation.
	 * "msymbol=' \\u00d7 '" Sets multiplication symbol for scientific notation,
	 * here a cross with spaces.
	 */
	 ...
	 * If the scale remains 1.0, this will be the size of the resulting
	 * ImageProcessor.
	 ...
	 * This method does not check for the minimum size MIN_FRAMEWIDTH,
	 * MIN_FRAMEHEIGHT.
	...
	/**
	 * Adjusts the format (style) with another plot as a template. Flags determine
	 * what to
	 * copy from the template; these can be X_RANGE, Y_RANGE, COPY_SIZE,
	 * COPY_LABELS, COPY_AXIS_STYLE,
	...
	public void useTemplate(Plot plot, int templateFlags) {
		...
		for (int i = 0; i < currentMinMax.length; i++)
			if ((templateFlags >> (i / 2) & 0x1) != 0) {
				...
				if (!plotDrawn)
					defaultMinMax[i] = plot.currentMinMax[i];
			}
		...
		if ((templateFlags & (COPY_LEGEND | COPY_CONTENTS_STYLE)) != 0) {
			...
			for (PlotObject plotObject : allPlotObjects) {
				if (plotObject.type == PlotObject.XY_DATA && !plotObject.hasFlag(PlotObject.HIDDEN)) {
					...
					if (plotPObjectIndex >= plotPObjectsSize)
						break;
	...
	/**
	 * Sets the scale. Everything, including labels, line thicknesses, etc will be
	 * scaled by this factor.
	 * Also multiplies the plot size by this value. Used for 'Create high-resolution
	 * plot'.
	 * Should be called before creating the plot.
	 * Note that plots with a scale different from 1.0 must not be shown in a
	 * PlotWindow, but only as
	 ...
	 * If a label has the form {txt1,txt2,txt3}, the corresponding axis will be
	 * labeled
	 ...
	 * Call update() thereafter to make the change visible (if the image is shown
	 * already).
	 */
	 ...
	 * Does not modify the flags for logarithmic axes on/off and the FORCE2GRID
	 * flags.
	 * Call update() thereafter to make the change visible (if it is shown already).
	 */
	...
	/**
	 * Sets whether to show minor ticks on the x axis (if linear). Also sets major
	 * ticks if true and no grid is set.
	 * Call update() thereafter to make the change visible (if the image is shown
	 * already).
	 */
	...
	/**
	 * Sets whether to show minor ticks on the y axis (if linear). Also sets major
	 * ticks if true and no grid is set.
	 * Call update() thereafter to make the change visible (if the image is shown
	 * already).
	 */
	...
	/**
	 * Sets the properties of the axes. Call update() thereafter to make the change
	 * visible
	 * (if the image is shown already).
	 */
	public void setAxes(boolean xLog, boolean yLog, boolean xTicks, boolean yTicks, boolean xMinorTicks,
			boolean yMinorTicks,
	...
	/**
	 * The default flags, taking PlotWindow.noGridLines, PlotWindow.noTicks into
	 * account
	 */
	public static int getDefaultFlags() {
		int defaultFlags = 0;
		if (!PlotWindow.noGridLines) // note that log ticks are also needed because the range may span less than a
										// decade, then no grid is visible
	 ...
	 * "line", "connected circle", "filled", "bar", "separated bar", "circle",
	 * "box", "triangle", "diamond", "cross",
	 * "x" or "dot". Run <i>Help&gt;Examples&gt;JavaScript&gt;Graph Types</i> to see
	 * examples.
	 * If 'type' is in the form "code: <macroCode>", the macro given is executed to
	 * draw the symbol;
	 * macro variables 'x' and 'y' are the pixel coordinates of the point, 'xval'
	 * and 'yval' are the plot data
	 * and 'i' is the index of the data point (starting with 0 for the first point
	 * in the array).
	 * The drawing including line thickness, font size, etc. be scaled by scale
	 * factor 's' (to make high-resolution plots work).
	 * Example: "code: setFont('sanserif',12*s,'bold
	 * anti');drawString(d2s(yval,1),x-14*s,y-4*s);"
	...
	public void add(String type, double[] xvalues, double[] yvalues) {
		...
		addPoints(Tools.toFloat(xvalues), Tools.toFloat(yvalues), null, iShape,
				iShape == CUSTOM ? type.substring(5, type.length()) : null);
	}
	...
	public void add(String type, double[] yvalues) {
		...
		if (iShape == -1)
			...
			addPoints(null, Tools.toFloat(yvalues), null, iShape,
					iShape == CUSTOM ? type.substring(5, type.length()) : null);
	}
	 ...
	 * 
	 * @param xValues    the x coordinates, or null. If null, integers starting at 0
	 *                   will be used for x.
	...
	public void addPoints(float[] xValues, float[] yValues, float[] yErrorBars, int shape, String label) {
		...
		if (objectToReplace >= 0)
			allPlotObjects.set(objectToReplace, new PlotObject(xValues, yValues, yErrorBars, shape, currentLineWidth,
					currentColor, currentColor2, label));
		else
			allPlotObjects.add(new PlotObject(xValues, yValues, yErrorBars, shape, currentLineWidth, currentColor,
					currentColor2, label));
		objectToReplace = -1;
		if (plotDrawn)
			updateImage();
	...
	/**
	 * Returns the number for a given plot symbol shape, -1 for xError and -2 for
	 * yError (all case-insensitive)
	 */
	...
	public void drawVectors(ArrayList x1, ArrayList y1, ArrayList x2, ArrayList y2) {
		drawVectors(getDoubleFromArrayList(x1), getDoubleFromArrayList(y1), getDoubleFromArrayList(x2),
				getDoubleFromArrayList(y2));
	}

	/**
	 * Adds vertical error bars to the last data passed to the plot (via the
	 * constructor or addPoints).
	 */
	...
	/**
	 * Adds horizontal error bars to the last data passed to the plot (via the
	 * constructor or addPoints).
	 */
	 ...
	 * the lower right corner. Uses the justification specified by
	 * setJustification().
	 * When called with the same position as the previous addLabel call, the text of
	 * that previous call is replaced
	 */
	public void addLabel(double x, double y, String label) {
		if (textLoc != null && x == textLoc.getX() && y == textLoc.getY())
			allPlotObjects.set(textIndex, new PlotObject(label, x, y, currentJustification, currentFont, currentColor,
	...
	public void addText(String label, double x, double y) {
		allPlotObjects
				.add(new PlotObject(label, x, y, currentJustification, currentFont, currentColor, PlotObject.LABEL));
	}

	/**
	 * Adds an automatically positioned legend, where 'labels' can be a
	 * tab-delimited or
	 * newline-delimited list of curve or point labels in the sequence these data
	 * were added.
	 * Hidden data sets are ignored.
	 * If 'labels' is null or empty, the labels of the data set previously (if any)
	 * are used.
	 * To modify the legend's style, call 'setFont' and 'setLineWidth' before
	 * 'addLegend'.
	 */
	...
	/**
	 * Adds a legend at the position given in 'options', where 'labels' can be
	 * tab-delimited or
	 * newline-delimited list of curve or point labels in the sequence these data
	 * were added.
	 * Hidden data sets are ignored; no labels (and no delimiters) should be
	 * provided for these.
	 * Apart from top to bottom and bottom to top (controlled by "bottom-to-top" in
	 * the options),
	 * the sequence may be altered by preceding numbers followed by double
	 * underscores, such as
	 * "1__Other Data Set\t0__First Data Set" (the number and double underscore
	 * won't be displayed).
	 * When this possibility is used, only items with double underscores will be
	 * shown in the legend
	 * (preceding double underscores without a number make the item appear in the
	 * legend without altering the sequence).
	 * If 'labels' is null or empty, the labels of the data set previously (if any)
	 * are used.
	 * To modify the legend's style, call 'setFont' and 'setLineWidth' before
	 * 'addLegend'.
	 ...
	 * 'addLegend'
	 * 
	 * @param labels labels of the points or curves in the sequence of the data were
	 *               added, tab-delimited or linefeed-delimited.
	 *               The labels of the datasets will be set to these values. If null
	 *               or not given, the labels set
	 *               previously (if any) will be used.
	 *               Hidden data sets are ignored; no labels (and no delimiters)
	 *               should be provided for these.
	 *               Apart from top to bottom and bottom to top (controlled by the
	 *               LEGEND_BOTTOM_UP flag),
	 *               the sequence may be altered by preceding numbers followed by
	 *               double underscores, such as
	 *               "1__Other Data Set\t0__First Data Set" (the number and double
	 *               underscore won't be displayed).
	 *               When this possibility is used, only items with double
	 *               underscores will be shown in the legend
	 *               (preceding double underscores without a number make the item
	 *               appear in the legend without altering the sequence).
	 * @param flags  Bitwise or of position (AUTO_POSITION, TOP_LEFT etc.),
	 *               LEGEND_TRANSPARENT, and LEGEND_BOTTOM_UP if desired.
	...
	/**
	 * Sets the label for the plot object nuber 'index' in the sequence they were
	 * added.
	 * With index=-1, sets the label for the last object added.
	 * For x/y data, the label is used for the legend and as header in
	 * getResultsTableWithLabels.
	 * For Text/Label objects, it affects the label shown (but the plot is not
	 * redisplayed).
	 */
	public void setLabel(int index, String label) {
		if (index < 0)
			index = allPlotObjects.size() + index;
	...
	/**
	 * Changes the drawing color for the next objects that will be added to the
	 * plot.
	 ...
	 * Changes the drawing color for the next objects that will be added to the
	 * plot.
	 * It also sets secondary color: This is the color of the line for
	 * CONNECTED_CIRCLES,
	 * and the color for filling open symbols (CIRCLE, BOX, TRIANGLE).
	 * Set it to null or use the one-argument call setColor(color) to disable
	 * filling.
	 * For selecting the color of the curve passed with the constructor,
	 * use <code>setColor</code> before <code>draw</code>.
	 * The frame and labels are always drawn in black.
	...
	public void drawNormalizedLine(double x1, double y1, double x2, double y2) {
		allPlotObjects
				.add(new PlotObject(x1, y1, x2, y2, currentLineWidth, 0, currentColor, PlotObject.NORMALIZED_LINE));
	}

	/** Draws a line using the coordinate system defined by setLimits(). */
	public void drawDottedLine(double x1, double y1, double x2, double y2, int step) {
		allPlotObjects
				.add(new PlotObject(x1, y1, x2, y2, currentLineWidth, step, currentColor, PlotObject.DOTTED_LINE));
	}

	/**
	 * Sets the font size for all following addLabel() etc. operations. The
	 * currently set font when
	 * displaying the plot determines the font of all labels & numbers.
	 * After the plot has been shown, sets the font for the numbers and the legend
	 * (if present).
	 * If the plot is hown already, call update() thereafter to make the change
	 * visible.
	 */
	...
	/**
	 * Sets the font for all following addLabel() etc. operations. The currently set
	 * font when
	 * displaying the plot determines the font of all labels & numbers.
	 * After the plot has been shown, sets the font for the numbers and the legend
	 * (if present);
	...
	/**
	 * Sets the font size and style for all following addLabel() etc. operations.
	 * This leaves
	 * the font name and style of the previously used fonts unchanged. The currently
	 * set font
	...
	/**
	 * Sets the xLabelFont; must not be null. If this method is not used, the last
	 * setFont
	 * of setFontSize call before displaying the plot determines the font, or if
	 * neither
	...
	/**
	 * Gets the font for xLabel ('x'), yLabel('y'), numbers ('f' for 'frame') or the
	 * legend ('l').
	...
	/**
	 * Sets the font for xLabel ('x'), yLabel('y'), numbers ('f' for 'frame') or the
	 * legend ('l')
	 */
	...
	/**
	 * Get the x coordinates of the data set passed with the constructor (if not
	 * null)
	 * or otherwise of the data set of the first 'addPoints'. Returns null if
	 * neither exists
	 */
	...
	/**
	 * Get the y coordinates of the data set passed with the constructor (if not
	 * null)
	 * or otherwise of the data set of the first 'addPoints'. Returns null if
	 * neither exists
	 */
	...
	/**
	 * Get the data of the n-th Plot Object containing xy data in the sequence they
	 * were added
	 * (Other Plot Objects such as labels, arrows, lines, shapes and hidden
	 * PlotObjects are not counted).
	 * The array returned has elements [0] x data, [1] y data, [2] x error bars, [3]
	 * y error bars.
	...
	public float[][] getDataObjectArrays(int index) {
		...
		for (PlotObject plotObject : allPlotObjects) {
			if (plotObject.type != PlotObject.XY_DATA || plotObject.hasFlag(PlotObject.HIDDEN))
				continue;
			if (index == i)
				return new float[][] { plotObject.xValues, plotObject.yValues, plotObject.xEValues,
						plotObject.yEValues };
			i++;
		}
		return null;
	...
	/**
	 * Gets an array with human-readable designations of the PlotObjects (curves,
	 * labels, ...)
	 * in the sequence they were added (the object passed with the constructor is
	 * first,
	...
	/**
	 * Gets an array with human-readable designations of the PlotObjects containing
	 * xy data
	 * in the sequence they were added. Other Plot Objects such as labels, arrows,
	 * lines,
	 * shapes and hidden PlotObjects are not counted.
	 * (the object passed with the constructor is first, even though it is plotted
	 * last).
	 */
	...
	/**
	 * Returns the number of PlotObjects (curves, labels, ...) passed with the
	 * constructor or added by 'add' or 'draw' methods.
	...
	String[] getPlotObjectDesignations(int mask, boolean includeHidden) {
		...
		if (names.length == 0)
			return names;
		int iData = 1, iArrow = 1, iLine = 1, iText = 1, iBox = 1, iShape = 1; // Human readable counters of each object
																				// type
		int i = 0;
		for (PlotObject plotObject : allPlotObjects) {
			int type = plotObject.type;
			if ((type & mask) == 0 || (!includeHidden && plotObject.hasFlag(PlotObject.HIDDEN)))
				continue;
			String label = plotObject.label;
			switch (type) {
				case PlotObject.XY_DATA:
					names[i] = "Data Set " + iData + ": " + (plotObject.label != null ? plotObject.label
							: "(" + plotObject.yValues.length + " data points)");
					iData++;
					break;
				...
				case PlotObject.DOTTED_LINE:
					String detail = "";
					if (type == PlotObject.DOTTED_LINE)
						detail = "dotted ";
	...
	/**
	 * Add the i-th PlotObject (in the sequence how they were added, including
	 * hidden ones)
	 * from another plot to this one. PlotObjects here refers to curves, arrows,
	 * labels etc.
	 ...
	 * Main Color, Secondary Color (or "none"), Line Width [, Symbol shape for
	 * XY_DATA] [,hidden]
	...
	/**
	 * Get the label the i-th PlotObject (in the sequence how they were added,
	 * including hidden ones).
	 * Returns null if no label. PlotObjects here refers to curves, arrows, labels
	 * etc.
	 * (not legend, exes and frame, though implemented as PlotObjects)
	 */
	...
	/**
	 * Set the label the i-th PlotObject (in the sequence how they were added,
	 * including hidden ones)
	 * PlotObjects here refers to curves, arrows, labels etc.
	 * (not legend, exes and frame, though implemented as PlotObjects)
	 */
	...
	/**
	 * Returns the index of the first plot object with x,y data (points, line) or
	 * arrows
	 * with all data equal to those given. Returns or -1 is no such plot object
	 * exists.
	 * The array 'values' should contain the x, y, x error bar, yerror bar data. The
	 * 'values' array may have any size;
	 * only the data given are compared (e.g. for an array with length 2, there is
	 * no check for erro bars).
	...
	/**
	 * Returns the index of the first plot object fitting the type mask and with all
	 * data equal to those given.
	...
	/**
	 * Creates a snapshot of the plot properties (formatting, range etc., not
	 * PlotObjects such as data and corresponding curves etc.),
	...
	/**
	 * Restores the plot properties (formatting, range etc., not PlotObjects such as
	 * data and corresponding curves etc.)
	 * from a snapshot previously created by savePlotPlotProperties. See also
	...
	/**
	 * Sets the plot range to the initial value determined from minima&maxima or
	 * given by setLimits.
	...
	public void setLimitsToDefaults(boolean updateImg) {
		...
		if (plotDrawn && updateImg)
			updateImage();
	}

	/**
	 * Sets the plot range to encompass all data. Updates the image if existing and
	 * updateImg is true.
	 */
	...
	public void setPreviousMinMax() {
		if (Double.isNaN(savedMinMax[0]))
			return; // no saved values yet
	...
	/**
	 * Draws the plot (if not done before) in an ImageProcessor and returns the
	 * ImageProcessor with the plot.
	 */
	 ...
	 * If an ImagePlus for this plot already exists, displays the plot in that
	 * ImagePlus and returns it.
	 */
	 ...
	 * in this case the return value is null (use getImagePlus().getWindow()
	 * instead).
	...
	public PlotWindow show() {
		...
		PlotWindow pw = new PlotWindow(this); // note: this may set imp to null if pw has listValues and autoClose are
												// set
	...
	/**
	 * Draws the plot specified for the first time. Does nothing if the plot has
	 * been drawn already.
	 * Call getProcessor to retrieve the ImageProcessor with it.
	 * Does no action with respect to the ImagePlus (if any)
	 */
	public void draw() {
		// IJLog.log("draw(); plotDrawn="+plotDrawn);
		if (plotDrawn)
			return;
		getInitialMinAndMax();
		pp.frame.setFont(nonNullFont(pp.frame.getFont(), currentFont)); // make sure we have a number font for
	...
	/**
	 * Freezes or unfreezes the plot. In the frozen state, the plot cannot be
	 * resized or updated,
	 * and the Plot class does no modifications to the ImageProcessor.
	 * Changes are recorded nevertheless and become effective with
	 * <code>setFrozen(false)</code>.
	 */
	public void setFrozen(boolean frozen) {
		...
		if (!pp.isFrozen) { // unfreeze operations ...
			...
			if (win != null)
				win.updateImage(imp); // show any changes made during the frozen state
		}
	}
	 ...
	 * This rectangle includes the black outline frame at the top and left, but not
	 * at the bottom
	 * and right (when the frame is plotted with 1 pxl width).
	 * The image scale is its width or height in pixels divided by the data range in
	 * x or y.
	 */
	...
	/**
	 * Creates a new high-resolution plot by scaling it and displays that plot if
	 * showIt is true.
	...
	public ImagePlus makeHighResolution(String title, float scale, boolean antialiasedText, boolean showIt) {
		Plot hiresPlot = null;
		try {
			hiresPlot = (Plot) clone(); // shallow clone, thus arrays&objects are not cloned, but they will be used only
										// now
		} catch (Exception e) {
			return null;
		}
	...
	/**
	 * Converts pixels to calibrated coordinates. In contrast to the image
	 * calibration, also
	...
	/**
	 * Converts calibrated coordinates to pixel coordinates. In contrast to the
	 * image calibration, also
	...
	public double scaleXtoPxl(double x) {
		if (xMin == xMax) {
			if (x == xMin)
				...
				return x > xMin ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY;
		}
		if (logXAxis)
	...
	public double scaleYtoPxl(double y) {
		if (yMin == yMax) {
			if (y == xMin)
				...
				return y > yMin ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY;
		}
		if (logYAxis)
	...
	private int scaleX(double x) {
		if (xMin == xMax) {
			if (x == xMin)
				...
				return x > xMin ? Integer.MAX_VALUE : Integer.MIN_VALUE;
		}
		if (logXAxis)
	...
	private int scaleY(double y) {
		if (yMin == yMax) {
			if (y == yMin)
				...
				return y > yMin ? Integer.MAX_VALUE : Integer.MIN_VALUE;
		}
		if (logYAxis)
	 ...
	 * Converts calibrated coordinates to pixel coordinates. In contrast to the
	 * image calibration, also
	 * works with log axes and inverted x axes. Returns a large number instead NaN
	 * for log x axis and zero or negative x
	 */
	 ...
	 * image calibration, also
	 * works with log axes and inverted x axes. Returns a large number instead NaN
	 * for log y axis and zero or negative y
	 */
	...
	/**
	 * Scales a value of the original plot for a high-resolution plot. Returns an
	 * integer number of pixels >=1
	 */
	int sc(float length) {
	...
	/**
	 * Whether a color is non-grayscale, which requires color (not grayscale) for
	 * the plot
	 */
	...
	/**
	 * Draws the plot contents (all PlotObjects and the frame and legend), without
	 * axes etc.
	 */
	void drawContents(ImageProcessor ip) {
		...
		// draw all the plot objects in the sequence they were added, except for the one
		// of the constructor
		for (PlotObject plotObject : allPlotObjects)
			if (!plotObject.hasFlag(PlotObject.CONSTRUCTOR_DATA)) {
				// properties lineWidth, Font, Color set for one object remain for the next
				...
				if (plotObject.getFont() != null)
					...
					plotObject.setFont(font);
				// IJLog.log("type="+plotObject.type+" color="+plotObject.color);
				drawPlotObject(plotObject, ip);
			}

		// draw the line passed with the constructor last, using the settings present
		// when calling 'draw'
		if (allPlotObjects.size() > 0 && allPlotObjects.get(0).hasFlag(PlotObject.CONSTRUCTOR_DATA)) {
			...
			if (currentLineWidth > 0)
				lineWidth = currentLineWidth; // linewidth when drawing determines frame linewidth
		}

		// finally draw the frame & legend
		if (!plotDrawn && pp.frame.lineWidth == DEFAULT_FRAME_LINE_WIDTH) { // when modifying PlotObjects styles later,
																			// don't change the frame line width any
																			// more
			pp.frame.lineWidth = lineWidth;
			if (pp.frame.lineWidth == 0)
				pp.frame.lineWidth = 1;
			if (pp.frame.lineWidth > 3)
				pp.frame.lineWidth = 3;
		}
		ip.setLineWidth(sc(pp.frame.lineWidth));
		ip.setColor(pp.frame.color);
		int x2 = frame.x + frame.width - 1;
		int y2 = frame.y + frame.height - 1;
		ip.moveTo(frame.x, frame.y); // draw the frame. Can't use ip.drawRect because it is inconsistent for
	...
	ImageProcessor getBlankProcessor() {
		makeMarginValues();
		// IJLog.log("Plot.getBlankPr preferredH="+preferredPlotHeight+"
		// pp.h="+pp.height);
		...
		if (ip == null || pp.width != ip.getWidth() || pp.height != ip.getHeight()
				|| (isColored != (ip instanceof ColorProcessor))) {
			if (isColored) {
	...
	/**
	 * Calculate the actual range, major step interval and set variables for data
	 * <-> pixels scaling
	 */
	double[] makeRangeGetSteps() {
		...
		for (int i = 0; i < currentMinMax.length; i += 2) { // for x and y direction
			...
			// don't zoom in too much (otherwise float conversion to int pixels may be
			// wrong)
			...
			if ((i == 0 && !simpleXAxis()) || (i == 2 && !simpleYAxis())) {
				...
				double step = Tools.getNumberFromList(pp.frame.options, i == 0 ? "xinterval=" : "yinterval="); // user-defined
																												// interval
				if (!Double.isNaN(step)) {
					int nSteps = (int) (Math.floor(currentMinMax[i + 1] / step + 1e-10)
							- Math.ceil(currentMinMax[i] / step - 1e-10));
					if (nSteps < 1)
						step = Double.NaN; // user-suppied interval too large, less than two numbers would be shown
					if ((i == 0 && nSteps * sc(minGridspacing) * 0.5 > frameSize)
							|| i != 0 && nSteps * sc(pp.frame.getFont().getSize()) > frameSize)
				...
				if (Double.isNaN(step)) { // automatic interval
					step = Math.abs((currentMinMax[i + 1] - currentMinMax[i]) *
							Math.max(1.0 / maxIntervals,
									(float) sc(minGridspacing) / frameSize + (maxIntervals > 12 ? 0.02 : 0.06))); // the
																													// smallest
																													// allowable
																													// step
				...
				if (force2grid) {
					...
					// if (!logXAxis || stepForSnap >= 0.999) { //don't snap on log axis if minor
					// ticks are not full decades
					// currentMinMax[i] = stepForSnap * Math.round(currentMinMax[i]/stepForSnap);
					// currentMinMax[i+1] = stepForSnap *
					// Math.round(currentMinMax[i+1]/stepForSnap);
					// }
				}
			}
			if (i == 0) {
		...
		if (!(xMax - xMin != 0.0)) // if range==0 (all data the same), or NaN shift zero level so one can see the
									// curve
			xBasePxl += sc(10);
		yScale = frameHeight / (yMax - yMin);
		if (!(yMax - yMin != 0.0))
			yBasePxl -= sc(10);
		// IJLog.log("x,yScale="+(float)xScale+","+(float)yScale+"
		// xMin,max="+(float)xMin+","+(float)xMax+"
		// yMin.max="+(float)yMin+","+(float)yMax);
	...
	/**
	 * Gets the initial plot limits (i.e., x&y ranges). For compatibility with
	 * previous versions of ImageJ,
	 * only the first PlotObject (with numeric data) is used to determine the
	 * limits.
	 */
	void getInitialMinAndMax() {
		int axisRangeFlags = 0;
		if (Double.isNaN(defaultMinMax[0]))
			axisRangeFlags |= X_RANGE;
		if (Double.isNaN(defaultMinMax[2]))
			axisRangeFlags |= Y_RANGE;
		if (axisRangeFlags != 0) {
	...
	/**
	 * Gets the minimum and maximum values from the first XY_DATA or ARROWS
	 * plotObject or all such plotObjects;
	 * axisRangeFlags determine for which axis to calculate the min&max (X_RANGE for
	 * x axis, Y_RANGE for y axis);
	 * for the other axes the limit is taken from defaultMinMax
	 * Array elements returned are xMin, xMax, yMin, yMax. Also sets enlargeRange to
	 * tell which limits should be enlarged
	...
	double[] getMinAndMax(boolean allObjects, int axisRangeFlags) {
		...
		double[] allMinMax = new double[] { xSign * Double.MAX_VALUE, -xSign * Double.MAX_VALUE,
				ySign * Double.MAX_VALUE, -ySign * Double.MAX_VALUE };
		...
		for (PlotObject plotObject : allPlotObjects) {
			if ((plotObject.type == PlotObject.XY_DATA || plotObject.type == PlotObject.ARROWS)
					&& !plotObject.hasFlag(PlotObject.HIDDEN)) {
		...
		if (allMinMax[0] == Double.MAX_VALUE && allMinMax[1] == -Double.MAX_VALUE) { // no x values at all? keep
																						// previous
			allMinMax[0] = defaultMinMax[0];
			allMinMax[1] = defaultMinMax[1];
		}
		if (allMinMax[2] == Double.MAX_VALUE && allMinMax[3] == -Double.MAX_VALUE) { // no y values at all? keep
																						// previous
	...
	/**
	 * Enlarges the current minimum and maximum ranges to include the data range of
	 * the last plotObject added,
	 ...
	 * axisRangeFlags determine for which axis (X_RANGE for x axis, Y_RANGE for y
	 * axis)
	 * The minimum modifies allMinAndMax[0] (x), allMinAndMax[2] (y); the maximum
	 * modifies [1], [3].
	 * If allMinAndMax values are modified, the corresponding enlargeRange array
		...
		if (plotObject.type == PlotObject.XY_DATA) {
			if ((axisRangeFlags & X_RANGE) != 0) {
				...
				if (!(plotObject.shape == LINE || plotObject.shape == FILLED) || plotObject.yEValues != null)
					suggestedEnlarge = ALWAYS_ENLARGE; // enlarge to make space at the obrders (we don't try to keep x=0
														// at the frame border)
			...
			if ((axisRangeFlags & Y_RANGE) != 0) {
				int suggestedEnlarge = 0;
				if (plotObject.shape == DOT || plotObject.xEValues != null) // these can't be seen if merging with the
																			// frame
	 ...
	 * The minimum modifies allMinAndMax[axisIndex] the maximum modifies
	 * allMinAndMax[axisIndex+1].
	 * Also cares about whether the range should be enlarged to avoid hiding markers
	 * at the borders:
	 * suggestedEnlarge is 0 for lines or a suggestion for the data type; if the
	 * allMinAndMax is
	...
	void getMinAndMax(double[] allMinAndMax, int[] enlargeRange, int suggestedEnlarge,
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (v1 < allMinAndMax[minIndex]) {
				...
				if (suggestedEnlarge == 0 && ((i > 0 && i < data.length - 1) || v2 != v1)) // for lines except at the
																							// end: also enlarge
		...
		// lines with many points (>10%) at min or max? Add extra space at borders
		// ('usually', i.e. unless limit is zero)
		...
		if (enlargeRange[maxIndex] == 0 && nMaxEqual > 2 && nMaxEqual * 10 > data.length)
			enlargeRange[maxIndex] = USUALLY_ENLARGE;
		// all data at min or max? Always add space to avoid hiding the line behind the
		// frame
		if (nMinEqual == data.length)
			enlargeRange[minIndex] = ALWAYS_ENLARGE;
		if (nMaxEqual == data.length)
			enlargeRange[maxIndex] = ALWAYS_ENLARGE;
		// same min or max as for current data set found already previously, but not
		// asking yet for added space at borders?
	 ...
	 * This only happens for limits where the corresponding value of the class array
	 * 'enlargeRange' is set:
	 * ALWAYS_ENLARGE: always enlarges the range; USUALLY_ENLARGE means that the
	 * limits should not be shifted across zero
	 */
	void enlargeRange(double[] minMax) {
		if (enlargeRange == null)
			return;
	...
	/**
	 * Returns the first font of the list that is not null, or defaultFont if both
	 * are null
	 */
	 ...
	 * numbers 10-13 for a dialog to set a single limit, and 14-15 for an axis
	 * options dialog.
	...
	void drawAxesTicksGridNumbers(double[] steps) {
		...
		Font scFontMedium = scFont.deriveFont(scFont.getSize2D() * 10f / 12f); // for axis numbers if full size does not
																				// fit
		...
		if (hasFlag(X_NUMBERS | (logXAxis ? (X_TICKS | X_MINOR_TICKS) : X_LOG_TICKS) + X_GRID)) {
			...
			int suggestedDigits = (int) Tools.getNumberFromList(pp.frame.options, "xdecimals="); // is not given, NaN
																									// cast to 0
			...
			if (xMin == xMax) {
				...
				if (minorTicks && (!logXAxis || step > 1.1)) { // 'standard' log minor ticks only for full decades
					...
					if (Math.abs(minorPerMajor - Math.round(minorPerMajor)) > 1e-10) // major steps are not an integer
																						// multiple of minor steps?
																						// (e.g. user step 90 deg)
						mstep = step / 4;
					if (logXAxis && mstep < 1)
						mstep = 1;
					...
					int minorNumberLimit = haveMinorLogNumbers
							? (int) (0.12 * Math.abs(xScale) / (fm.charWidth('0') + sc(2)))
							: 0; // more numbers on minor ticks when zoomed in
		...
		if (hasFlag(Y_NUMBERS | (logYAxis ? (Y_TICKS | Y_MINOR_TICKS) : Y_LOG_TICKS) + Y_GRID)) {
			...
			int suggestedDigits = (int) Tools.getNumberFromList(pp.frame.options, "ydecimals="); // is not given, NaN
																									// cast to 0
			...
			if (yMin == yMax) {
				...
				// IJLog.log(IJ.d2s(currentMinMax[2],digits)+": w="+w1+";
				// "+IJ.d2s(currentMinMax[3],digits)+": w="+w2+baseFont+"
				// Space="+(leftMargin-sc(4+5)-fm.getHeight()));
				for (int i = i1; i <= i2; i++) {
				...
				if (logYAxis && majorTicks && Math.abs(yScale) > sc(MIN_X_GRIDSPACING)) { // minor ticks for log within
																							// the decade
					int minorNumberLimit = haveMinorLogNumbers ? (int) (0.4 * Math.abs(yScale) / fm.getHeight()) : 0; // more
																														// numbers
																														// on
																														// minor
																														// ticks
	...
	/**
	 * Returns the array of categories from an axis label in the form
	 * {cat1,cat2,cat3}, or null if not this form
	 * 
	...
	/**
	 * Returns the smallest "nice" number >= v. "Nice" numbers are .. 0.5, 1, 2, 5,
	 * 10, 20 ...
	 */
	 ...
	 * 'Digits' should be >=0 for drawing the mantissa (=1.38 in this example),
	 * negative to draw only 10^exponent
	 * Currently only supports center justification and right justification (y of
	 * center line)
	...
	int drawExpString(double value, int digits, int x, int y, int justification,
		...
		if (Tools.parseDouble(s) == 0)
			s = "0"; // don't write 0 as 0*10^0
		...
		// IJLog.log(s+" -> "+base+"^"+exponent+" maxAsc="+fontAscent+"
		// font="+baseFont);
		ip.setJustification(RIGHT);
	...
	/**
	 * Returns the user-supplied (via setOptions) or default multiplication symbol
	 * (middot)
	 */
	...
	/**
	 * Returns the number of digits to display the number n with resolution
	 * 'resolution';
	 ...
	 * Returns 'suggestedDigits' if not 0 and compatible with the resolution;
	 * negative values of
	...
	static int getDigits2(double n, double resolution, int maxDigits, int suggestedDigits) {
		...
		// IJLog.log("n="+(float)n+"digitsRaw="+digits+" log10ofN="+log10ofN+"
		// sciDigits="+sciDigits);
		if ((digits < -2 && log10ofN >= maxDigits) || suggestedDigits < 0)
			digits = sciDigits; // scientific notation for large numbers or if desired via suggestedDigits
								// (plot.setOptions)
		else if (digits < 0)
			digits = 0;
	...
	private void drawPlotObject(PlotObject plotObject, ImageProcessor ip) {
		// IJLog.log("DRAWING type="+plotObject.type+"
		// lineWidth="+plotObject.lineWidth+" shape="+plotObject.shape);
		if (plotObject.hasFlag(PlotObject.HIDDEN))
			return;
		...
		switch (type) {
			case PlotObject.XY_DATA:
				...
				if (plotObject.yEValues != null) // error bars in front of bars and fill area below the line, but behind
				...
				if (plotObject.hasFilledMarker()) { // fill markers with secondary color
					...
					for (int i = 0; i < nPoints; i++)
						if ((!logXAxis || plotObject.xValues[i] > 0) && (!logYAxis || plotObject.yValues[i] > 0)
							...
							fillShape(plotObject.shape, scaleX(plotObject.xValues[i]), scaleY(plotObject.yValues[i]),
									markSize);
					ip.setColor(plotObject.color);
					ip.setLineWidth(sc(plotObject.lineWidth));
				}
				...
				if (plotObject.hasMarker()) { // draw the marker symbols
					...
					for (int i = 0; i < Math.min(plotObject.xValues.length, plotObject.yValues.length); i++) {
						if ((!logXAxis || plotObject.xValues[i] > 0) && (!logYAxis || plotObject.yValues[i] > 0)
								&& !Double.isNaN(plotObject.xValues[i]) && !Double.isNaN(plotObject.yValues[i]))
							drawShape(plotObject, scaleX(plotObject.xValues[i]), scaleY(plotObject.yValues[i]),
									plotObject.shape, markSize, i);
					}
			...
			case PlotObject.ARROWS:
				...
				for (int i = 0; i < plotObject.xValues.length; i++) {
					...
					if (xt1 == xt2 && yt1 == yt2)
						...
						if (arrowHeadLength > sc(MAX_ARROWHEAD_LENGTH))
							arrowHeadLength = sc(MAX_ARROWHEAD_LENGTH);
						if (arrowHeadLength < sc(MIN_ARROWHEAD_LENGTH))
							arrowHeadLength = sc(MIN_ARROWHEAD_LENGTH);
			...
			case PlotObject.LINE:
				ip.setClipRect(frame);
				ip.drawLine(scaleX(plotObject.x), scaleY(plotObject.y), scaleX(plotObject.xEnd),
						scaleY(plotObject.yEnd));
				ip.setClipRect(null);
				break;
			case PlotObject.NORMALIZED_LINE:
				ip.setClipRect(frame);
			...
			case PlotObject.DOTTED_LINE:
				...
				for (int i = 0; i <= n; i++)
					ip.drawDot(ix1 + (int) Math.round((ix2 - ix1) * (double) i / n),
							iy1 + (int) Math.round((iy2 - iy1) * (double) i / n));
				ip.setClipRect(null);
				break;
			case PlotObject.LABEL:
			case PlotObject.NORMALIZED_LABEL:
				...
				int xt = type == PlotObject.LABEL ? scaleX(plotObject.x)
						: leftMargin + (int) (plotObject.x * frameWidth);
				int yt = type == PlotObject.LABEL ? scaleY(plotObject.y)
						: topMargin + (int) (plotObject.y * frameHeight);
	...
	void drawBarChart(PlotObject plotObject) {
		...
		if (separatedBars && n > 1)
			halfBarWidthInPixels = Math.max(1, (int) Math.round(Math.abs(0.5
					* (plotObject.xValues[n - 1] - plotObject.xValues[0]) / (n - 1) * xScale * SEPARATED_BAR_WIDTH)));
		...
		for (int i = 0; i < n; i++) {
			...
			if (plotObject.color2 != null) {
				...
				for (int x2 = Math.min(left, right); x2 <= Math.max(left, right); x2++)
					ip.drawLine(x2, y0, x2, y); // cant use ip.fillRect (ignores the clipRect), so we it fill line by
												// line
			}
			ip.setColor(plotObject.color);
			ip.setLineWidth(sc(plotObject.lineWidth));
	...
	/**
	 * Draw the symbol for the data point number 'pointIndex' (pointIndex < 0 when
	 * drawing the legend)
	 */
	void drawShape(PlotObject plotObject, int x, int y, int shape, int size, int pointIndex) {
		...
		switch (shape) {
			...
			case BOX:
				...
				break;
			case TRIANGLE:
				ip.drawLine(x, ybase - sc(1), xend + sc(1), yend); // height must be odd, otherwise rounding leads to
																	// asymmetric shape
			...
			case CUSTOM:
				...
				break;
			default: // CIRCLE, CONNECTED_CIRCLES: 5x5 oval approximated by 5x5 square without
						// corners
				if (sc(size) < 5.01) {
	 ...
	 * Note that ip.fill, ip.fillOval etc. can't be used here: they do not care
	...
	/**
	 * Adds an arrow from position 1 to 2 given in pixels; 'size' is the length of
	 * the arrowhead
	 * 
	 * @deprecated Use as a public method is not supported any more because it is
	 *             incompatible with rescaling
	 */
	@Deprecated
	...
	private void drawVerticalErrorBars(float[] x, float[] y, float[] e) {
		...
		for (int i = 0; i < nPoints; i++) {
			if (Float.isNaN(x[i]) || Float.isNaN(y[i]) || (logXAxis && !(x[i] > 0)))
				continue;
			int x0 = scaleX(x[i]);
	...
	private void drawHorizontalErrorBars(float[] x, float[] y, float[] e) {
		...
		for (int i = 0; i < nPoints; i++) {
			if (Float.isNaN(x[i]) || Float.isNaN(y[i]) || (logXAxis && !(y[i] > 0)))
				continue;
			int y0 = scaleY(y[i]);
	...
	/**
	 * Fills space between polyline and y=0 with the current color (the secondary
	 * color of the plotObject)
	 */
	void drawFloatPolyLineFilled(ImageProcessor ip, float[] xF, float[] yF, int len) {
		...
		int y2 = scaleY(yF[0]);
		boolean isNaN1;
		boolean isNaN2 = Float.isNaN(xF[0]) || Float.isNaN(yF[0]) || (logXAxis && xF[0] <= 0)
				|| (logYAxis && yF[0] <= 0);
		for (int i = 1; i < len; i++) {
			isNaN1 = isNaN2;
			...
			if (left < frame.x && right < frame.x)
				continue; // ignore if all outside the plot area
			if (left >= frame.x + frame.width && right >= frame.x + frame.width)
				continue;
			if (left < frame.x)
				left = frame.x;
			if (left >= frame.x + frame.width)
				left = frame.x + frame.width - 1;
			if (right < frame.x)
				right = frame.x;
			if (right >= frame.x + frame.width)
			...
			if (left != right) {
				for (int xi = Math.min(left, right); xi <= Math.max(left, right); xi++) {
					 ...
					 * if (y < yMin) y = yMin; // dont extrapolate (in case rounding to pixels falls
					 * outside [xi, xi+1] interval)
					 * if (y > yMax) y = yMax;
					 */
	...
	/**
	 * Returns only indexed and sorted plot objects, if at least one label is
	 * indexed like "1__MyLabel"
	 */
	...
	void drawLegend(PlotObject legendObject, ImageProcessor ip) {
		...
		for (PlotObject plotObject : usedPlotObjects)
			if (plotObject.type == PlotObject.XY_DATA && !plotObject.hasFlag(PlotObject.HIDDEN)
					&& plotObject.label != null) { // label exists: was set now or previously
				nLabels++;
				String label = plotObject.label;
				if (indexedObjects != null)
		...
		for (PlotObject plotObject : usedPlotObjects)
			if (plotObject.type == PlotObject.XY_DATA && !plotObject.hasFlag(PlotObject.HIDDEN)
				...
				if (shape == SEPARATED_BAR)
					shape = BOX; // for bar plots, draw a square in the legend
				int yShiftLine = 0;
				if (shape == FILLED || shape == BAR && plotObject.color2 != null) // shift line up to make space for
																					// fill pattern
				...
				if (plotObject.shape == SEPARATED_BAR && markerSize < 0.6 * legendObject.getFontSize())
					markerSize = 2 * (int) (0.3 * legendObject.getFontSize()) + 1; // for 'separated bar', a larger box
																					// (an odd number, to have it
																					// centered)
				if (plotObject.hasFilledMarker() || (plotObject.shape == SEPARATED_BAR && plotObject.color2 != null)) {
					...
					ip.fillRect(xLine0, y - yShiftLine, 2 * (xMarker - xLine0) + 1,
							yShiftLine + (int) (0.3 * legendObject.getFontSize()));
				}
				...
				if (plotObject.hasCurve() || plotObject.shape == BAR) {
					Color c = plotObject.shape == CONNECTED_CIRCLES
							...
							: plotObject.color;
					ip.setColor(c);
					ip.fillRect(xLine0, y - lineWidth / 2 - yShiftLine, 2 * (xMarker - xLine0) + 1, lineWidth); // draw
																												// line
																												// as a
																												// rectangle
				}
	...
	Rectangle legendRect(int positionCode, int width, int height, int frameThickness) {
		...
		int x0 = (leftPosition) ? leftMargin + sc(2 * LEGEND_PADDING) + frameThickness / 2
				: leftMargin + frameWidth - width - sc(2 * LEGEND_PADDING) - frameThickness / 2;
		int y0 = (topPosition) ? topMargin + sc(LEGEND_PADDING) + frameThickness / 2
	...
	/**
	 * The position code of the legend position where the smallest amount of
	 * foreground pixels is covered
	 */
	...
	/**
	 * Returns the x, y coordinates at the cursor position or the nearest point as a
	 * String
	 */
	...
	/**
	 * Returns a reference to the PlotObject having the data passed with the
	 * constructor or (if that was null)
	...
	/**
	 * Returns a reference to the PlotObject with x & y data (points, curve) added
	 * last, or null if none.
	 */
	...
	/**
	 * returns whether there are several plots so that one cannot give a single y
	 ...
	 * When all columns are the same length, x columns equal to the first x column
	 * are
	 * not written, independent of writeFirstXColumn.
	 * Column headings are "X", "Y", "X1", "Y1", etc, irrespective of any labels of
	 * the data sets
	 */
	 ...
	 * are
	 * not written, independent of writeFirstXColumn.
	 * When the data sets have labels and useLabels is true, they are used for
	 * column headings,
		...
		for (PlotObject plotObject : allPlotObjects) {
			if (plotObject.type == PlotObject.XY_DATA) {
				boolean sameX = firstXYobject != null && Arrays.equals(firstXYobject.xValues, plotObject.xValues)
						&& allSameLength;
				boolean sameXY = sameX && Arrays.equals(firstXYobject.yValues, plotObject.yValues); // ignore duplicates
																									// (e.g. Markers
																									// plus Curve)
				boolean writeX = firstXYobject == null ? writeFirstXColumn : !sameX;
				addToLists(headings, data, plotObject, dataSetNumber, writeX, /* writeY= */!sameXY,
						/* multipleSets= */nDataSets > 1, useLabels);
				if (firstXYobject == null)
					firstXYobject = plotObject;
				dataSetNumber++;
			} else if (plotObject.type == PlotObject.ARROWS) {
				addToLists(headings, data, plotObject, arrowsNumber, /* writeX= */true, /* writeY= */true,
						/* multipleSets= */nDataSets > 1, /* useLabels= */false);
				arrowsNumber++;
			}
		}
		...
		if (writeX) {
			...
			if (plotObject.type != PlotObject.ARROWS) {
				...
				if (dataSetNumber == 0 && plotXLabel != null) { // use x axis label for 1st dataset if permitted
					...
					else if (plotXLabel.startsWith(" ") && plotXLabel.endsWith(" ")) // legacy: always use axis label
																						// for 1st data if spaces at
																						// start&end
		...
		if (writeY) {
			...
			if (plotObject.type != PlotObject.ARROWS) {
				...
				if (dataSetNumber == 0 && plotYLabel != null) {
					...
					else if (plotYLabel.startsWith(" ") && plotYLabel.endsWith(" ")) // legacy: always use axis label
																						// for 1st data if spaces at
																						// start&end
	...
	/**
	 * Convert a string to a label suitable for a ResultsTable without whitespace,
	 * quotes or commas,
	 * to avoid problems when saving and reading the table. Returns null if an empty
	 * string or null.
	 */
...
/**
 * This class contains the properties of the plot, such as size, format, range,
 * etc, except for the data+format (plot contents).
 * To enable reading serialized PlotObjects of plots created with previous
 * versions of ImageJ,
 * the variable names MUST NEVER be changed! Also any additions should be made
 * after careful thought,
 * since they have to be kept for all future versions.
 */
class PlotProperties implements Cloneable, Serializable {
	/**
	 * The serialVersionUID should not be modified, otherwise saved plots won't be
	 * readable any more
	 */
	static final long serialVersionUID = 1L;
	//
	PlotObject frame = new PlotObject(Plot.DEFAULT_FRAME_LINE_WIDTH); // the frame, including background color and axis
																		// numbering
	...
	/**
	 * Returns an array of all PlotObjects defined as PlotProperties. Note that some
	 * may be null
	 */
	...
	/**
	 * Returns the PlotObject for xLabel ('x'), yLabel('y'), frame ('f'; includes
	 * number font) or the legend ('l').
	 */
	...
	public PlotProperties deepClone() {
		...
		if (frame != null)
			pp2.frame = frame.deepClone();
		if (xLabel != null)
			pp2.xLabel = xLabel.deepClone();
		if (yLabel != null)
			pp2.yLabel = yLabel.deepClone();
		if (legend != null)
			pp2.legend = legend.deepClone();
		if (rangeMinMax != null)
			pp2.rangeMinMax = rangeMinMax.clone();
		return pp2;
	}
...
/**
 * This class contains the data and properties for displaying a curve, a set of
 * arrows, a line or a label in a plot,
 * as well as the legend, axis labels, and frame (including background and fonts
 * of axis numbering).
 * Note that all properties such as lineWidths and Fonts have to be scaled up
...
class PlotObject implements Cloneable, Serializable {
	...
	static final long serialVersionUID = 1L;
	/**
	 * Constants for the type of objects. These are powers of two so one can use
	 * them as masks
	 */
	...
	/**
	 * flag for the data set passed with the constructor. Note that 0 to 0x0f are
	 * reserved for fonts modifiers, 0x010-0x800 are reserved for legend modifiers
	 */
	public final static int CONSTRUCTOR_DATA = 0x1000;
	/** flag for hiding a PlotObject */
	public final static int HIDDEN = 0x2000;
	/**
	 * Type of the object; XY_DATA stands for curve or markers, can be also ARROWS
	 * ... SHAPES
	 */
	...
	/**
	 * The x and y data arrays and the error bars (if non-null). These arrays also
	 * serve as x0, y0, x1, y1
	 * arrays for plotting arrays of arrows
	 */
	public float[] xValues, yValues, xEValues, yEValues;
	/**
	 * For SHAPES: For boxplots with whiskers ('boxes'), elements of the ArrayList
	 * are float[6] with x and all 5 y values
	 * (for 'boxesx', y and all 5 x values), for 'rectangles', float[4] with x1, y1,
	 * x2, y2.
	 */
	public ArrayList shapeData;
	/**
	 * For SHAPES only, shape type & options. Currently implemented: 'boxes',
	 * 'boxesx' (box plots with whiskers), 'rectangles', 'redraw_grid'
	 */
	...
	public int shape;
	/**
	 * The line width in pixels for 'normal' plots (for high-resolution plots, to be
	...
	public Color color;
	/**
	 * The secondary color (for filling closed symbols and for the line of
	 * CIRCLES_AND_LINE, may be null for unfilled/default
	 */
	public Color color2;
	...
	public int step;
	/**
	 * A label for the y data of the curve, a text to draw, or the text of a legend
	 * (tab-delimited lines)
	 */
	public String label;
	...
	/**
	 * Text objects (labels, legend, axis labels) only: the font; maybe null for
	 * default. This is not serialized (transient)
	 */
	private transient Font font;
	/**
	 * String for representation of the font family (for Serialization); may be null
	 * for default. Font style is in flags, font size in fontSize.
	 */
	private String fontFamily;
	/**
	 * Font size (for Serialization), for 'normal' plots (for high-resolution plots,
	 * to be multiplied by a scale factor)
	 */
	...
	PlotObject(float[] xValues, float[] yValues, float[] yErrorBars, int shape, float lineWidth, Color color,
			Color color2, String yLabel) {
		this.type = XY_DATA;
	...
	/**
	 * Constructor for the legend. <code>flags</code> is bitwise or of TOP_LEFT etc.
	 * and LEGEND_TRANSPARENT if desired.
	...
	/**
	 * Constructor for the frame, including axis numbers. In the current version,
	 * the primary color (line color) is always black
	 */
	...
	boolean hasMarker() {
		return type == XY_DATA && (shape == Plot.CIRCLE || shape == Plot.X || shape == Plot.BOX
				|| shape == Plot.TRIANGLE
	...
	boolean hasFilledMarker() {
		return type == XY_DATA && color2 != null
				&& (shape == Plot.CIRCLE || shape == Plot.BOX || shape == Plot.TRIANGLE ||
	...
	Font getFont() {
		if (font == null && fontFamily != null) // after recovery from serialization, create the font from its
												// description
	...
	/**
	 * Returns all data xValues, yValues, xEValues, yEValues as a float[][] array.
	 * Note that future versions may have more data.
	 */
	 ...
	 * Note that colors & font are not cloned; it is assumed that these wil not be
	...
	public PlotObject deepClone() {
		...
		if (xValues != null)
			po2.xValues = xValues.clone();
		if (yValues != null)
			po2.yValues = yValues.clone();
		if (xEValues != null)
			po2.xEValues = xEValues.clone();
		if (yEValues != null)
			po2.yEValues = yEValues.clone();
		if (shapeData != null)
			po2.shapeData = cloneArrayList(shapeData);
		return po2;
	}
	...
	/**
	 * Converts old (pre-1.52i) type codes for the PlotObjects to the new ones,
	 * which can be used as masks
	 */
	void updateType() {
		type = 1 << type;
	}
	...
	public String toString() { // for debug messages
		String s = "PlotObject type=" + type + " flags=" + flags + " xV:" + (xValues == null ? "-" : yValues.length)
				+ " yV:" + (yValues == null ? "-" : yValues.length) + " label=" + label + " col=" + color + " fSize="
				+ fontSize + " ff=" + fontFamily;
		return s;
	}
} // class PlotObject
