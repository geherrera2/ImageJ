...
public class Toolbar extends Canvas implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
		...
		switch (tool) {
			...
			case OVAL:
				...
				if (ovalType == BRUSH_ROI) {
					yOffset = y - 1;
					polyline(6, 4, 8, 2, 12, 1, 15, 2, 16, 4, 15, 7, 12, 8, 9, 11, 9, 14, 6, 16, 2, 16, 0, 13, 1, 10, 4,
							9, 6, 7, 6, 4);
				} else if (ovalType == ELLIPSE_ROI) {
					xOffset = x - 1;
					yOffset = y + 1;
					polyline(11, 0, 13, 0, 14, 1, 15, 1, 16, 2, 17, 3, 17, 7, 12, 12, 11, 12, 10, 13, 8, 13, 7, 14, 4,
							14, 3, 13, 2, 13, 1, 12, 1, 11, 0, 10, 0, 9, 1, 8, 1, 7, 6, 2, 7, 2, 8, 1, 10, 1, 11, 0);
				} else
			...
			case FREEROI:
				xOffset = x;
				yOffset = y + 3;
				polyline(2, 0, 5, 0, 7, 3, 10, 3, 12, 0, 15, 0, 17, 2, 17, 5, 16, 8, 13, 10, 11, 11, 6, 11, 4, 10, 1, 8,
						0, 6, 0, 2, 2, 0);
				return;
			case LINE:
				if (arrowMode) {
			...
			case HAND:
				xOffset = x;
				yOffset = y;
				polyline(5, 17, 5, 16, 0, 11, 0, 8, 1, 8, 5, 11, 5, 2, 8, 2, 8, 8, 8, 0, 11, 0, 11, 8, 11, 1, 14, 1, 14,
	...
	private void showMessage(int tool) {
		...
		switch (tool) {
			...
			case HAND:
				...
				return;
			case DROPPER:
				String fg = foregroundColor.getRed() + "," + foregroundColor.getGreen() + ","
						+ foregroundColor.getBlue();
				String bg = backgroundColor.getRed() + "," + backgroundColor.getGreen() + ","
	...
	public void setTool(int tool) {
		if (tool == UNUSED) // "Unused" (blank) tool replaced with gap in 1.48h
			...
			tool++;
		if (IJ.debugMode)
			IJLog.log("Toolbar.setTool: " + tool);
	...
	public static void repaintTool(int tool) {
		...
		if (tb != null) {
			...
			if (IJ.debugMode)
				IJLog.log("Toolbar.repaintTool: " + tool + " " + g);
			if (g == null)
				return;
			if (dscale > 1.0)
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (item == rectItem || item == roundRectItem || item == rotatedRectItem) {
			...
			if (!(label.equals("Help...") || label.equals("Remove Custom Tools")) && !isTool && !label.endsWith("Tool")
					&& !label.endsWith("Tool "))
				currentSet = label;
			if (isTool) {
	...
	public int addTool(String toolTip) {
		...
		if (names[tool].endsWith(" Menu Tool"))
			installMenu(tool);
		if (IJ.debugMode)
			IJLog.log("Toolbar.addTool: " + tool + " " + toolTip);
		return tool;
	}
	...
	public void installStartupTools() {
		if (IJ.debugMode)
			IJLog.log("Toolbar.installStartupTools");
		for (int i = 0; i <= 6; i++) {
	...
	private boolean installBuiltinTool(String label) {
		if (IJ.debugMode)
			IJLog.log("Toolbar.installBuiltinTool: " + label);
		boolean ok = true;
		PlugInTool tool = null;
