...
public class PolygonRoi extends Roi {
	 ...
	 * Type must be Roi.POLYGON, Roi.FREEROI, Roi.TRACED_ROI, Roi.POLYLINE,
	 * Roi.FREELINE or Roi.ANGLE.
	 */
	 ...
	 * Type must be Roi.POLYGON, Roi.FREEROI, Roi.POLYLINE, Roi.FREELINE or
	 * Roi.ANGLE.
	 */
	...
	/**
	 * Creates a new polygon or polyline ROI from a Polygon. Type must be
	 * Roi.POLYGON,
	...
	/**
	 * Creates a new polygon or polyline ROI from a FloatPolygon. Type must be
	 * Roi.POLYGON,
	 * Roi.FREEROI, Roi.TRACED_ROI, Roi.POLYLINE, Roi.FREELINE or Roi.ANGLE.
	 */
		...
		do {
			...
			for (int p2 = p1 + 1; p2 < p3; p2++) {
				...
				if ((dx1 * dx1 + dy1 * dy1) > kk * (dx2 * dx2 + dy2 * dy2)) {
					if (xpf != null) {
						xpf[p2] = xpf[p3];
						ypf[p2] = ypf[p3]; // replace sharp vertex with end point
					} else {
						xp[p2] = xp[p3];
						yp[p2] = yp[p3];
					}
	...
	void finishPolygon() {
		if (xpf != null) {
			...
			if (useLineSubpixelConvention()) {
				width++;
				height++; // for open polylines & PointRois, include the pixels affected by 'draw'
			}
			bounds = null;
		}
		if (nPoints < 2 || (!(type == FREELINE || type == POLYLINE || type == ANGLE)
	...
	void updateClipRectAndDraw() {
		...
		int x2, y2;
		if (activeHandle > 0) {
			x2 = x + xp[activeHandle - 1];
			y2 = y + yp[activeHandle - 1];
		} else {
			x2 = x + xp[nPoints - 1];
			y2 = y + yp[nPoints - 1];
		}
		...
		if (y2 > ymax)
			ymax = y2;
		x2 = x + xp[activeHandle];
		y2 = y + yp[activeHandle];
		if (x2 < xmin)
			xmin = x2;
		if (y2 < ymin)
			ymin = y2;
	...
	protected void resetBoundingRect() {
		...
		// IJLog.log("reset: "+ymin+" "+before+" "+yp[0]);
		x += xmin;
		y += ymin;
		width = xmax - xmin;
		height = ymax - ymin;
	...
	private void resetSubPixelBoundingRect() {
		// IJLog.log("resetSubPixelBoundingRect: "+state+" "+bounds);
		if (xSpline != null) {
	...
	float[][] getSpline(int evaluationPoints, float[] xSpline, float[] ySpline) {
		...
		for (int i = 1; i < nPoints; i++) {
			...
			if (dLength < 0.001f)
				dLength = 0.001f; // avoid numerical problems with duplicate points
		...
		if (!isLine()) { // closed polygon: close the line
			...
			if (xpf.length < nNodes)
				enlargeArrays(nNodes);
			xpf[nNodes - 1] = xpf[0];
			ypf[nNodes - 1] = ypf[0];
		}
	...
	public void fitSplineForStraightening() {
		// create preliminary finer splines with half-pixel steps (these do not change
		// asynchronously)
	...
	/**
	 * Interpolates an open polgonial shape to get line segments of (roughly) length
	 * 'segmentLength',
	 * and returns arrays of x & y coordinates.
	 * Since the full length is not necessarily an integer multiple of
	 * 'segmentLength',
	 * the segmentLength actually used is returned as array element [2][0].
	 * If the ImagePlus is given, calibrated coordinates are used, based on its
	 * calibration,
	 * and 'segmentLength' should be in calibrated units.
	 * Returns null if an input array is null, or the number of points is 0
	 */
	static float[][] getEquidistantPoints(float[] xpoints, float[] ypoints, int npoints, double segmentLength,
			ImagePlus imp) {
		if (xpoints == null || xpoints == null || npoints <= 0)
			return null;
		if (xpoints.length < npoints)
			npoints = xpoints.length; // arguments might be inconsistent due to asynchronous modification
		...
		for (int i = 1; i < npoints; i++) {
			...
			while ((distanceOverNextWrite >= 0.0 || i == npoints - 1) && pointsWritten < npOut) { // we have to write a
																									// new point
				double fractionOverNextWrite = distanceOverNextWrite / distance;
				if (distance == 0)
					fractionOverNextWrite = 0;
				// IJLog.log("i="+i+" n="+pointsWritten+"/"+npOut+"
				// leng="+IJ.d2s(lengthRead)+"/"+IJ.d2s(length)+"
				// done="+IJ.d2s(pointsWritten*step)+" over="+IJ.d2s(fractionOverNextWrite)+"
				// x,y="+IJ.d2s(x2 - fractionOverNextWrite*dx)+","+IJ.d2s(y2 -
				// fractionOverNextWrite*dy));
	 ...
	 * In x, the offset is chosen slightly below 0.5 to reduce the impact of
	 * numerical
	 * errors.
	 */
	...
	/**
	 * Returns the perimeter (for ROIs) or length (for lines), using calibration if
	 * imp is not null
	 */
	...
	/**
	 * Returns the length of a polygon with integer coordinates. Uses no calibration
	 * if imp is null.
	 */
	...
	/**
	 * Returns the length of a polygon with float coordinates. Uses no calibration
	 * if imp is null.
	 */
	...
	void enlargeArrays(int newSize) {
		...
		yp2 = yp2temp;
		if (IJ.debugMode)
			IJLog.log("PolygonRoi: " + maxPoints + " points -> " + newSize);
		maxPoints = newSize;
	}
