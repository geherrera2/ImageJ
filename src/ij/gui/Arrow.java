...
public class Arrow extends Line {
	...
	public void draw(Graphics g) {
		...
		at.setTransform(mag, 0.0, 0.0, mag, (-xbase + 0.5) * mag, (-ybase + 0.5) * mag); // 0.5: int coordinate at pixel
																							// center
		if (outline) {
	...
	private Shape getPath() {
		...
		if (outline) {
			...
			if (headShaftRatio > 1.0)
				headShaftRatio = 1.0;
			// IJLog.log(headShaftRatio+" "+(int)shaftLength+" "+(int)headLength+"
			// "+(int)tailx+" "+(int)taily+" "+(int)headtipx+" "+(int)headtipy);
		}
	...
	private double getOutlineWidth() {
		...
		// if (width<1) width=1;
		// if (head<1) head=1;
		// IJLog.log(getStrokeWidth()+" "+IJ.d2s(width,2)+" "+IJ.d2s(head,2)+"
		// "+IJ.d2s(headShaftRatio,2)+" "+IJ.d2s(lineWidth,2)+" "+IJ.d2s(width*head,2));
		return lineWidth;
	}
	...
	/*
	 * Set the style, where 'style' is "filled", "notched", "open", "headless" or
	 * "bar",
	 * plus optionial modifiers of "outline", "double", "small", "medium" and
	 * "large".
	 */
