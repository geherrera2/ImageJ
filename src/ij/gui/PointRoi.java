 ...
 * 
 * @see <a href=
 *      "http://wsr.imagej.net/macros/js/PointProperties.js">PointProperties.js</a>
 */
public class PointRoi extends PolygonRoi {
	...
	/**
	 * Creates a new PointRoi using the specified int arrays of offscreen
	 * coordinates.
	 */
	...
	/**
	 * Creates a new PointRoi using the specified float arrays of offscreen
	 * coordinates.
	 */
	...
	public void setOptions(String options) {
		if (options == null)
			return;
		if (options.contains("tiny"))
			size = TINY;
		else if (options.contains("medium"))
			size = MEDIUM;
		else if (options.contains("extra"))
			size = EXTRA_LARGE;
		else if (options.contains("large"))
			size = LARGE;
		else if (options.contains("xxxl"))
			size = XXXL;
		else if (options.contains("xxl"))
			size = XXL;
		if (options.contains("cross"))
			type = CROSS;
		else if (options.contains("dot"))
			type = DOT;
		else if (options.contains("circle"))
			type = CIRCLE;
		if (options.contains("nolabel"))
			setShowLabels(false);
		else if (options.contains("label"))
			setShowLabels(true);
	...
	public void draw(Graphics g) {
		...
		if (showLabels && nPoints > 1) {
			...
			// IJLog.log("fontSize: "+fontSize+" "+scale);
		...
		if (ic != null && overlay && ic.getShowAllList() != null && ic.getShowAllList().contains(this)
				&& !Prefs.showAllSliceOnly)
			slice = 0; // draw point irrespective of currently selected slice
		if (Prefs.showAllPoints)
			slice = 0;
		// IJLog.log("draw: "+positions+" "+imp.getCurrentSlice());
		for (int i = 0; i < nPoints; i++) {
			// IJLog.log(i+" "+slice+" "+(positions!=null?positions[i]:-1)+"
			// "+getPosition());
	...
	private synchronized void incrementCounter(ImagePlus imp) {
		// IJLog.log("incrementCounter: "+nPoints+" "+counter+"
		...
		if (counter != 0 || isStack || counters != null) {
			...
			// if (positions[nPoints-1]==0 || positions[nPoints-1]==1 ||
			// counters[nPoints-1]==0)
			// IJLog.log("incrementCounter: "+nPoints+" "+" "+positions[nPoints-1]+"
			// "+counters[nPoints-1]+" "+imp);
	...
	/**
	 * Returns the points of this Roi that are not contained in the specified area
	 * ROI.
	...
	/**
	 * Returns the points contained (not contained) in <code>roi</code> if
	 * <code>keepContained</code> is true (false).
	 */
	...
	public boolean contains(int x, int y) {
		for (int i = 0; i < nPoints; i++) {
			if (x == this.x + xpf[i] && y == this.y + ypf[i])
				return true;
		}
		return false;
	}
	...
	public void setCounters(int[] counters) {
		if (counters != null) {
			...
			for (int i = 0; i < n; i++) {
				...
				// IJLog.log(i+" cnt="+counter+" slice="+position);
	...
	/**
	 * Returns the stack slice of the point with the given index, or 0 if no slice
	...
	public int isHandle(int sx, int sy) {
		...
		int handle = -1;
		int sx2, sy2;
		int slice = !Prefs.showAllPoints && positions != null && imp != null && imp.getStackSize() > 1
				? imp.getCurrentSlice()
				: 0;
		for (int i = 0; i < nPoints; i++) {
