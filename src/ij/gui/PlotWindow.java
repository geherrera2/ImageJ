...
public class PlotWindow extends ImageWindow implements ActionListener, ItemListener,
	...
	/**
	 * Interpolate line profiles. To set, use Edit/Options/Plots or
	...
	private Roi[] rangeArrowRois; // the overlays (arrows etc) for changing the range. Note: #10-15 must
									// correspond to PlotDialog.dialogType!
	 ...
	 * Window layout is finished at latest a few millisec after windowActivated,
	 * then the
	 * 'wasActivated' boolean is set to tell the ImageCanvas that resize events
	 * should
	 * lead to resizing the canvas (before, creating the layout can lead to resize
	 * events)
	 */
	...
	/**
	 * Maximizes the width for the coordinate&status readout field and its parent
	 * bottomPanel
	 */
	 ...
	 * This text may get temporarily replaced for 'tooltips' (mouse over range
	 * arrows etc.).
	...
	/**
	 * Mouse wheel: zooms when shift or ctrl is pressed, scrolls in x if space bar
	 * down, in y otherwise.
	 */
	...
	/**
	 * Creates an overlay with triangular buttons and othr symbols for changing the
	 * axis range
	 * limits and shows it
	 */
	void showRangeArrows() {
		...
		TextRoi txtRoi = new TextRoi(1, height - 19, "\u2009R\u2009", theFont); // thin spaces to make roi slightly
																				// wider
	 ...
	 * numbers 10-13 for a dialog to set a single limit, and 14-15 for the axis
	 * options.
	 */
	...
	/**
	 * Returns the plot values with simple headings (X, Y, Y1 etc, not the labels)
	 * as a ResultsTable.
	 * Use plot.getResultsTableWithLabels for a table with data set labels as column
	 * headings
	 */
	 ...
	 * This requires that the PlotWindow has been initialized with a Plot having a
	 * PlotMaker
	 */
	private void enableLivePlot() {
		...
		if (IJ.debugMode)
			IJLog.log("PlotWindow.createListeners");
	...
	private void disableLivePlot() {
		if (IJ.debugMode)
			IJLog.log("PlotWindow.disableLivePlot: " + srcImp);
		if (srcImp == null)
			return;
		if (bgThread != null)
			bgThread.interrupt();
	...
	public synchronized void roiModified(ImagePlus img, int id) {
		if (IJ.debugMode)
			IJLog.log("PlotWindow.roiModified: " + img + "  " + id);
		if (img == srcImp) {
			doUpdate = true;
			notify();
		}
	...
	public void run() {
		while (true) {
			...
			if (doUpdate && plot != null && plot.getNumPlotObjects() > 0) {
				plot.useTemplate(this.plot,
						this.plot.templateFlags | Plot.COPY_SIZE | Plot.COPY_LABELS | Plot.COPY_AXIS_STYLE |
	...
	/**
	 * Freezes the active plot window, so the image does not get redrawn for
	 * zooming,
