...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener,
	...
	public void showHistogram(ImagePlus srcImp, ImageStatistics stats) {
		...
		if (limitToThreshold && histogram.length == 256) {
			...
			if (ip.getMinThreshold() != ImageProcessor.NO_THRESHOLD) {
				int lower = DrawHistogram.scaleDown(ip, ip.getMinThreshold());
				int upper = DrawHistogram.scaleDown(ip, ip.getMaxThreshold());
		...
		DrawHistogram.drawHistogram(srcImp, ip, fixedRange, stats.histMin, stats.histMax);
		imp.updateAndDraw();
	}
	...
	public void mouseMoved(int x, int y) {
		...
		if ((frame != null) && x >= frame.x && x <= (frame.x + frame.width)) {
			...
			DrawHistogram.drawValueAndCount(ip, value, histogram[index]);
		} else
			DrawHistogram.drawValueAndCount(ip, Double.NaN, -1);
	...
	void replot() {
		...
		ip.setLineWidth(1);
		if (logScale) {
			DrawHistogram.drawLogPlot(yMax > 0 ? yMax : newMaxCount, ip);
			DrawHistogram.drawPlot(yMax > 0 ? yMax : newMaxCount, ip);
		} else
			DrawHistogram.drawPlot(yMax > 0 ? yMax : newMaxCount, ip);
		this.imp.updateAndDraw();
	}
