...
public class Overlay implements Iterable<Roi> {
	...
	public Overlay rotate(double angle, double xcenter, double ycenter) {
		// IJLog.log("rotate: "+angle+" "+xcenter+" "+ycenter);
		Overlay overlay2 = create();
		for (int i = 0; i < size(); i++) {
			Roi roi = get(i);
			int position = roi.getPosition();
	...
	/**
	 * Set 'false' to prevent ROIs in this overlay from being dragged by their
	 * labels.
	 */
	...
	public String toString() {
		return "Overlay[size=" + size() + " " + (scalableLabels ? "scale" : "") + " "
				+ Colors.colorToString(getLabelColor()) + "]";
	}
