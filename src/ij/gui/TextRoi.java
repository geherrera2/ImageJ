...
public class TextRoi extends Roi {
	...
	/**
	 * Use this constructor as a drop-in replacement for
	...
	public void draw(Graphics g) {
		if (IJ.debugMode)
			IJLog.log("draw: " + theText[0] + "  " + this.width + "," + this.height);
	...
	/**
	 * Returns the state of the global 'antialiasedText' variable, which is used by
	 * the "Fonts" widget.
	 */
	...
	/**
	 * Returns the value of the 'justification' instance variable (LEFT, CENTER or
	 * RIGHT).
	 */
	...
	public String getMacroCode(String cmd, ImagePlus imp) {
		...
		if (addSelection) {
			code += "Overlay.drawString(\"" + text() + "\", " + this.x + ", " + (this.y + fontHeight) + ", "
					+ getAngle() + ");\n";
			code += "Overlay.show();\n";
		} else {
	...
	private String getColorArgs(Color c) {
		return IJ.d2s(c.getRed() / 255.0, 2) + ", " + IJ.d2s(c.getGreen() / 255.0, 2) + ", "
				+ IJ.d2s(c.getBlue() / 255.0, 2);
	}
