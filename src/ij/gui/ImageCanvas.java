...
public class ImageCanvas extends Canvas implements MouseListener, MouseMotionListener, Cloneable {
	...
	public void setSourceRect(Rectangle r) {
		...
		if (IJ.debugMode)
			IJLog.log("setSourceRect: " + magnification + " " + (int) (srcRect.height * magnification + 0.5) + " "
					+ dstHeight + " " + srcRect);
	}
	...
	// public void repaint() {
	// super.repaint();
	// //if (IJ.debugMode) IJLog.log("repaint: "+imp);
	// }

	public void paint(Graphics g) {
		// if (IJ.debugMode) IJLog.log("paint: "+imp);
		painted = true;
		Roi roi = imp.getRoi();
		...
		try {
			...
			if (img != null)
				g.drawImage(img, 0, 0, (int) (srcRect.width * magnification + 0.5),
						(int) (srcRect.height * magnification + 0.5),
	...
	private void drawOverlay(Overlay overlay, Graphics g) {
		...
		int n = overlay.size();
		// if (IJ.debugMode) IJLog.log("drawOverlay: "+n);
		...
		for (int i = 0; i < n; i++) {
			...
			if (hyperstack) {
				...
				// IJLog.log(c+" "+z+" "+t+" "+position+" "+roiManagerShowAllMode);
				if (position > 0) {
				...
				if (position > 0 && imp.getCompositeMode() == IJ.COMPOSITE)
					position = 0;
				// IJLog.log(position+" "+currentImage+" "+roiManagerShowAllMode+" "+c+" "+z+"
				// "+t);
	...
	void drawRoiLabel(Graphics g, int index, Roi roi) {
		...
		// IJLog.log("drawRoiLabel: "+" "+label+" "+x+" "+y+" "+flattening);
	...
	void resizeCanvas(int width, int height) {
		ImageWindow win = imp.getWindow();
		// IJLog.log("resizeCanvas: "+srcRect+" "+imageWidth+" "+imageHeight+" "+width+"
		// "+height+" "+dstWidth+" "+dstHeight+" "+win.maxBounds);
		if (!maxBoundsReset && (width > dstWidth || height > dstHeight) && win != null && win.maxBounds != null
			...
			if (resetMaxBoundsCount != 0)
				resetMaxBounds(); // Works around problem that prevented window from being larger than maximized
									// size
			resetMaxBoundsCount++;
		}
		...
		// IJLog.log("resizeCanvas2: "+srcRect+" "+dstWidth+" "+dstHeight+" "+width+"
		// "+height);
	}
	...
	public void fitToWindow() {
		...
		double ymag = (double) (bounds.height - (ImageWindow.VGAP * 2 + insets.top + insets.bottom + sliderHeight))
				/ srcRect.height;
	...
	void adjustSourceRect(double newMag, int x, int y) {
		// IJLog.log("adjustSourceRect1: "+newMag+" "+dstWidth+" "+dstHeight);
		...
		srcRect = r;
		setMagnification(newMag);
		// IJLog.log("adjustSourceRect2: "+srcRect+" "+dstWidth+" "+dstHeight);
	}

	/**
	 * Returns the size to which the window can be enlarged, or null if it can't be
	 * enlarged.
	 * <code>newWidth, newHeight</code> is the size needed for showing the full
	 * image
	...
	public void zoomOut(int sx, int sy) {
		...
		if (Math.abs(srcRatio - imageRatio) > 0.05) {
			...
			// IJLog.log(newMag+" "+srcRect+" "+dstWidth+" "+dstHeight);
			int newDstWidth = (int) (srcRect.width * newMag);
			int newDstHeight = (int) (srcRect.height * newMag);
			setMagnification(newMag);
			setMaxBounds();
			// IJLog.log(newDstWidth+" "+dstWidth+" "+newDstHeight+" "+dstHeight);
	...
	protected void scroll(int sx, int sy) {
		...
		// IJLog.log("scroll: "+ox+" "+oy+" "+xMouseStart+" "+yMouseStart);
		...
		srcRect.x = newx;
		srcRect.y = newy;
		// IJLog.log(sx+" "+sy+" "+newx+" "+newy+" "+srcRect);
		imp.draw();
		Thread.yield();
	}
	...
	public void setDrawingColor(int ox, int oy, boolean setBackground) {
		// IJLog.log("setDrawingColor: "+setBackground+this);
			...
			case Toolbar.WAND:
				...
				if (Recorder.record && npoints > 0) {
					if (Recorder.scriptMode())
						Recorder.recordCall(
								"IJ.doWand(imp, " + ox + ", " + oy + ", " + tolerance + ", \"" + mode + "\");");
					else {
		...
		if (longClickDelay > 0) {
			...
			pressTimer.schedule(new TimerTask() {
				public void run() {
					...
					// IJLog.log(size2+" "+empty+" "+unchanged+" "+state+" "+roi1+" "+roi2);
	...
	protected void handlePopupMenu(MouseEvent e) {
		if (disablePopupMenu)
			return;
		if (IJ.debugMode)
			IJLog.log("show popup: " + (e.isPopupTrigger() ? "true" : "false"));
		int sx = e.getX();
		int sy = e.getY();
	...
	void setRoiModState(MouseEvent e, Roi roi, int handle) {
		...
		if (e.isShiftDown())
			...
			roi.modState = Roi.NO_MODS;
		// IJLog.log("setRoiModState: "+roi.modState+" "+ roi.state);
	}
	...
	private boolean activateOverlayRoi(int ox, int oy) {
		...
		for (int i = o.size() - 1; i >= 0; i--) {
			...
			if (roi == null)
				continue;
			// IJLog.log(".isAltDown: "+roi.contains(ox, oy));
