 ...
 * to spaces when the dialog is displayed. For example, change the checkbox
 * labels
...
public class GenericDialog extends Dialog implements ActionListener, TextListener,
	...
	public void addNumericField(String label, double defaultValue, int digits, int columns, String units) {
		...
		c.gridwidth = 1;
		// IJLog.log("x="+c.gridx+", y= "+c.gridy+", width="+c.gridwidth+", ancher=
		// "+c.anchor+" "+c.insets);
		add(fieldLabel, c);
		if (addToSameRow) {
			c.insets.left = 0;
	...
	private void addSlider(String label, double minValue, double maxValue, double defaultValue, double scale,
		...
		if (columns < 1)
			columns = 1;
		// IJLog.log("scale=" + scale + ", columns=" + columns + ", digits=" + digits);
	 ...
	 * Note that addMessage (and addStringField, if its column width is more than 8)
	 * use
	 ...
	 * dialog.
	 * For other listeners, the OK button will not cause a call to
	 * dialogItemChanged;
	...
	public String getNextString() {
		...
		if (macro) {
			theText = Macro.getValue(macroOptions, label, theText);
			if (theText != null && (theText.startsWith("&") || numberExpected
					|| label.toLowerCase(Locale.US).startsWith(theText))) {
				// Is the value a macro variable?
				if (theText.startsWith("&"))
					theText = theText.substring(1);
				Interpreter interp = Interpreter.getInstance();
