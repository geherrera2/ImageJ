...
/**
 * This class implements the Plot Window's Data>"Add from Plot", "Add form
 * Table", "Add Fit" and
 * "More>Contents Style" dialogs
 */
public class PlotContentsDialog implements DialogListener {
	/**
	 * Types of dialog (ERROR suppresses the dialog after an invalid call of a
	 * constructor)
	 */
	...
	private static final String[] HEADINGS = new String[] { "Plot Contents Style", "Add From Plot", "Plot From Table",
			"Add Plot Data", "Add Fit" };
	...
	private final static boolean[] COLUMN_ALLOW_NONE = new boolean[] { true, false, true, true }; // y data cannot be
																									// null
	...
	private static final String[] PLOT_COLORS = new String[] { "blue", "red", "black", "#00c0ff", "#00e000", "gray",
			"#c08060", "magenta" };

	/**
	 * Prepares a new PlotContentsDialog for an existing plot. Use showDialog
	 * thereafter.
	 * 
	 * @param dialogType may be STYLE (contents style), ADD_FROM_PLOT (add object
	 *                   from other plot), ADD_FROM_TABLE, and ADD_FIT
	 */
	public PlotContentsDialog(Plot plot, int dialogType) {
		this.plot = plot;
		this.dialogType = plot == null ? ERROR : dialogType;
		if (plot != null)
			currentPlotNumObjects = plot.getNumPlotObjects();
	}

	/**
	 * Prepares a new PlotContentsDialog for creating a new plot using data from a
	 * ResultsTable.
	...
	/**
	 * Prepares a new PlotContentsDialog for creating a new plot using float[]
	 * arrays as data.
	 * Each 'data' array in the ArrayList must have a corresponding element in the
	 * 'headings' array.
	 * 'defaultHeadings' may contain the headings of the items selected initially
	 * for x, y, x error and y error, respectively.
	 * 'defaultHeadings' and each of its entries may be null, and the array may have
	 * any length.
	 */
	...
	/**
	 * Prepares a new PlotContentsDialog for adding data from float[] arrays to a
	 * plot.
	 * Each 'data' array in the ArrayList must have a corresponding element in the
	 * 'headings' array.
	 * 'defaultHeadings' may contain the headings of the items selected initially
	...
	/**
	 * Avoids showing a selection for the error bars; must be called before
	 * showDialog.
	 */
	...
	public void showDialog(Frame parent) {
		...
		if (dialogType == STYLE && designations.length == 0) {
			...
			if (allPlots.length == 0)
				return; // should never happen; we have at least the current plot
		} else if (dialogType == ADD_FROM_TABLE && !creatingPlot) {
			prepareAddFromTable();
			if (allTables.length == 0)
				return; // should never happen; PlotWindow should not enable if no table
		}
		...
		gd = parent == null ? new GenericDialog(HEADINGS[dialogType]) : new GenericDialog(HEADINGS[dialogType], parent);
		...
		if (dialogType == STYLE) {
			...
			if (creatingPlot)
				tableChoice.setEnabled(false); // we can't select the table, we have only one
		...
		if (dialogType == ADD_FIT) {
			...
			IJLog.log(curveFitterStatusString);
		}
		if (Recorder.record && !Recorder.scriptMode()) {
			if (dialogType == ADD_FROM_PLOT) {
				Recorder.recordString("Plot.addFromPlot(\"" + plotChoice.getSelectedItem() + "\", "
						+ objectChoice.getSelectedIndex() + ");\n");
			} else if (dialogType == ADD_FROM_TABLE) {
				if (creatingPlot)
					Recorder.recordString("Plot.create(\"" + plot.getTitle() + "\", \"" + plot.getLabel('x') + "\", \""
							+ plot.getLabel('y') + "\");\n");
				String xSource = columnChoice[0].getSelectedIndex() == 0 ? ""
						: "Table.getColumn(\"" + columnChoice[0].getSelectedItem() + "\", \""
								+ tableChoice.getSelectedItem() + "\"), ";
				String ySource = "Table.getColumn(\"" + columnChoice[1].getSelectedItem() + "\", \""
						+ tableChoice.getSelectedItem() + "\")";
				Recorder.recordString(
						"Plot.add(\"" + symbolChoice.getSelectedItem() + "\", " + xSource + ySource + ");\n");
	...
	private String getStyleString() {
		...
		String style = color.trim() + "," + color2.trim() + "," + (float) width + "," + symbol
				+ (visible ? "" : "hidden");
		return style;
	}

	/**
	 * Sets the style fields of the dialog according to the style of the PlotObject
	 * having the
	...
	/**
	 * Set up the Choice of Plot objects for the source plot after selecting that
	 * plot
	 */
	private void makeSourcePlotObjects() {
		...
		int nPlotObjects = allPlots[plotIndex] == plot ? currentPlotNumObjects : plotObjectNames.length; // for the own
																											// plot, the
																											// number of
																											// objects
																											// may have
	...
	/**
	 * For "Add from Plot", adds item to the plot according to the current Choice
	 * settings
	...
	private void addObjectFromPlot() {
		...
		currentObjectIndex = plot.addObjectFromPlot(allPlots[plotIndex], objectIndex); // no updateImage; will be done
																						// later
	...
	/**
	 * Prepare the lists 'allTables', "allTableNames' for the "Add from Table"
	 * dialog.
	...
	/**
	 * Returns whether there is at least one table that can be used for "Add from
	 * Table"
	 */
	...
	/**
	 * Set up the Choices for the source columns for "Add from Table" and "Add from
	 * Arrays"
	 */
	...
	/**
	 * For "Add from Table" and "Add from Arrays" adds item to the plot according to
	...
	private void addObjectFromTable() {
		...
		if (data[1] == null)
			return; // no y data? then can't plot
		...
		if (creatingPlot) {
			plot.setXYLabels(data[0] == null ? "x" : columnChoice[0].getSelectedItem(),
					columnChoice[1].getSelectedItem());
	...
	/**
	 * For "Add from Table" and "Add from Arrays", tries to set a 'y' data column
	 * that has not been plotted yet.
	 */
	...
	/**
	 * For "Add from Table" and "Add from Arrays", retrieves the data arrays
	 * according to the columnChoices
	 */
	...
	private void addFitCurve() {
		...
		curveFitterStatusString = "Fit for " + plot.getTitle() + ": " + fitDataChoice.getSelectedItem()
				+ cf.getResultString(); // will be shown in Log when done
	...
	/**
	 * Returns whether the table is non-null and has at least one non-trivial data
	 * column
	 */
	...
	private static int getIndex(String[] strArray, String searchTerm) {
		for (int i = 0; i < strArray.length; i++)
			if (strArray[i].equals(searchTerm))
				return i;
		return -1;
	}
	...
	private void suggestColor() {
		...
		for (int i = 0; i < designations.length; i++) {
			if (!(designations[i].toLowerCase().startsWith("data")))
				continue;
		...
		if (previousColor2 != null && previousColor2.equals(previousColor)) // if fill color was main color, keep it
																			// such
			previousColor2 = newColor;
		else if (Colors.decode(previousColor2, null) != null) { // if we had a separate fill color, make main color
																// brighter
			Color newC = Colors.decode(newColor, Color.BLACK);
			Color newC2 = new Color(makeBrighter(newC.getRed()), makeBrighter(newC.getGreen()),
					makeBrighter(newC.getBlue()));
