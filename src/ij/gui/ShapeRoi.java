...
/**
 * A subclass of <code>ij.gui.Roi</code> (2D Regions Of Interest) implemented in
 * terms of java.awt.Shape.
 * A ShapeRoi is constructed from a <code>ij.gui.Roi</code> object, or as a
 * result of logical operators
 * (i.e., union, intersection, exclusive or, and subtraction) provided by this
 * class. These operators use the package
...
public class ShapeRoi extends Roi {
	...
	/**
	 * The maximum tolerance allowed in calculating the length of the curve segments
	 * of this ROI's shape.
	 */
	static final double MAXERROR = 1.0e-3;

	/**
	 * Coefficient used to obtain a flattened version of this ROI's shape. A
	 * flattened shape is the
	 * closest approximation of the original shape's curve segments with line
	 * segments.
	 * The FLATNESS is an indication of the maximum deviation between the flattened
	 * and the original shape.
	 */
	static final double FLATNESS = 0.1;

	/**
	 * Flatness used for filling the shape when creating a mask. Lower values result
	 * in higher accuracy
	 * (determining which pixels near the border are filled), but lower speed when
	 * filling shapes with
	 * curved borders.
	 */
	...
	/**
	 * Parsing a shape composed of linear segments less than this value will result
	 * in Roi objects of type
	 * {@link ij.gui.Roi#POLYLINE} and {@link ij.gui.Roi#POLYGON} for open and
	 * closed shapes, respectively.
	 * Conversion of shapes open and closed with more than MAXPOLY line segments
	 * will result,
	 * respectively, in {@link ij.gui.Roi#FREELINE} and {@link ij.gui.Roi#FREEROI}
	 * (or
	 * {@link ij.gui.Roi#TRACED_ROI} if {@link #forceTrace} flag is
	...
	/**
	 * The instance value of the maximum tolerance (MAXERROR) allowed in calculating
	 * the
	...
	/**
	 * If <strong></code>true</code></strong> then methods that manipulate this
	 * ROI's shape will work on
	 * a flattened version of the shape.
	 */
	private boolean flatten;

	/**
	 * Flag which specifies how Roi objects will be constructed from closed
	 * (sub)paths having more than
	...
	/**
	 * Flag which specifies if Roi objects constructed from open (sub)paths composed
	 * of only two line segments
	 * will be of type {@link ij.gui.Roi#ANGLE}.
	 * If <strong><code>true</code></strong> then (sub)path will be parsed into a
	 * {@link ij.gui.Roi#ANGLE};
	 ...
	 * @param flatness   The flatness factor used in convertion of curve segments
	 *                   into line segments.
	 * @param maxerror   Error correction for calculating length of Bezeir curves.
	 * @param forceAngle flag used in the conversion of Shape objects to Roi objects
	 *                   (see {@link #shapeToRois()}.
	 * @param forceTrace flag for conversion of Shape objects to Roi objects (see
	 *                   {@link #shapeToRois()}.
	 * @param flatten    if <strong><code>true</code></strong> then the shape of
	 *                   this ROI will be flattened
	 *                   (i.e., curve segments will be aproximated by line
	 *                   segments).
	 * @param maxPoly    Roi objects constructed from shapes composed of linear
	 *                   segments fewer than this
	 *                   value will be of type {@link ij.gui.Roi#POLYLINE} or
	 *                   {@link ij.gui.Roi#POLYGON}; conversion of
	 *                   shapes with linear segments more than this value will
	 *                   result in Roi objects of type
	 *                   {@link ij.gui.Roi#FREELINE} or {@link ij.gui.Roi#FREEROI}
	 *                   unless the average side length
	 *                   is large (see {@link #shapeToRois()}).
	 */
	ShapeRoi(Roi r, double flatness, double maxerror, boolean forceAngle, boolean forceTrace, boolean flatten,
			int maxPoly) {
	 ...
	 * segment consists of the segment type followed by 0-6 coordintes (0-3 end
	 * points and control
	 * points). Depending on the type, a segment uses from 1 to 7 elements of the
	 * array.
	 */
	...
	public ShapeRoi or(ShapeRoi sr) {
		return unaryOp(sr, OR);
	}

	/**
	 * Unary intersection operator.
	 * The caller is set to its intersection with the argument (i.e., the
	 * overlapping regions between the
	 * operands).
	 * 
	 * @return the overlapping regions between <strong><code>this</code></strong>
	 *         and <code>sr</code>
	 */
	public ShapeRoi and(ShapeRoi sr) {
		return unaryOp(sr, AND);
	}
	 ...
	 * 
	 * @return the union of the non-overlapping regions of <code>this</code> and
	 *         <code>sr</code>
	 * @see ij.gui.Roi#xor(Roi[])
	 * @see ij.gui.Overlay#xor(int[])
	 */
	public ShapeRoi xor(ShapeRoi sr) {
		return unaryOp(sr, XOR);
	}
	 ...
	 * <thead>
	 * <tr>
	 * <th scope=col>Roi class</th>
	 * <th scope=col>Roi type</th>
	 ...
	 * <td>ij.gui.Roi</td>
	 * <td>Roi.RECTANGLE</td>
	 * <td>java.awt.geom.Rectangle2D.Double</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.OvalRoi</td>
	 * <td>Roi.OVAL</td>
	 * <td>java.awt.Polygon of the corresponding traced roi</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.Line</td>
	 * <td>Roi.LINE</td>
	 * <td>java.awt.geom.Line2D.Double</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.POLYGON</td>
	 * <td>java.awt.Polygon or (if subpixel resolution) closed
	 * java.awt.geom.GeneralPath</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.FREEROI</td>
	 ...
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.POLYLINE</td>
	 * <td>open java.awt.geom.GeneralPath</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.FREELINE</td>
	 ...
	 * </tr>
	 * <tr>
	 * <td>ij.gui.ShapeRoi</td>
	 * <td>Roi.COMPOSITE</td>
	 * <td>shape of argument</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.ShapeRoi</td>
	 * <td>ShapeRoi.NO_TYPE</td>
	 * <td>null</td>
	 * </tr>
	 * </tbody>
	 * </table>
	...
	private Shape roiToShape(Roi roi) {
		...
		switch (roiType) {
			...
			case Roi.OVAL:
				if (roiType == Roi.OVAL) {
					// shape = new Ellipse2D.Double(-0.001, -0.001, r.width+0.002, r.height+0.002);
					// //inaccurate (though better with increased diameter)
		...
		if (shape != null) {
			...
			this.startX = x;
			this.startY = y;
			// IJLog.log("RoiToShape: "+x+" "+y+" "+width+" "+height+" "+bounds);
		}
		return shape;
	}

	/**
	 * Constructs a GeneralPath from a float array of segment type+coordinates for
	 * each segment.
	 * The resulting GeneralPath has winding rule WIND_EVEN_ODD, which is
	 * appropriate for closed shapes
	 */
	static GeneralPath makeShapeFromArray(float[] array) {
		...
		while (true) {
			// if(index<array.length)IJLog.log(index+" type="+array[index]);
	...
	/**
	 * Reads the data of a segment from an array describing the shape at as in a
	 * PathIterator. Reading starts at 'index'.
	 * The position in the array for the next segment is returned, -1 if the array
	 * ends.
	 * The segment type and coordinates of the segment are stored into 'seg' (which
	 * must have a length of at least 7)
	 */
	...
	/**
	 * Saves an Roi so it can be retrieved later using getRois(). Not compatible
	 * with the other functions of this class.
	 * 
	 * @deprecated Use ShapeRoi(Roi) creator and merge with
	 *             <code>or(ShapeRoi)</code>.
	 */
	 ...
	 * <br>
	 * This method parses the shape into (possibly more than one) Roi objects
	 * and returns them in an array.
	 * <br>
	 * A simple, &quot;regular&quot; path results in a single Roi following these
	 * simple rules:
	 * <table>
	 * <col><col><col>
	 * <thead>
	 ...
	 * </tr>
	 * <tr>
	 * <td>java.awt.geom.Ellipse2D.Double</td>
	 * <td>ij.gui.OvalRoi</td>
	 * <td>Roi.OVAL</td>
	 * </tr>
	 * <tr>
	 ...
	 * <br>
	 * <br>
	 * Each subpath of a <code>java.awt.geom.GeneralPath</code> is converted
	 * following these rules:
	 * <table frame="border">
	 * <col><col><col><col><col><col>
	 * <thead>
	 * <tr>
	 * <th rowspan="2" scope=col>Segment<br>
	 * types</th>
	 * <th rowspan="2" scope=col>Number of<br>
	 * segments</th>
	 * <th rowspan="2" scope=col>Closed<br>
	 * path</th>
	 * <th rowspan="2" scope=col>Value of<br>
	 * forceAngle</th>
	 * <th rowspan="2" scope=col>Value of<br>
	 * forceTrace</th>
	 * <th rowspan="2" scope=col>Roi type</th>
	 * </tr>
	 * </thead>
	 * <tbody>
	 * <tr>
	...
	public Roi[] getRois() {
		...
		if (shape instanceof Rectangle2D.Double) {
			Roi r = new Roi((int) ((Rectangle2D.Double) shape).getX(), (int) ((Rectangle2D.Double) shape).getY(),
					(int) ((Rectangle2D.Double) shape).getWidth(), (int) ((Rectangle2D.Double) shape).getHeight());
			rois.add(r);
		} else if (shape instanceof Ellipse2D.Double) {
			Roi r = new OvalRoi((int) ((Ellipse2D.Double) shape).getX(), (int) ((Ellipse2D.Double) shape).getY(),
					(int) ((Ellipse2D.Double) shape).getWidth(), (int) ((Ellipse2D.Double) shape).getHeight());
			rois.add(r);
		} else if (shape instanceof Line2D.Double) {
			Roi r = new ij.gui.Line((int) ((Line2D.Double) shape).getX1(), (int) ((Line2D.Double) shape).getY1(),
					(int) ((Line2D.Double) shape).getX2(), (int) ((Line2D.Double) shape).getY2());
			rois.add(r);
		} else if (shape instanceof Polygon) {
			Roi r = new PolygonRoi(((Polygon) shape).xpoints, ((Polygon) shape).ypoints, ((Polygon) shape).npoints,
					Roi.POLYGON);
			rois.add(r);
		} else {
			PathIterator pIter;
	 ...
	 * 
	 * @return an ij.gui.Roi object or null if it cannot be simplified to become a
	 *         non-composite roi.
	 */
	...
	/**
	 * Implements the rules of conversion from
	 * <code>java.awt.geom.GeneralPath</code> to <code>ij.gui.Roi</code>.
	 * 
	 * @param nSegments                 The number of segments that compose the path
	 *                                  (= number of vertices for a polygon)
	 * @param polygonLength             length of polygon in pixels, or NaN if
	 *                                  curved segments
	 * @param horizontalVerticalIntOnly Indicates whether the GeneralPath is
	 *                                  composed of only vertical and horizontal
	 *                                  lines with integer coordinates
	 * @param forceTrace                Indicates that closed shapes with
	 *                                  <code>horizontalVerticalIntOnly=true</code>
	 *                                  should become TRACED_ROIs
	 * @param closed                    Indicates a closed GeneralPath
	 * @see #shapeToRois()
	 * @return a type flag like Roi.RECTANGLE or NO_TYPE if the type cannot be
	 *         determined
	 */
	private int guessType(int nSegments, double polygonLength, boolean horizontalVerticalIntOnly, boolean forceTrace,
		...
		// IJLog.log("guessType n= "+nSegments+" len="+polygonLength+"
		// longE="+(polygonLength/(nSegments*Math.sqrt(nSegments)) >= 2)+"
		// hvert="+horizontalVerticalIntOnly+" clos="+closed+" -> "+roiType);
		return roiType;
	}
	...
	private Roi createRoi(float[] xPoints, float[] yPoints, int roiType) {
		if (roiType == NO_TYPE || roiType == Roi.COMPOSITE)
			return null;
		Roi roi = null;
		...
		switch (roiType) {
			case Roi.LINE:
				roi = new ij.gui.Line(xPoints[0], yPoints[0], xPoints[1], yPoints[1]);
				break;
			case Roi.RECTANGLE:
	...
	/**
	 * Checks whether the center of the specified pixel inside of this ROI's shape
	 * boundaries.
	 * Note the ImageJ convention of 0.5 pixel shift between outline and pixel
	 * center,
	 * i.e., pixel (0,0) is enclosed by the rectangle spanned between (0,0) and
	 * (1,1).
	 * The value slightly below 0.5 is for rounding according to the ImageJ
	 * convention
	 * (which is opposite to that of the java.awt.Shape class):
	 * In ImageJ, points exactly at the left (right) border are considered outside
	 * (inside);
	 * points exactly on horizontal borders, are considered outside (inside) at the
	 * border
	 ...
	 * Use contains(int, int) to determine whether a given pixel is contained in the
	 * Roi.
	 */
	public boolean containsPoint(double x, double y) {
		if (!super.containsPoint(x, y))
			return false;
		return shape.contains(x - this.x + 1e-3, y - this.y + 1e-6); // adding a bit to reduce the likelyhood of
																		// numerical errors at integers
	}
	...
	/**
	 * Returns a path iterator for this ROI's shape containing no curved (only
	 * straight) segments
	 */
	...
	/**
	 * Length of the control polygon of the cubic B&eacute;zier curve argument, in
	 * double precision.
	 */
	...
	/**
	 * Length of the control polygon of the quadratic B&eacute;zier curve argument,
	 * in double precision.
	 */
	...
	/**
	 * Length of the chord between the end points of the cubic B&eacute;zier curve
	...
	/**
	 * Length of the chord between the end points of the quadratic B&eacute;zier
	 * curve argument, in double precision.
	 */
	...
	/**
	 * Calculates the length of a cubic B&eacute;zier curve specified in double
	 * precision.
	 * The algorithm is based on the theory presented in paper <br>
	 * &quot;Jens Gravesen. Adaptive subdivision and the length and energy of
	 * B&eacute;zier curves. Computational Geometry
	 * <strong>8:</strong><em>13-31</em> (1997)&quot;
	 * implemented using <code>java.awt.geom.CubicCurve2D.Double</code>.
	 * Please visit
	 * {@link <a href="http://www.graphicsgems.org/gems.html#gemsiv">Graphics Gems
	 * IV</a>} for
	...
	/**
	 * Calculates the length of a quadratic B&eacute;zier curve specified in double
	 * precision.
	 * The algorithm is based on the theory presented in paper <br>
	 * &quot;Jens Gravesen. Adaptive subdivision and the length and energy of
	 * B&eacute;zier curves. Computational Geometry
	...
	// c is an array of even length with x0, y0, x1, y1, ... ,xn, yn coordinate
	// pairs
	/**
	 * Scales a coordinate array with the size calibration of a 2D image.
	 * The array is modified in place.
	 * 
	 * @param c  Array of coordinates in double precision with a
	 *           <strong>fixed</strong> structure:<br>
	 *           <code>x0,y0,x1,y1,....,xn,yn</code> and with even length of
	 *           <code>2*(n+1)</code>.
	 * @param pw The x-scale of the image.
	 * @param ph The y-scale of the image.
	 * @param n  number of values in <code>c</code> that should be modified (must be
	 *           less or equal to the size of <code>c</code>
	 *
	 */
	...
	/**
	 * Applies an offset xBase, yBase to the n coordinates, which are supposed to be
	 * in x, y, x, y ... sequence
	 */
	...
	/**
	 * Retrieves the end points and control points of the path as a float array. The
	 * array
	 * contains a sequence of variable length segments that use from from one to
	 * seven array elements.
	 * The first element of a segment is the type as defined in the PathIterator
	 * interface. SEG_MOVETO
	 * and SEG_LINETO segments also include two coordinates (one end point),
	 * SEG_QUADTO segments include four
	 ...
	 * <br>
	 * - create a single non-Shape Roi and add it to <code>rois</code> in case
	 * there is only one subpath, otherwise add this Roi unchanged to
	 * <code>rois</code>
	 * (task = ONE_ROI and rois non-null)
	 * <br>
	 * - add each subpath as a Roi to rois; curved subpaths will be flattened, i.e.
	 * converted to a
	 * polygon approximation (task != ONE_ROI and rois non-null)
	 * <br>
	 * - measure the combined length of all subpaths/Rois and return it (task =
	 * GET_LENGTH, rois may be null)
	 * 
	 * @param pIter  the PathIterator to be parsed.
	 * @param params an array with one element that will hold the calculated total
	 *               length of the rois if its initial value is 0.
	 *               If params holds the value SHAPE_TO_ROI, it will be tried to
	 *               convert this ShapeRoi to a non-composite Roi. If this
	 *               is not possible and this ShapeRoi is not empty, a reference to
	 *               this ShapeRoi will be returned.
	 * @param rois   an ArrayList that will hold ij.gui.Roi objects constructed from
	 *               subpaths of this path;
	...
	double parsePath(PathIterator pIter, int task, ArrayList rois) {
		...
		boolean done = false;
		while (true) {
			int segType = done ? NO_SEGMENT_ANY_MORE : pIter.currentSegment(fcoords); // read segment (if there is one
																						// more)
			...
			// IJLog.log("segType="+segType+" nCoord="+nCoords+" done="+done+"
			// nPoi="+nPoints+" len="+pathLength);
			if (segType == NO_SEGMENT_ANY_MORE || (segType == PathIterator.SEG_MOVETO && xPoints.size() > 0)) {
				...
				closed = closed || (xPoints.size() > 0 && xPoints.get(0) == xPoints.getLast()
						&& yPoints.get(0) == yPoints.getLast());
				...
				boolean forceTrace = getLength && (!done || nSubPaths > 0); // when calculating the length for >1
																			// subpath, assume traced rois if it can be
																			// such
				int roiType = guessType(xPoints.size(), uncalLength, horVertOnly, forceTrace, closed);
				Roi roi = null;
				if (roiType == COMPOSITE && rois != null) { // for ShapeRois with curves, we have the length from the
															// path already, make roi only if needed
					Shape shape = makeShapeFromArray(shapeArray.toArray()); // the curved subpath (image pixel
																			// coordinates)
					FloatPolygon fp = getFloatPolygon(shape, FLATNESS, /* separateSubpaths= */ false,
							/* addPointForClose= */ false, /* absoluteCoord= */ false);
				...
				if (getLength && roi != null && !Double.isNaN(uncalLength)) {
					...
					pathLength = roi.getLength();// we don't use the path length of the Shape; e.g. for traced rois
													// ImageJ has a better algorithm
					roi.setImage(null);
				}
			...
			switch (segType) {
				...
				case PathIterator.SEG_QUADTO:
					if (getLength) {
						QuadCurve2D.Double curve = new QuadCurve2D.Double(lastCalX, lastCalY, coords[0], coords[2],
								coords[2], coords[3]);
				...
				case PathIterator.SEG_CUBICTO:
					if (getLength) {
						CubicCurve2D.Double curve = new CubicCurve2D.Double(lastCalX, lastCalY, coords[0], coords[1],
	...
	public void draw(Graphics g) {
		...
		// IJLog.log("draw: "+overlay+" "+isActiveOverlayRoi);
	...
	public void drawRoiBrush(Graphics g) {
		...
		if ((flags & 16) == 0)
			return; // exit if mouse button up
		int osize = size;
		size = (int) (size * mag);
	 ...
	 * Takes into account the usual ImageJ convention of 0.5 pxl shift between the
	 * outline and pixel
	 * coordinates; e.g., pixel (0,0) is surrounded by the rectangle spanned between
	 * (0,0) and (1,1).
	 * Note that apart from the 0.5 pixel shift, ImageJ has different convention for
	 * the border points
	...
	public ImageProcessor getMask() {
		...
		if (mask != null && mask.getPixels() != null && mask.getWidth() == width && mask.getHeight() == height)
			return mask;
		/*
		 * The following code using Graphics2D.fill would in principle work, but is very
		 * inaccurate
		 * at least with Oracle Java 8 or OpenJDK Java 10.
		 * For near-vertical polgon edges of 1000 pixels length, the deviation can be
		 * >0.8 pixels in x.
		 * Thus, approximating the shape by a polygon and using the PolygonFiller is
		 * more accurate
		 ...
		 * g2d.transform(AffineTransform.getTranslateInstance(-0.48, -0.49994)); //very
		 * inaccurate, only reasonable with "-0.48"
		 * g2d.fill(shape);
		...
		PolygonFiller pf = new PolygonFiller(fpoly.xpoints, fpoly.ypoints, fpoly.npoints, (float) (getXBase() - x),
				(float) (getYBase() - y));
	...
	/**
	 * Sets the <code>java.awt.Shape</code> object encapsulated by
	 * <strong><code>this</code></strong>
	 * to the argument.
	 * <br>
	 * This object will hold a (shallow) copy of the shape argument. If a deep copy
	 * of the shape argumnt is required, then a clone of the argument should be
	 * passed
	...
	/**
	 * Returns a FloatPolygon with all vertices of the flattened shape, i.e., the
	 * shape approximated by
	 * straight line segments. This method is for listing the coordinates and
	 * creating the convex hull.
	 * 
	 * @param flatness         Roughly the maximum allowable distance between the
	 *                         shape and the approximate polygon
	 * @param separateSubpaths whether individual subpaths should be separated by
	 *                         NaN coordinates
	 * @param addPointForClose whether the starting point of a closed subpath should
	 *                         be repeated at its end.
	 *                         Note that with <code>addPointForClose = false</code>,
	 *                         there is no distinction between open and closed
	 *                         subpaths.
	 * @param absoluteCoord    specifies whether the coordinates should be with
	 *                         respect to image bounds, not Roi bounds.
	 */
	public FloatPolygon getFloatPolygon(double flatness, boolean separateSubpaths, boolean addPointForClose,
			boolean absoluteCoord) {
		return getFloatPolygon(shape, flatness, separateSubpaths, addPointForClose, absoluteCoord);
	}

	public FloatPolygon getFloatPolygon(Shape shape, double flatness, boolean separateSubpaths,
			boolean addPointForClose, boolean absoluteCoord) {
		if (shape == null)
			return null;
		...
		int n = xpf.length;
		if (n > 0 && Float.isNaN(xpf[n - 1]))
			n--; // omit NaN at the end
		return new FloatPolygon(xpf, ypf, n);
	}

	public FloatPolygon getFloatConvexHull() {
		FloatPolygon fp = getFloatPolygon(FLATNESS, /* separateSubpaths= */ false, /* addPointForClose= */ false,
				/* absoluteCoord= */ true);
	...
	public FloatPolygon getFloatPolygon() {
		return getFloatPolygon(FLATNESS, /* separateSubpaths= */ false, /* addPointForClose= */ false, /*
																										 * absoluteCoord=
																										 */ true);
	}
	...
	/**
	 * Retuns the number of vertices, of this shape as approximated by straight
	 * lines.
