 ...
 *
 * Version 2009-06-01 (code refurbished; tolerance, 4- & 8-connected options
 * added)
 */
public class Wand {
     ...
     * gets selected; four- or 8-connected behaviour depends on
    ...
    public void autoOutline(int startX, int startY, double tolerance, int mode) {
        if (startX < 0 || startX >= width || startY < 0 || startY >= height)
            return;
        if (fpixels != null && Float.isNaN(getPixel(startX, startY)))
            return;
        ...
        while (true) { // loop until we have not traced an inner hole
            ...
            if (legacyMode)
                return; // in legacy mode, don't care what we have got
            if (insideSelected) { // not an inner hole
                if (first)
                    return; // started at seed, so we got it (sucessful)
            ...
            if (!inside(x, y))
                do {
                    x++; // traverse the hole
                    if (x > width)
                        throw new RuntimeException("Wand Malfunction"); // should never happen
                } while (!inside(x, y));
            do {
                x++;
    ...
    private boolean isLine(int xs, int ys) {
        ...
        if (IJ.debugMode)
            IJLog.log((((double) insideCount) / area < 0.25 ? "line " : "blob ") + insideCount + " " + area + " "
                    + IJ.d2s(((double) insideCount) / area));
        return ((double) insideCount) / area < 0.25;
    }

    /**
     * Set 'true' and output will contain intermediate points for straight lines
    ...
    /**
     * Returns 'true' if output contains intermediate points for straight lines
     * longer than one pixel.
     */
