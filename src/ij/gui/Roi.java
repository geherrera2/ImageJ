 ...
 * </b>
 * <ul>
 * <li>Area ROIs: Integer coordinates refer to the top-left corner of the pixel
 * with these coordinates.
 * Thus, pixel (0,0) is enclosed by the rectangle spanned between points (0,0)
 * and (1,1),
 ...
 * Thus, a line from (0,0) to (1,0) has its start and end points in the center
 * of
 * pixels (0,0) and (1,0), respectively, and drawing the line should affect both
 * pixels. For images dispplayed at high zoom levels, this means that (open)
 * lines
 * and single points are displayed 0.5 pixels further to the right and bottom
 * than
 * the outlines of area ROIs (closed lines) with the same coordinates.
 * </ul>
 * Note that rectangular and (nonrotated) oval ROIs do not support subpixel
 * resolution.
 * Since ImageJ 1.52t, this convention does not depend on the
 * Prefs.subpixelResolution
 * (previously accessible via Edit>Options>Plot) and this flag has no effect any
 * more.
 *
 */
public class Roi extends Object implements Cloneable, java.io.Serializable, Iterable<Point> {
	 ...
	 * <br>
	 * [1] "FeretAngle" (angle of diameter with maximum caliper width, between 0 and
	 * 180 deg)
	 * <br>
	 * [2] "MinFeret" (minimum caliper width)
	 * <br>
	 * [3][4] , "FeretX" and "FeretY", the X and Y coordinates of the starting point
	 ...
	 * <p>
	 * The following array elements are end points of the maximum and minimum
	 * caliper diameter,
	 ...
	 * <br>
	 * [10][11] "FeretX2", "FeretY2", end point of the maxium-caliper-width
	 * diameter.
	 * Both of these points are vertices of the convex hull.
	 * <br>
	 * The final four array elements are the starting and end points of the minimum
	 * caliper width,
	 * <br>
	 * [12],[13] "MinFeretX", "MinFeretY", and
	 * <br>
	 * [14],[15] "MinFeretX2", "MinFeretY2". These two pooints are not sorted by x,
	 * but the first point point (MinFeretX, MinFeretY) is guaranteed to be a vertex
	 * of the convex hull,
	 * while second point (MinFeretX2, MinFeretY2) usually is not a vertex point but
	 * at a
	...
	public double[] getFeretValues() {
		...
		// Get the point with the largest distance from the line between these two
		// points
		// Of all these pairs, take the one where the distance is the lowest
		// The following code requires a counterclockwise convex hull with no duplicate
		// points
		...
		for (int i = 0; i < poly.npoints; i++) { // find caliper width for one side of calipers touching points i-1, i
			...
			xnorm *= normalizationFactor * pw; // normalized vector perpendicular to line between i-1, i; * scale factor
												// for product below
	...
	/**
	 * Aborts constructing or modifying the roi (called by the ImageJ class on
	 * escape)
	 */
	...
	void move(int sx, int sy) {
		...
		if ((lineWidth > 1 && isLine()) || ignoreClipRect
				|| ((this instanceof PolygonRoi) && ((PolygonRoi) this).isSplineFit()))
			imp.draw();
		else
			imp.draw(clipX, clipY, clipWidth, clipHeight);
		oldX = x;
	...
	public void draw(Graphics g) {
		...
		if (fillColor != null)
			color = fillColor;
		if (Interpreter.isBatchMode() && imp != null && imp.getOverlay() != null && strokeColor == null
				&& fillColor == null)
			return;
		g.setColor(color);
	 ...
	 * border
	 * with the lower (higher) y. This convention is opposite to that of the
	 * java.awt.Shape class.
	 */
	...
	/**
	 * Returns the inverted roi, or null if this is not an area roi or cannot be
	 * converted to a ShapeRoi.
	 * If imp is not given, assumes a rectangle of size 2e9*2e9 for the boundary.
	 */
	public Roi getInverse(ImagePlus imp) {
		if (!isArea())
			return null;
		Roi fullImage = (imp == null) ? new Roi(0, 0, 2000000000, 2000000000)
			...
			if (type == OVAL)
				...
				if (cornerDiameter == 0)
					...
					if (Recorder.scriptMode())
						Recorder.recordCall("imp.setRoi(new Roi(" + x + "," + y + "," + width + "," + height + ","
								+ cornerDiameter + "));");
					else
	 ...
	 * true, subtracts
	 * it from the previous selection. Called by the IJ.doWand() method, and the
	 * makeRectangle(),
	...
	public void showStatus() {
		...
		String size;
		if (cal.scaled())
			size = ", w=" + IJ.d2s(width * cal.pixelWidth) + " (" + width + "), h=" + IJ.d2s(height * cal.pixelHeight)
					+ " (" + height + ")";
		else
	...
	public ImageProcessor getMask() {
		if (cornerDiameter > 0)
			return (new ShapeRoi(new RoundRectangle2D.Float(x, y, width, height, cornerDiameter, cornerDiameter)))
					.getMask();
		else
			return null;
	}
	...
	public static synchronized void setGroupName(int groupNumber, String name) {
		...
		// IJLog.log("setGroupName: "+groupNumber+" "+name+" "+groupNames.length);
		...
		switch (type) {
			case POLYGON:
				...
				if (this instanceof EllipseRoi)
					s = "Ellipse";
				if (this instanceof RotatedRectRoi)
					s = "Rotated Rectangle";
				break;
			case FREEROI:
				s = "Freehand";
	...
	/**
	 * @deprecated This method was previously used to draw lines and polylines
	 *             shifted
	 *             by 0.5 pixels top the bottom and right, for better agreement with
	 *             the
	 *             position used by ProfilePlot, with the default taken from
	 *             Prefs.subPixelResolution. Now the shift is independent of this
	 *             setting and only depends on the ROI type (area or line/point
	 *             ROI).
	 */
	@Deprecated
	...
	/**
	 * Converts image canvas screen x coordinates to floating-point offscreen image
	 * pixel
	 * coordinates, depending on whether this roi uses the line or area convention
	 * for coordinates.
	 */
	...
	/**
	 * Converts image canvas screen y coordinates to floating-point offscreen image
	 * pixel
	 * coordinates, depending on whether this roi uses the line or area convention
	 * for coordinates.
	 */
