...
public class IJ {
	...
	public static Object runPlugIn(String commandName, String className, String arg) {
		if (arg == null)
			arg = "";
		if (IJ.debugMode)
			IJLog.log("runPlugIn: " + className + argument(arg));
		...
		try {
			...
			if (!(className != null && className.startsWith("ij.plugin.MacAdapter"))) {
				IJLog.log("Plugin or class not found: \"" + className + "\"\n(" + e + ")");
				String path = Prefs.getCustomPropsPath();
				if (path != null)
					;
				IJLog.log("Error may be due to custom properties at " + path);
			}
		} catch (InstantiationException e) {
			IJLog.log("Unable to load plugin (ins)");
		} catch (IllegalAccessException e) {
			IJLog.log("Unable to load plugin, possibly \nbecause it is not public.");
		}
	...
	static Object runUserPlugIn(String commandName, String className, String arg, boolean createNewLoader) {
		if (IJ.debugMode)
			IJLog.log("runUserPlugIn: " + className + ", arg=" + argument(arg));
		if (applet != null)
			return null;
		if (createNewLoader)
			classLoader = null;
		...
		try {
			...
			if (dotIndex >= 0 && className.contains("_")) {
				...
				if (debugMode)
					IJLog.log("runUserPlugIn: rerunning " + className);
	...
	public static void run(String command, String options) {
		// IJLog.log("run1: "+command+" "+Thread.currentThread().hashCode()+"
		// "+options);
		...
		// IJLog.log("run2: "+command+" "+Thread.currentThread().hashCode());
	}
	...
	/**
	 * @deprecated
	 *             replaced by IJLog.log(), ResultsTable.setResult() and
	 *             TextWindow.append().
	...
	/**
	 * TextWindow calls this method with a null argument when the "Results" window
	 * is closed.
	 */
	...
	public static void outOfMemory(String name) {
		...
		if (!memMessageDisplayed)
			IJLog.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>");
		IJLog.log("<Out of memory>");
		if (!memMessageDisplayed) {
			IJLog.log("<All available memory (" + tot + ") has been>");
			IJLog.log("<used. To make more available, use the>");
			IJLog.log("<Edit>Options>Memory & Threads command.>");
			IJLog.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>");
	...
	/**
	 * Sets the display range (minimum and maximum displayed pixel values) of the
	 * current image.
	 */
	 ...
	 * 16 and 32 bit images unless the display range has been reset using
	 * IJ.resetMinAndMax().
	 */
	 ...
	 * the specified <code>displayMode</code> ("Red", "Black & White", "Over/Under"
	 * or "No Update").
	 */
	...
	/**
	 * Sets the lower and upper threshold levels of the specified image and updates
	 * the display using
	 * the specified <code>displayMode</code> ("Red", "Black & White", "Over/Under"
	 * or "No Update").
	 * With calibrated images, 'lowerThreshold' and 'upperThreshold' must be density
	 * calibrated values.
	...
	/**
	 * Sets the transfer mode used by the <i>Edit/Paste</i> command, where mode is
	 * "Copy", "Blend", "Average", "Difference",
	 * "Transparent", "Transparent2", "AND", "OR", "XOR", "Add", "Subtract",
	 * "Multiply", or "Divide".
	 */
	...
	/**
	 * Opens an image using a file open dialog and returns it as an ImagePlus
	 * object.
	 */
	 ...
	 * //
	 * http://stackoverflow.com/questions/10135074/download-file-from-https-server-
	 * using-java
	 ...
	 * specified file path.
	 * The path must end in ".tif", ".jpg", ".gif", ".zip", ".raw", ".avi", ".bmp",
	...
	/**
	 * Saves the specified image, lookup table or selection to the specified file
	 * path.
	 * The file path should end with ".tif", ".jpg", ".gif", ".zip", ".raw", ".avi",
	 * ".bmp",
	 * ".fits", ".pgm", ".png", ".lut", ".roi" or ".txt". The specified image is
	 * saved in
	 ...
	 * selection XY
	 * coordinates or text window to the specified file path. The format argument
	 * must be "tiff",
	 * "jpeg", "gif", "zip", "raw", "avi", "bmp", "fits", "pgm", "png",
	 * "text image", "lut", "selection", "measurements",
	 * "xy Coordinates" or "text". If <code>path</code> is null or an emply string,
	 * a file
	 ...
	 *                 contain "white" , "black" (the default), "ramp",
	 *                 "composite-mode",
	...
	/**
	 * Opens a new image. <code>Type</code> should contain "8-bit", "16-bit",
	 * "32-bit" or "RGB".
	 * In addition, it can contain "white", "black" or "ramp". <code>Width</code>
	 * and <code>height</code> specify the width and height of the image in pixels.
	 * <code>Depth</code> specifies the number of stack slices.
	...
	/**
	 * Set 'true' and IJ.error() output will be temporarily redirected to the "Log"
	 * window.
	 */
	...
	/**
	 * Returns the state of the 'redirectErrorMessages' flag, which is set by
