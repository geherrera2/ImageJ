...
public class ThreadUtil {
	...
	/**
	 * The threadPoolExecutor holds at least as many threads for parallel execution
	 * as the number of
	 * processors; additional threads are added as required. These additional
	 * threads will be
	 ...
	 *                  best use <code>Callable<Void></code> (then the
	 *                  <code>Void call()</code> method
	 *                  should return null). If the array size is 1, the
	 ...
	 *         return results, the get() methods of these Futures may be used to get
	 *         the results.
	 */
	 ...
	 *                  if no return value is needed (then the <code>call</code>
	 *                  methods should
	 *                  return null).
	 * @return Array of the <code>java.util.concurrent.Future</code>s,
	...
	public static void joinAll(Future[] futures) {
		...
		for (int i = 0; i < futures.length; i++) {
			...
			try {
				...
				ij.IJLog.log("Error in thread called by " + Thread.currentThread().getName() + ":\n" + eOther);
			}
		}
		if (interrupted) {
