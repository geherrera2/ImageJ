...
public class DicomTools {
	...
	public static ImageStack sort(ImageStack stack) {
		if (IJ.debugMode)
			IJLog.log("Sorting by DICOM image number");
		if (stack.size() == 1)
			return stack;
	...
	private static String[] getSortStrings(ImageStack stack, String tag) {
		...
		int n = stack.size();
		boolean checkRescaleSlope = (stack instanceof VirtualStack) ? ((VirtualStack) stack).getBitDepth() == 16
				: false;
		...
		for (int i = 1; i <= n; i++) {
			...
			if (Double.isNaN(value)) {
				if (IJ.debugMode)
					IJLog.log("  " + tag + "  tag missing in slice " + i);
				if (showError)
					rescaleSlopeError(stack);
				return null;
			}
			if (getSeriesNumber(tags) != series) {
				if (IJ.debugMode)
					IJLog.log("  all slices must be part of the same series");
				if (showError)
					rescaleSlopeError(stack);
				return null;
			}
	...
	public static double getVoxelDepth(ImageStack stack) {
		...
		if (IJ.debugMode)
			IJLog.log("DicomTools.getVoxelDepth: " + voxelDepth + "  " + pos0 + "  " + posn);
		return voxelDepth;
	}

	/**
	 * Returns the value (as a string) of the specified DICOM tag id (in the form
	 * "0018,0050")
	 * of the specified image or stack slice. Returns null if the tag id is not
	 * found.
	 */
	...
	private static String getTag(String hdr, String tag) {
		...
		if (index1 == -1)
			return null;
		// IJLog.log(hdr.charAt(index1+11)+" "+hdr.substring(index1,index1+20));
