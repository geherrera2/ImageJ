...
/**
 * Saves an ROI to a file or stream. RoiDecoder.java has a description of the
 * file format.
 * 
...
public class RoiEncoder {
	...
	void write(Roi roi, OutputStream f) throws IOException {
		...
		if (n > 65535 && type != point) {
			...
			ij.IJ.beep();
			ij.IJLog.log("Non-polygonal selections with more than 65k points cannot be saved.");
			n = 65535;
		}
	...
	void saveShapeRoi(Roi roi, int type, OutputStream f, int options) throws IOException {
		...
		// ij.IJLog.log("saveShapeRoi: "+HEADER_SIZE+" "+shapeArray.length);
	...
	void saveTextRoi(TextRoi roi) {
		...
		// ij.IJLog.log("saveTextRoi: "+HEADER_SIZE+" "+textRoiDataLength+"
		// "+fontNameLength+" "+textLength);
	...
	void putHeader2(Roi roi, int hdr2Offset) {
		// ij.IJLog.log("putHeader2: "+hdr2Offset+" "+roiNameSize+" "+roiName);
