...
public class TiffDecoder {
	...
	void dumpTag(int tag, int count, int value) {
		...
		// ij.IJLog.log(tag + ", \"" + name + "\", value=" + value + cs + "\n");
	}
		...
		for (int i = 0; i < nEntries; i++) {
			...
			switch (tag) {
				...
				case PREDICTOR:
					...
					if (value == 3)
						IJLog.log("TiffDecoder: unsupported predictor value of 3");
					break;
				case COLOR_MAP:
					if (count == 768)
	...
	void getMetaData(int loc, FileInfo fi) throws IOException {
		...
		for (int i = 0; i < nTypes; i++) {
			...
			if (debugMode) {
				...
				int count = counts[i];
				index += count;
				if (index >= metaDataCounts.length)
					index = 1;
	...
	void getSliceLabels(int first, int last, FileInfo fi) throws IOException {
		...
		for (int i = first; i <= last; i++) {
			...
			if (len > 0) {
				...
				// ij.IJLog.log(i+" "+fi.sliceLabels[i-1]+" "+len);
			} else
