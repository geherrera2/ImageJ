...
public class ImageReader {
	...
	/**
	 * Constructs a new ImageReader using a FileInfo object to describe the file to
	 * be read.
	 * 
	 * @see ij.io.FileInfo
	 */
	...
	short[] readCompressed16bitImage(InputStream in) throws IOException {
		if (IJ.debugMode)
			IJLog.log("ImageReader.read16bit, offset=" + fi.stripOffsets[0]);
		short[] pixels = new short[nPixels];
		int base = 0;
		short last = 0;
		for (int k = 0; k < fi.stripOffsets.length; k++) {
			// IJLog.log("seek: "+k+" "+fi.stripOffsets[k]+" "+fi.stripLengths[k]+" "+(in
	 ...
	 * } catch(Exception e) {}
	 * }
	 * IJLog.log(label+": debug: offset="+offset+", fi="+fi);
	 * }
	 */
}
