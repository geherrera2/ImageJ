...
public class DirectoryChooser {
			...
			EventQueue.invokeAndWait(new Runnable() {
				public void run() {
					...
					if (defaultDir != null) {
						...
						if (IJ.debugMode)
							IJLog.log("DirectoryChooser,setSelectedFileW: " + f);
	...
	void getDirectoryUsingJFileChooserOnThisThread(final String title) {
		...
		try {
			...
			if (defaultDir != null) {
				File f = new File(defaultDir);
				if (IJ.debugMode)
					IJLog.log("DirectoryChooser,setSelectedFile: " + f);
				chooser.setSelectedFile(f);
			}
			chooser.setApproveButtonText("Select");
			if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
	...
	void getDirectoryUsingFileDialog(String title) {
		...
		if (IJ.debugMode)
			IJLog.log("DirectoryChooser: dir=\"" + dir + "\",  file=\"" + name + "\"");
	...
	public String getDirectory() {
		if (IJ.debugMode)
			IJLog.log("DirectoryChooser.getDirectory: " + directory);
