 ...
 * 14-15 right
 * 16-17 NCoordinates
 * 18-33 x1,y1,x2,y2 (straight line) | x,y,width,height (double rect) | size
 * (npoints)
 ...
 * 
 * @see <a href=
 *      "http://wsr.imagej.net/macros/js/DecodeRoiFile.js">DecodeRoiFile.js</a>
 */
	...
	public Roi getRoi() throws IOException {
		...
		switch (type) {
			...
			case point:
				// IJLog.log("type: "+type);
				// IJLog.log("n: "+n);
				// IJLog.log("rect: "+left+","+top+" "+width+" "+height);
				if (n == 0 || n < 0)
					break;
				int[] x = new int[n];
				int[] y = new int[n];
