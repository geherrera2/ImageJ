...
public class DragAndDropHandler extends TransferHandler {
	...
	/**
	 * Imports the drag&drop file or list of files and sets the JFileChooser to
	 * this.
		...
		for (DataFlavor dataFlavor : transferFlavors) {
			try {
				...
				if (dataFlavor.isFlavorJavaFileListType()) {
					...
					if (IJ.debugMode)
						IJLog.log("dragAndDrop FileList size=" + fileList.size() + " first: " + fileList.get(0));
				} else if (isSupportedTransferFlavor(dataFlavor)) {
					String str = (String) t.getTransferData(dataFlavor);
					if (IJ.debugMode)
						IJLog.log("dragAndDrop str=" + str);
					...
					for (String s : strs) {
						if (s.length() > 0) {
							...
							if (!file.exists())
								try {
									file = new File(new URI(s)); // When not successful, try a whether it is a URL, e.g.
				...
				if (fileList == null || fileList.size() == 0)
					continue; // this data flavor did not work
				...
				if (jFileChooser.isMultiSelectionEnabled()) { // multiple files accepted
					...
					if (jFileChooser.getDialogType() == JFileChooser.SAVE_DIALOG && file.isDirectory())
						jFileChooser.setCurrentDirectory(file); // in a save operation, if we get a directory, move
																// there
					else
	...
	/**
	 * Returns whether this transfer flavor is supported. We support File Lists and
	 * Strings (plain or as list of URLs).
	 */
