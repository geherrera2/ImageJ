...
public class Opener {
	...
	public static final int UNKNOWN = 0, TIFF = 1, DICOM = 2, FITS = 3, PGM = 4, JPEG = 5,
			...
			TIFF_AND_DICOM = 14, CUSTOM = 15, AVI = 16, OJJ = 17, TABLE = 18, RAW = 19; // don't forget to also update
																						// 'types'
		...
		if (imp != null) {
			...
			switch (this.fileType) {
				...
				case UNKNOWN:
					...
					String msg = (f.exists()) ? "Format not supported or reader plugin not found:"
							: "File not found:";
					if (path != null) {
						if (path.length() > 64)
		...
		try {
			EventQueue.invokeAndWait(new Runnable() {
				public void run() {
					...
					if (IJ.debugMode)
						IJLog.log("Opener.openMultiple: " + sdir + " " + dir);
	...
	public ImagePlus openImage(String directory, String name) {
		...
		if (IJ.debugMode)
			IJLog.log("openImage: \"" + types[this.fileType] + "\", " + path);
		switch (this.fileType) {
			case TIFF:
	...
	private ImagePlus openUsingHandleExtraFileTypes(String path) {
		...
		if (imp == null && (this.fileType == UNKNOWN || this.fileType == TIFF)
				&& WindowManager.getImageCount() == nImages)
	...
	/**
	 * Can't open imagej.nih.gov URLs due to encryption so redirect to imagej.net
	 * mirror.
	 */
	...
	public static ImagePlus openUsingImageIO(String path) {
		...
		if (img == null)
			return null;
		if (IJ.debugMode)
			IJLog.log("type=" + img.getType() + ", alpha=" + img.getColorModel().hasAlpha() + ", bands="
	...
	boolean allSameSizeAndType(FileInfo[] info) {
		...
		// if (IJ.debugMode) {
		// IJLog.log("sameSizeAndType: " + sameSizeAndType);
		// IJLog.log("contiguous: " + contiguous);
		// }
	 ...
	 * if (imp==null)
	 * return false;
	 * boolean rgb = imp.getStackSize()==3 && imp.getNChannels()==3 &&
	 * imp.getBitDepth()==8;
	 * if (!rgb)
	 * return false;
	 * for (int i=1; i<=3; i++) {
	 ...
	 * break;
	 * }
	 * if (!((i==0 && (bytes[255]&255)==255 && (bytes[511]&255)==0 &&
	 * (bytes[767]&255)==0)
	 * || (i==1 && (bytes[255]&255)==0 && (bytes[511]&255)==255 &&
	 * (bytes[767]&255)==0)
	 * || (i==2 && (bytes[255]&255)==0 && (bytes[511]&255)==0 &&
	 * (bytes[767]&255)==255))) {
	 * rgb = false;
	 * break;
	 * }
	...
	public int getFileType(String path) {
		...
		// IJLog.log("getFileType: "+ name+" "+b0+" "+b1+" "+b2+" "+b3);
		...
		if (b0 == 80 && (b1 == 49 || b1 == 52 || b1 == 50 || b1 == 53 || b1 == 51 || b1 == 54)
