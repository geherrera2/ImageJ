...
public class FileOpener {
	...
	void setCalibration(ImagePlus imp) {
		...
		if (stackSize > 1) {
			...
			if (frames == 0)
				frames = 1;
			// IJLog.log("setCalibration: "+channels+" "+slices+" "+frames);
	...
	public InputStream createInputStream(FileInfo fi) throws IOException, MalformedURLException {
		...
		if (fi.inputStream != null)
			...
			if (fi.directory != null && fi.directory.length() > 0
					&& !(fi.directory.endsWith(Prefs.separator) || fi.directory.endsWith("/")))
	...
	static void error(String msg, FileInfo fi, long offset, long length) {
		...
		if (silentMode) {
			IJLog.log("Error opening " + fi.getFilePath());
			IJLog.log(msg2);
		} else
	...
	public Properties decodeDescriptionString(FileInfo fi) {
		if (fi.description == null || fi.description.length() < 7)
			return null;
		if (IJ.debugMode)
			IJLog.log("Image Description: " + new String(fi.description).replace('\n', ' '));
