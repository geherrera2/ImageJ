...
public class CompositeImage extends ImagePlus {
	...
	public static final Color[] colors = { Color.red, Color.green, Color.blue, Color.white, Color.cyan, Color.magenta,
	...
	public CompositeImage(ImagePlus imp, int mode) {
		...
		if (IJ.debugMode)
			IJLog.log("CompositeImage: " + imp + " " + mode + " " + channels);
		ImageStack stack2;
	...
	public synchronized void updateImage() {
		...
		// IJLog.log(nChannels+" "+ch+" "+currentChannel+" "+newChannel);
		...
		if (singleChannel && nChannels <= 3) {
			switch (currentChannel) {
				case 0:
					cip[0].updateComposite(rgbPixels, ImageProcessor.UPDATE_RED);
					break;
				case 1:
					cip[1].updateComposite(rgbPixels, ImageProcessor.UPDATE_GREEN);
					break;
				case 2:
					cip[2].updateComposite(rgbPixels, ImageProcessor.UPDATE_BLUE);
					break;
			}
			...
			for (int i = 1; i < nChannels; i++)
				if (active[i])
					cip[i].updateComposite(rgbPixels, projectionMode);
		}
		if (IJ.debugMode)
			IJLog.log("" + (System.nanoTime() - t0) / 1000L);
		createBufferedImage();
	...
	private void makeInvertedComposite(boolean[] chnActive) {
		...
		for (int c = 0; c < nChn; c++) {
			...
			switch (bitDepth) {
				case 8:
					in8[c] = (byte[]) cip[c].getPixels();
					break;
				case 16:
					in16[c] = (short[]) cip[c].getPixels();
					break;
				case 32:
					in32[c] = (float[]) cip[c].getPixels();
					break;
			}
		}
		...
		for (int idx = 0; idx < w * h; idx++) {
			for (int c = 0; c < nChn; c++) {
				switch (bitDepth) {
					case 8:
						v[c] = (int) Math.floor(((in8[c][idx] & 0xff) - mins[c]) * scale[c]);
						break;
					case 16:
						v[c] = (int) Math.floor(((in16[c][idx] & 0xffff) - mins[c]) * scale[c]);
						break;
					case 32:
						v[c] = (int) Math.floor((in32[c][idx] - mins[c]) * scale[c]);
						break;
				}
