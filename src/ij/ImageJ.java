...
public class ImageJ extends Frame implements ActionListener,
	...
	public ImageJ(java.applet.Applet applet, int mode) {
		...
		if (IJ.debugMode)
			IJLog.log("ImageJ starting in debug mode: " + mode);
		...
		if (mode != NO_SHOW) {
			...
			if (size != null) {
				...
				if (IJ.isWindows() && (size.height > 108 || IJ.javaVersion() >= 10)) {
					// workaround for IJ window layout and FileDialog freeze problems with Windows
					// 10 Creators Update
					IJ.wait(10);
					pack();
					if (IJ.debugMode)
	...
	public void actionPerformed(ActionEvent e) {
		if ((e.getSource() instanceof MenuItem)) {
			...
			lastKeyCommand = null;
			if (IJ.debugMode)
				IJLog.log("actionPerformed: time=" + ellapsedTime + ", " + e);
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (IJ.debugMode)
			IJLog.log("Windows: " + WindowManager.getWindowCount());
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		int flags = e.getModifiers();
		if (IJ.debugMode)
			IJLog.log("keyPressed: code=" + keyCode + " (" + KeyEvent.getKeyText(keyCode)
	...
	public void keyTyped(KeyEvent e) {
		char keyChar = e.getKeyChar();
		int flags = e.getModifiers();
		// if (IJ.debugMode) IJLog.log("keyTyped: char=\"" + keyChar + "\" (" +
		// (int)keyChar
	...
	public void windowActivated(WindowEvent e) {
		if (IJ.isMacintosh() && !quitting) {
			...
			if (mb != null && mb != getMenuBar()) {
				...
				// if (IJ.debugMode) IJLog.log("setMenuBar: "+Menus.setMenuBarCount);
			}
		}
	}
