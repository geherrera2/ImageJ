...
/**
 * An object of this type is a tokenized macro file and the associated symbol
 * table.
 */
public class Program implements MacroConstants {
	...
	public Program() {
		...
		if (IJ.debugMode)
			IJLog.log("Symbol table: " + (stLoc + 1) + "  " + table.length + "  " + systemTable.length);
	}
	...
	void enlargeSymbolTable() {
		...
		maxSymbols *= 2;
		if (IJ.debugMode)
			IJLog.log("enlargeSymbolTable: " + table.length);
	}
	...
	public void dumpProgram() {
		IJLog.log("");
		IJLog.log("Tokenized Program");
		String str;
		int token, address;
		for (int i = 0; i <= pc; i++)
			IJLog.log(i + "	 " + lineNumbers[i] + "   " + (code[i] & TOK_MASK) + "   " + decodeToken(code[i]));
	}
	...
	public String toString() {
		return "pgm[code=" + (code != null ? "" + code.length : "null") + " tab="
				+ (table != null ? "" + table.length : "null") + "]";
	}
