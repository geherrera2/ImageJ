...
public class Interpreter implements MacroConstants {
	...
	final void getToken() {
		if (done)
			return;
		token = pgm.code[++pc];
		// IJLog.log(pc+" "+pgm.decodeToken(token));
		if (token <= 127)
			return;
	...
	void doFor() {
		...
		while (true) {
			...
			if (startPC != 0)
				...
				while (token != ')') {
					getToken();
					// IJLog.log(pgm.decodeToken(token,tokenAddress));
	...
	final void doUserFunctionAssignment() {
		// IJLog.log("doUserFunctionAssignment0: "+pgm.decodeToken(token,
		// tokenAddress));
		putTokenBack();
		int savePC = pc;
		getToken(); // the variable
	...
	final double getFactor() {
		...
		// IJLog.log("getFactor: "+value+" "+pgm.decodeToken(preToken,0));
		return value;
	}
	...
	final Variable lookupLocalVariable(int symTabAddress) {
		// IJLog.log("lookupLocalVariable: "+topOfStack+" "+startOfLocals+"
		// "+topOfGlobals);
		Variable v = null;
	...
	void dumpStack() {
		...
		if (stack != null) {
			for (int i = topOfStack; i >= 0; i--) {
				...
				IJLog.log(i + " " + (symbol != null ? symbol.str : "null") + " " + v);
			}
		}
	}
	...
	public static void abort() {
		// IJLog.log("abort: "+(instance!=null?""+instance.hashCode():"null"));
		if (instance != null)
			instance.abortMacro();
	}
	...
	/**
	 * Returns the names of all variables and functions with human-readable
	 * annotations
	 */
	...
	public void showArrayInspector(int row) {
		...
		if (stack.length > stkPos && stkPos >= 0) {
			...
			if (var.getType() == Variable.ARRAY) {
				...
				String oldText = txtPanel.getText();// possible NullPointer at
													// ij.text.TextPanel.getText(TextPanel.java:875) vData == null
