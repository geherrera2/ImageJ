...
public class Tokenizer implements MacroConstants {
	...
	public Program tokenize(String program) {
		...
		if (pgm.hasFunctions)
			addUserFunctions();
		// IJLog.log(program.length()+" "+pgm.getSize()+"
		// "+IJ.d2s((double)program.length()/pgm.getSize(),1)+" "+program.length()/10);
		return pgm;
	}
				...
				if (nextToken == WORD || (nextToken >= PREDEFINED_FUNCTION && nextToken <= ARRAY_FUNCTION)) {
					...
					for (int j = 0; j < code.length; j++) {
						...
						if ((token == WORD || (token >= PREDEFINED_FUNCTION && token <= ARRAY_FUNCTION))
							...
							// IJLog.log((code[j]&TOK_MASK)+" "+(code[j]>>TOK_SHIFT)+" "+USER_FUNCTION+"
							// "+address);
						} else if (token == EOF)
							break;
					}
					// IJLog.log(i+" "+pgm.decodeToken(nextToken, address));
				}
			} else if (token == EOF)
				break;
		}
